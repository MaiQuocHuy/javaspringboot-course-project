...
public class InstructorSectionServiceImp implements InstructorSectionService {
  ...
  public ResponseEntity<ApiResponse<List<SectionWithLessonsDto>>> getCourseSections(
    ...
    try {
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve course sections. Please try again later.");
    }
  }

  private SectionWithLessonsDto convertToSectionWithLessonsDto(
      Section section, String currentUserId) {
  ...
  public ResponseEntity<ApiResponse<SectionResponseDto>> createSection(
    ...
    log.info(
        "Creating section for courseId: {}, instructorId: {}, title: {}",
        courseId,
        instructorId,
    ...
    try {
      ...
      SectionResponseDto responseDto =
          SectionResponseDto.builder()
  ...
  public ResponseEntity<ApiResponse<SectionResponseDto>> updateSection(
    ...
    log.info(
        "Updating section {} for courseId: {}, instructorId: {}, title: {}, description: {}",
        sectionId,
        courseId,
        instructorId,
        updateSectionDto.getTitle(),
        updateSectionDto.getDescription());
      ...
      try {
        ...
        if (instructor != null) {
          ...
          systemLogHelper.logUpdate(
              instructor, "Section", updatedSection.getId(), oldSectionLogDto, newSectionLogDto);
  ...
  public ResponseEntity<ApiResponse<Void>> deleteSection(
    ...
    log.info(
        "Deleting section {} for courseId: {}, instructorId: {}",
        sectionId,
        courseId,
        instructorId);
    ...
    try {
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to delete section. Please try again later.");
    }
  }

  /**
   * Reorders sections after deletion to maintain continuous order values. Sections with order index
   * greater than the deleted section's order will have their order decreased by 1.
   */
  private void reorderSectionsAfterDeletion(String courseId, Integer deletedOrderIndex) {
    log.info(
        "Reordering sections for course {} after deletion at order {}",
        courseId,
        deletedOrderIndex);

    try {
      // Get all sections with order greater than the deleted section
      List<Section> sectionsToReorder =
          sectionRepository.findSectionsByCourseIdAndOrderGreaterThan(courseId, deletedOrderIndex);
  ...
  public ResponseEntity<ApiResponse<Void>> reorderSections(
    ...
    log.info(
        "Reordering sections for courseId: {}, instructorId: {}, new order: {}",
        courseId,
        instructorId,
    ...
    try {
      ...
      List<Section> currentSections =
          sectionRepository.findSectionsByCourseIdOrderByOrder(courseId);

      // Validate the reorder request
      String validationError =
          validateSectionOrder(currentSections, reorderSectionsDto.getSectionOrder());
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to reorder sections. Please try again later.");
    }
  }

  /**
   * Validates that the provided section order contains all and only the sections that belong to the
   * course, with no duplicates.
   */
  private String validateSectionOrder(List<Section> currentSections, List<String> newOrder) {
    ...
    if (currentSections.size() != newOrder.size()) {
      return String.format(
          "Section order must contain exactly %d sections, but got %d",
    ...
    for (String sectionId : newOrder) {
      if (!currentSectionIds.contains(sectionId)) {
        return String.format(
            "Section ID %s does not exist or does not belong to this course", sectionId);
      }
    }
  ...
  /**
   * Updates the order index of sections based on the new order list. Each section gets an order
   * index equal to its position in the list (0-based). Uses a two-phase approach with dynamic
   * offset calculation to avoid unique constraint violations.
   */
  @Transactional
  private void reorderSectionsWithNewOrder(List<Section> sections, List<String> newOrder) {
    ...
    try {
      ...
      Map<String, Section> sectionMap =
          sections.stream().collect(Collectors.toMap(Section::getId, section -> section));

      // Build the ordered list of sections to update
      List<Section> sectionsToUpdate =
          newOrder.stream()
              .map(sectionMap::get)
      ...
      Integer maxCurrentOrder = sections.stream().mapToInt(Section::getOrderIndex).max().orElse(0);
      ...
      log.info(
          "Step 1 complete: Set temporary offset order for {} sections", sectionsToUpdate.size());
