...
/**
 * Cache service for instructor statistics operations
 * Handles caching of instructor dashboard statistics with appropriate TTL
 * and provides clean separation between service DTOs and cache DTOs
 */
@Service
@Slf4j
...
public class InstructorStatisticsCacheService {
    ...
    public InsDashboardDto getInstructorStatistics(String instructorId) {
        try {
            ...
            log.error("Error retrieving instructor statistics from cache for instructor: {}", instructorId, e);
            return null;
        }
    }
     ...
     *
     * @param instructorId         the instructor identifier
     * @param instructorStatistics the statistics to cache
     */
    public void storeInstructorStatistics(String instructorId, InsDashboardDto instructorStatistics) {
        try {
            ...
            log.debug("Successfully cached instructor statistics for instructor: {} with TTL: {} seconds",
                    instructorId, CacheConstants.INSTRUCTOR_STATISTICS_TTL.toSeconds());
    ...
    public void invalidateInstructorStatistics(String instructorId) {
        try {
            ...
            if (Boolean.TRUE.equals(deleted)) {
                log.debug("Successfully invalidated instructor statistics cache for instructor: {}", instructorId);
            } else {
                log.debug("No cache entry found to invalidate for instructor: {}", instructorId);
            }

        } catch (Exception e) {
            log.error("Error invalidating instructor statistics cache for instructor: {}", instructorId, e);
        }
    }
    ...
    public boolean isStatisticsCached(String instructorId) {
        try {
            ...
            log.error("Error checking instructor statistics cache existence for instructor: {}", instructorId, e);
            return false;
        }
    }
}