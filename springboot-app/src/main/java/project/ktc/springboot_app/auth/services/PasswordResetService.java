 ...
 *
 * <p>This service manages the complete password reset flow including: - OTP generation and email
 * sending - OTP verification and validation - Password update with security measures - Rate
 * limiting and abuse prevention
 *
 * <p>Security features: - Rate limiting to prevent spam - Secure OTP generation - Token expiration
 * and attempt limits - Previous token invalidation - Input validation and sanitization
 *
 * @author KTC Team
 * @version 1.0
 * @since 2025-01-26
...
public class PasswordResetService {
  ...
  public ForgotPasswordResponseDto initiateForgotPassword(ForgotPasswordRequestDto request) {
    ...
    return ForgotPasswordResponseDto.success(
        request.getEmail(), expiresAt, maxAttempts, previousTokensInvalidated);
  }
  ...
  public void confirmPasswordReset(ResetPasswordConfirmDto request) {
    ...
    Optional<PasswordResetToken> tokenOpt =
        passwordResetTokenRepository.findValidTokenByUserIdAndOtpCode(
            user.getId(), request.getOtpCode(), now);

    if (tokenOpt.isEmpty()) {
      // Check if there's a token with this OTP but it's invalid
      Optional<PasswordResetToken> invalidTokenOpt =
          passwordResetTokenRepository.findActiveTokenByOtpCode(request.getOtpCode(), now);
  ...
  private void checkRateLimit(String userId) {
    ...
    long recentRequests =
        passwordResetTokenRepository.countTokensCreatedByUserSince(userId, hourAgo);
    ...
    Optional<PasswordResetToken> recentToken =
        passwordResetTokenRepository.findLatestTokenByUserId(userId);

    if (recentToken.isPresent() && recentToken.get().getCreatedAt().isAfter(rateLimitWindow)) {
      long waitMinutes =
          rateLimitMinutes
              - java.time.Duration.between(recentToken.get().getCreatedAt(), LocalDateTime.now())
                  .toMinutes();
      throw new BusinessLogicException(
          String.format(
              "Please wait %d minute(s) before requesting another OTP.", Math.max(1, waitMinutes)));
    }
  }
  ...
  /**
   * Cleanup method to remove expired tokens (for scheduled execution). Should be called
