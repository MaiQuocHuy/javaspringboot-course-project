...
public interface InstructorRefundRepository extends JpaRepository<Refund, String> {
  ...
  /**
   * Get all refunds with pagination support for instructor Orders by creation date descending (most
  ...
  Page<Refund> findAllRefundsByInstructorId(
      @Param("instructorId") String instructorId, Pageable pageable);

  /**
   * Get all payments without pagination for instructor Orders by creation date descending (most
  ...
  /**
   * Get refund by ID with full details including user and course for instructor Returns refund with
  ...
  Optional<Refund> findRefundByIdAndInstructorIdWithDetails(
      @Param("refundId") String refundId, @Param("instructorId") String instructorId);

  /**
   * Get all refunds with search and filter support for instructor (paginated) Search by refund ID,
  ...
  /**
   * Get all refunds with search and filter support for instructor (non-paginated) Search by refund
   * ID, user name, or reason Filter by status and date range
   */
  @Query(
      "SELECT r FROM Refund r "
          + "LEFT JOIN FETCH r.payment p "
