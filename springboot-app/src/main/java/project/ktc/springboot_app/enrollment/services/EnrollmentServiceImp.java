...
public class EnrollmentServiceImp implements EnrollmentService {
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<MyEnrolledCourseDto>>> getMyCourses(
    ...
    Page<Enrollment> enrollmentsPage =
        enrollmentRepository.findByUserIdWithCourseAndInstructor(currentUserId, status, pageable);

    // Convert to DTOs with progress calculation
    List<MyEnrolledCourseDto> enrolledCourses =
        enrollmentsPage.getContent().stream()
            .map(this::mapToMyEnrolledCourseDto)
            .collect(Collectors.toList());

    // Create paginated response
    PaginatedResponse<MyEnrolledCourseDto> paginatedResponse =
        PaginatedResponse.<MyEnrolledCourseDto>builder()
  ...
  public ResponseEntity<ApiResponse<List<MyEnrolledCourseDto>>> getMyCourses(
    ...
    List<Enrollment> enrollments =
        enrollmentRepository.findByUserIdWithCourseAndInstructor(currentUserId, status);

    // Convert to DTOs with progress calculation
    List<MyEnrolledCourseDto> enrolledCourses =
        enrollments.stream().map(this::mapToMyEnrolledCourseDto).collect(Collectors.toList());
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<MyEnrolledCourseDto>>> getMyCourses(
    ...
    try {
      enrollmentsPage =
          enrollmentRepository.findByUserIdWithSearchAndFilter(
  ...
  public ResponseEntity<ApiResponse<List<MyEnrolledCourseDto>>> getRecentCourses() {
    try {
      ...
      List<Enrollment> recentEnrollments =
          enrollmentRepository.findTop3RecentEnrollmentsByUserId(currentUserId).stream()
              .limit(3)
              .collect(Collectors.toList());

      List<MyEnrolledCourseDto> recentCourses =
  ...
  public ResponseEntity<ApiResponse<StudentDashboardStatsDto>> getDashboardStats() {
    try {
      ...
      Long completedCourses =
          enrollmentRepository.countEnrollmentsByUserIdAndStatus(
      ...
      Long lessonsCompleted =
          enrollmentRepository.countTotalCompletedLessonsByUserId(currentUserId);

      // Get total lessons in all enrolled courses
      Long totalLessons =
          enrollmentRepository.countTotalLessonsInEnrolledCoursesByUserId(currentUserId);

      // Calculate overall progress
      Double overallProgress = 0.0;
      if (totalLessons != null && totalLessons > 0 && lessonsCompleted != null) {
        overallProgress =
            BigDecimal.valueOf((double) lessonsCompleted / totalLessons)
                .setScale(4, RoundingMode.HALF_UP)
                .doubleValue();
      }

      StudentDashboardStatsDto stats =
          StudentDashboardStatsDto.builder()
  ...
  public ResponseEntity<ApiResponse<List<StudentActivityDto>>> getRecentActivities(Integer limit) {
    try {
      ...
      int perTypeLimit =
          Math.max(1, activityLimit / 3); // Distribute equally among 3 activity types

      org.springframework.data.domain.Pageable pageable =
          org.springframework.data.domain.PageRequest.of(0, perTypeLimit);

      // Fetch recent enrollments
      List<Enrollment> recentEnrollments =
          enrollmentRepository.findRecentEnrollmentsByUserId(currentUserId, pageable);
      List<StudentActivityDto> enrollmentActivities =
          recentEnrollments.stream()
      ...
      List<LessonCompletion> recentCompletions =
          lessonCompletionRepository.findRecentCompletionsByUserId(currentUserId, pageable);
      List<StudentActivityDto> completionActivities =
          recentCompletions.stream()
              .map(this::mapLessonCompletionToActivity)
              .collect(Collectors.toList());

      // Fetch recent quiz submissions
      List<QuizResult> recentSubmissions =
          quizResultRepository.findRecentSubmissionsByUserId(currentUserId, pageable);
      List<StudentActivityDto> submissionActivities =
      ...
      List<StudentActivityDto> sortedActivities =
  ...
  private StudentActivityDto mapQuizResultToActivity(QuizResult quizResult) {
    double scorePercentage =
        quizResult.getScore() != null ? quizResult.getScore().doubleValue() : 0.0;
    return StudentActivityDto.builder()
  ...
  /** Handle pagination when progress filter is applied - requires fetching all data first */
  private ResponseEntity<ApiResponse<PaginatedResponse<MyEnrolledCourseDto>>>
      getMyCourses_WithProgressFilter(
    ...
    List<MyEnrolledCourseDto> allEnrolledCourses =
        allEnrollmentsPage.getContent().stream()
            .map(this::mapToMyEnrolledCourseDto)
            .collect(Collectors.toList());

    // Apply progress filter
    List<MyEnrolledCourseDto> filteredCourses =
        filterByProgress(allEnrolledCourses, progressFilter);
    ...
    List<MyEnrolledCourseDto> pageContent =
        (startIndex < totalElements) ? filteredCourses.subList(startIndex, endIndex) : List.of();
