...
/**
 * AffiliatePayout entity to track commission payouts for referral discounts Records payouts to
...
@Table(
    ...
    indexes = {
      @Index(
          name = "idx_affiliate_payout_user_status",
          columnList = "referred_by_user_id, payout_status"),
...
public class AffiliatePayout extends BaseEntity {
  ...
  /**
   * Link to the discount usage that triggered this payout Can be null if payout is created
   * independently
   */
  @ManyToOne(fetch = FetchType.LAZY)
