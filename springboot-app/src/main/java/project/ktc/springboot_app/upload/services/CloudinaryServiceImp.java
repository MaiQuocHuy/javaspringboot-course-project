...
public class CloudinaryServiceImp implements CloudinaryService {
	...
	public ImageUploadResponseDto uploadImage(MultipartFile file) {
		...
		try {
			...
			log.info(
					"Image uploaded successfully. URL: {}, Public ID: {}",
					response.getUrl(),
	 ...
	 */
	@Override
	public ImageUploadResponseDto uploadCertificateImage(byte[] imageData, String filename)
			throws IOException {
		...
		try {
			...
			log.info(
					"Certificate image uploaded successfully. URL: {}, Public ID: {}",
					response.getUrl(),
					response.getPublicId());
	...
	/**
	 * Validate uploaded file /** Generate unique public ID for the uploaded file
	 */
	...
	public VideoUploadResponseDto uploadVideo(MultipartFile file) {
		...
		try {
			...
			log.info(
					"Video upload successful. Public ID: {}, Secure URL: {}",
	 ...
	 *
	 * @param publicId
	 *            Public ID of the video to delete
	...
	private ImageUploadResponseDto buildResponseDto(
			Map<String, Object> uploadResult, MultipartFile file) {
	...
	private ImageUploadResponseDto buildResponseDto(
			Map<String, Object> uploadResult, String filename) {
		return ImageUploadResponseDto.builder()
				.url((String) uploadResult.get("secure_url"))
				.publicId((String) uploadResult.get("public_id"))
			...
			Map<String, Object> uploadResult = cloudinary
					...
					.upload(
							...
							ObjectUtils.asMap(
									...
									"folder", "course-audio",
									"resource_type",
									"video", // Audio is uploaded as video resource type in Cloudinary
									"quality", "auto",
									"format", "mp3" // Convert to MP3 for consistency
							));

			log.info(
					"Audio upload successful. Public ID: {}, URL: {}",
					uploadResult.get("public_id"),
					uploadResult.get("secure_url"));
	 ...
	 *
	 * @param publicId
	 *            Public ID of the audio to delete
	 * @return true if deletion was successful, false otherwise
	 */
	@Override
		...
		try {
			...
			Map<String, Object> result = cloudinary
					...
					.destroy(
							publicId,
							ObjectUtils.asMap(
									"resource_type", "video")); // Audio is stored as video resource type
	...
	private AudioUploadResponseDto buildAudioResponseDto(
			Map<String, Object> uploadResult, MultipartFile file) {
		...
		try {
			...
			Map<String, Object> uploadResult = cloudinary
					...
					.upload(
							file.getBytes(),
							ObjectUtils.asMap(
									"public_id", publicId, "resource_type", "raw" // Use raw for documents
							));

			log.info(
					"Document upload successful. Public ID: {}, URL: {}",
					uploadResult.get("public_id"),
					uploadResult.get("secure_url"));
	 ...
	 *
	 * @param publicId
	 *            Public ID of the document to delete
	 * @return true if deletion was successful, false otherwise
	 */
	@Override
	...
	private DocumentUploadResponseDto buildDocumentResponseDto(
			Map<String, Object> uploadResult, MultipartFile file) {
	...
	private VideoUploadResponseDto buildVideoResponseDto(
			Map<String, Object> uploadResult, MultipartFile file) {
	 ...
	 *
	 * @param videoUrlOrPublicId
	 *            Cloudinary video URL or public ID
	...
	public VideoMetadataResponseDto getVideoMetadata(String videoUrlOrPublicId) throws IOException {
		...
		try {
			...
			@SuppressWarnings("unchecked")
			Map<String, Object> resourceDetails = cloudinary
					.api()
					.resource(
							publicId,
							ObjectUtils.asMap(
	...
	/**
	 * Generate download URL for certificate PDF with attachment flag This forces
	...
	private String generateDownloadUrl(String publicId, String filename) {
		try {
			...
			// attachment flag
			String baseUrl = cloudinary.url().resourceType("raw").generate(publicId);
	 ...
	 */
	@Override
	public DocumentUploadResponseDto uploadCertificatePdf(byte[] pdfData, String filename)
			throws IOException {
		...
		try {
			...
			log.info(
					"Certificate PDF upload successful. Public ID: {}, URL: {}",
					uploadResult.get("public_id"),
					uploadResult.get("secure_url"));
