...
/**
 * Service to handle background processing tasks for completed payments
 * This keeps the main webhook response fast by offloading heavy operations
 */
@Service
@Slf4j
...
public class PaymentBackgroundProcessingService {
    ...
    /**
     * Processes all background tasks for a completed payment asynchronously
     * This method is called from the webhook handler to avoid blocking the response
     */
    @Async("taskExecutor")
    @Transactional
     ...
     */
    private void sendPaymentConfirmationEmailAsync(User user, Course course, Payment payment, String sessionId) {
        try {
            log.info("📧 Sending payment confirmation email for user: {} course: {}", user.getId(), course.getId());
