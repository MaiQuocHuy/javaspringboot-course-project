...
public class EnrollmentController {
  ...
  @Operation(
      ...
      description =
          "Enrolls the authenticated student into a specified course (free or paid). For paid courses, payment must be completed before enrollment.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
        ...
        @ApiResponse(
            responseCode = "400",
            description = "Course is unpublished or deleted",
            content =
                @Content(
                    mediaType = "application/json",
        ...
        @ApiResponse(
            responseCode = "402",
            description = "Payment not completed or amount insufficient",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ApiErrorResponse.class))),
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EnrollmentResponseDto>>
      enroll(@Parameter(description = "Course ID", required = true) @PathVariable String id) {
    return enrollmentService.enroll(id);
  }

  @GetMapping("/api/courses/{id}/enrollment-debug")
  @Operation(
      summary = "Debug enrollment status",
      description = "Temporary debug endpoint to check enrollment status")
  public ResponseEntity<Object> debugEnrollmentStatus(@PathVariable String id) {
    try {
      ...
      if (authentication != null
        ...
        Boolean isEnrolled =
            enrollmentRepository.existsByUserIdAndCourseId(currentUser.getId(), id);
