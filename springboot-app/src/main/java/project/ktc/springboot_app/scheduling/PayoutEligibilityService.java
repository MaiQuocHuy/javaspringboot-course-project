...
/**
 * Service for checking payout eligibility of payments Implements the same
 * business logic as the
 * manual payout API
 */
@Service
...
public class PayoutEligibilityService {
	...
	public List<Payment> findEligiblePayments() {
		...
		// Get all completed payments with refunds loaded to avoid
		// LazyInitializationException
		List<Payment> completedPayments = adminPaymentRepository
		...
		List<Payment> unpaidPayments = completedPayments.stream().filter(payment -> payment.getPaidOutAt() == null)
				.toList();
	...
	public boolean isPaymentEligibleForPayout(Payment payment) {
		try {
			...
			if (timeSinceCompletion.toDays() < payoutProperties.getWaiting().getPeriod().getDays()) {
				long hoursRemaining = (payoutProperties.getWaiting().getPeriod().getDays() * 24)
			...
			try {
				if (payment.getRefunds() != null && !payment.getRefunds().isEmpty()) {
					boolean hasPendingRefund = payment.getRefunds().stream()
							.anyMatch(
									refund -> refund != null && refund.getStatus() == Refund.RefundStatus.PENDING);
					boolean hasCompletedRefund = payment.getRefunds().stream()
							.anyMatch(
									refund -> refund != null && refund.getStatus() == Refund.RefundStatus.COMPLETED);
	...
	public PayoutEligibilitySummary getEligibilitySummary() {
		...
		try {
			...
			long alreadyPaidOut = completedPayments.stream().filter(p -> p.getPaidOutAt() != null).count();
	...
	private boolean hasBlockingRefunds(Payment payment) {
		...
		return payment.getRefunds().stream()
				.anyMatch(
						refund -> refund.getStatus() == Refund.RefundStatus.PENDING
