...
public class NotificationHelper {
  ...
  /**
   * Get all users who have a specific permission with filter-type-001 (ALL access) for notification
   * targeting Only users with ALL access permission should receive admin notifications
   */
  ...
  /**
   * Admin Notification 1: Student thanh toán thành công (MEDIUM priority) Notify users with
  ...
  /**
   * Admin Notification 2: Payment thay đổi trạng thái từ PENDING → COMPLETED (HIGH priority) Notify
  ...
  /**
   * Admin Notification 3: Có khóa học mới cần duyệt (HIGH priority) Notify users with
  ...
  /**
   * Admin Notification 4: Có đơn đăng ký instructor cần duyệt (HIGH priority) Notify users with
   * instructor_application:READ permission when a new instructor application needs approval
   */
  ...
  /**
   * Instructor Notification 1: Khóa học được duyệt (HIGH priority) Notify instructor when their
  ...
  /**
   * Instructor Notification 2: Khóa học bị từ chối (HIGH priority) Notify instructor when their
  ...
  /**
   * Instructor Notification 3: Có student mới tham gia khóa học (MEDIUM priority) Notify instructor
   * when a new student enrolls in their course
   */
  public CompletableFuture<NotificationResponseDto>
    ...
    String message =
        String.format("📚 %s has just enrolled in your course '%s'", studentName, courseName);
  ...
  /**
   * Instructor Notification 4: Application submitted (MEDIUM priority) Notify user when they submit
   * an instructor application and need to wait for approval
   */
  public CompletableFuture<NotificationResponseDto>
      createInstructorApplicationSubmittedNotification(String userId, String applicationId) {
    // Validate inputs
    if (userId == null || userId.trim().isEmpty()) {
      log.error(
    ...
    String message =
        "📝 Your instructor application has been submitted successfully! Please wait for admin approval before you can create courses.";
  ...
  /**
   * Instructor Notification 5: New refund request (MEDIUM priority) Notify instructor when a
  ...
  /**
   * Student Notification 1: Instructor Application được phê duyệt (HIGH priority) Notify student
  ...
  /**
   * Student Notification 2: Instructor Application bị từ chối (HIGH priority) Notify student when
