...
public class InstructorSectionServiceImp implements InstructorSectionService {
	 ...
	 * Sections with order index
	 * greater than the deleted section's order will have their order decreased by
	 * 1.
	 */
	private void reorderSectionsAfterDeletion(String courseId, Integer deletedOrderIndex) {
		...
		try {
			...
			List<Section> sectionsToReorder = sectionRepository.findSectionsByCourseIdAndOrderGreaterThan(courseId,
					deletedOrderIndex);
	...
	/**
	 * Updates the order index of sections based on the new order list. Each section
	 * gets an order
	 * index equal to its position in the list (0-based). Uses a two-phase approach
	 * with dynamic
