...
public class CourseServiceImp implements CourseService {
	...
	public ResponseEntity<ApiResponse<PaginatedResponse<CoursePublicResponseDto>>> findAllPublic(
		...
		List<CoursePublicResponseDto> courseResponses = sharedData.getCoursesWithCategories().stream()
				.map(
						course -> mapToCoursePublicResponse(
								course,
	...
	private SharedCourseDataDto getSharedCourseData(
		...
		if (averageRating == null && (categoryIds == null || categoryIds.size() <= 1)) {
			try {
				String singleCategoryId = (categoryIds != null && !categoryIds.isEmpty()) ? categoryIds.get(0) : null;
		...
		List<Course> coursesWithCategories = coursePage.getContent().stream()
				.map(
						course -> {
							Optional<Course> courseWithCats = courseRepository.findCourseWithCategories(course.getId());
							log.debug(
		...
		List<String> courseIds = coursesWithCategories.stream().map(Course::getId).collect(Collectors.toList());
	...
	private Map<String, Boolean> getUserEnrollmentStatus(
		...
		try {
			cachedEnrollmentStatus = coursesCacheService.getUserEnrollmentStatus(currentUserId, courseIdsKey);
		...
		Map<String, Boolean> enrollmentStatus = courseIds.stream()
				.collect(
						Collectors.toMap(
								courseId -> courseId,
								courseId -> enrollmentRepository.existsByUserIdAndCourseId(currentUserId, courseId)));
		log.debug(
	...
	public ResponseEntity<ApiResponse<PaginatedResponse<CourseAdminResponseDto>>> findCoursesForAdmin(
		...
		List<CourseAdminResponseDto> courseResponses = coursesWithCategories.stream()
				.map(
						course -> mapToCourseAdminResponse(
	 ...
	 * count Both
	 * queries now use consistent filtering for published, approved, and non-deleted
	 * courses
	 *
	 * @param instructorId
	 *            the instructor ID
	...
	private CourseDetailResponseDto.OverViewInstructorSummary getOverViewInstructorSummary(
		...
		try {
			Double averageRating = courseRepository.findAverageRatingByInstructorId(instructorId).orElse(0.0);
	...
	private CourseDetailResponseDto.SectionSummary mapToSectionSummary(Section section) {
		...
		if (section.getLessons() != null) {
			...
			sectionDuration = section.getLessons().stream()
					.filter(
							lesson -> lesson.getLessonType() != null
									&& "VIDEO".equals(lesson.getLessonType().getName())
									&& lesson.getContent() != null
	...
	private CoursePublicResponseDto mapToCoursePublicResponse(
		...
		if (course.getCategories() != null && !course.getCategories().isEmpty()) {
			categorySummaries = course.getCategories().stream()
					.map(
							cat -> CoursePublicResponseDto.CategorySummary.builder()
									.id(cat.getId())
									.name(cat.getName())
									.build())
					.collect(Collectors.toList());
	...
	public ResponseEntity<ApiResponse<CourseFilterMetadataResponseDto>> getCourseFilterMetadata() {
		try {
			...
			if (priceRange != null) {
				minPrice = priceRange.getMinPrice() != null
						? priceRange.getMinPrice()
						: new java.math.BigDecimal("0.0");
				maxPrice = priceRange.getMaxPrice() != null
			...
			CourseFilterMetadataResponseDto metadata = CourseFilterMetadataResponseDto.builder().minPrice(minPrice)
