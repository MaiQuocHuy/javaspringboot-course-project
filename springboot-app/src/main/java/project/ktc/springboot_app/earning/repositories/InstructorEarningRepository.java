...
public interface InstructorEarningRepository extends JpaRepository<InstructorEarning, String> {
  ...
  @Query(
      "SELECT ie FROM InstructorEarning ie "
  ...
  @Query(
      "SELECT COALESCE(SUM(ie.amount), 0) FROM InstructorEarning ie "
          + "WHERE ie.instructor.id = :instructorId "
          + "AND (:courseId IS NULL OR ie.course.id = :courseId) "
  ...
  @Query(
      "SELECT COUNT(ie) FROM InstructorEarning ie "
          + "WHERE ie.instructor.id = :instructorId "
          + "AND (:courseId IS NULL OR ie.course.id = :courseId) "
          + "AND (:dateFrom IS NULL OR ie.createdAt >= :dateFrom) "
          + "AND (:dateTo IS NULL OR ie.createdAt <= :dateTo)")
  ...
  @Query(
      "SELECT COALESCE(SUM(ie.amount), 0) as revenue FROM InstructorEarning ie "
  ...
  /** Find instructor earnings created between two dates Used for scheduled reports and analysis */
  @Query(
      "SELECT ie FROM InstructorEarning ie WHERE ie.createdAt BETWEEN :startDate AND :endDate ORDER BY ie.createdAt DESC")
