...
public class CourseServiceImp implements CourseService {
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<CoursePublicResponseDto>>> findAllPublic(
    ...
    List<String> processedCategoryIds =
    ...
    SharedCourseDataDto sharedData =
        getSharedCourseData(
            pageable.getPageNumber(),
    ...
    final Map<String, Boolean> enrollmentStatus =
        getUserEnrollmentStatus(currentUserId, sharedData);

    // ============ STEP 3: Build response by merging shared + user-specific data
    // ============

    List<CoursePublicResponseDto> courseResponses =
        sharedData.getCoursesWithCategories().stream()
            .map(
                course ->
                    mapToCoursePublicResponse(
                        course,
    ...
    PaginatedResponse<CoursePublicResponseDto> paginatedResponse =
  ...
  private SharedCourseDataDto getSharedCourseData(
      int pageNumber,
      int pageSize,
      String search,
      List<String> categoryIds,
    ...
    log.info(
        "🔍 getSharedCourseData called with categoryIds: {}, size: {}",
        categoryIds,
    ...
    if (averageRating == null && (categoryIds == null || categoryIds.size() <= 1)) {
      try {
        ...
        cachedData =
            coursesCacheService.getSharedCourseData(
                pageNumber,
                pageSize,
                search,
                singleCategoryId,
        ...
        if (cachedData != null) {
          log.info(
              "✅ Shared cache HIT - Found cached course data (courses + categories + enrollmentCounts)");
    ...
    Page<Course> coursePage =
        courseRepository.findPublishedCoursesWithFilters(
    ...
    List<Course> coursesWithCategories =
        coursePage.getContent().stream()
            .map(
                course -> {
                  Optional<Course> courseWithCats =
                      courseRepository.findCourseWithCategories(course.getId());
                  log.debug(
                      "Loaded categories for course {}: {}",
                      course.getId(),
    ...
    List<String> courseIds =
        coursesWithCategories.stream().map(Course::getId).collect(Collectors.toList());
  ...
  private Map<String, Boolean> getUserEnrollmentStatus(
      String currentUserId, SharedCourseDataDto sharedData) {
    ...
    try {
      cachedEnrollmentStatus =
          coursesCacheService.getUserEnrollmentStatus(currentUserId, courseIdsKey);
    ...
    Map<String, Boolean> enrollmentStatus =
        courseIds.stream()
            .collect(
                Collectors.toMap(
                    courseId -> courseId,
    ...
    log.debug(
        "Loaded enrollment status for {} courses for user: {}", courseIds.size(), currentUserId);
  ...
  public ResponseEntity<ApiResponse<CourseDetailResponseDto>> findOnePublic(String courseId) {
    ...
    Course course =
        courseRepository
            .findPublishedCourseByIdWithDetails(courseId)
            .orElseThrow(
    ...
    // Map to DTO
    CourseDetailResponseDto responseDto =
        mapToCourseDetailResponse(
            course,
            ratingSummary,
  ...
  public ResponseEntity<ApiResponse<CourseDetailResponseDto>> findOneBySlug(String slug) {
    ...
    // ============ STEP 1: Get shared course data (check cache first) ============
    CourseDetailResponseDto cachedCourse =
        coursesCacheService.getCourseDetailsBySlug(slug, CourseDetailResponseDto.class);
    ...
    if (cachedCourse != null) {
      ...
      Course course =
          courseRepository
              .findPublishedCourseBySlugWithDetails(slug)
              .orElseThrow(
                  () -> new ResourceNotFoundException("Course not found with slug: " + slug));

      // Step 2: Fetch categories separately to avoid MultipleBagFetchException
      Optional<Course> courseWithCategories =
          courseRepository.findCourseWithCategories(course.getId());
      if (courseWithCategories.isPresent()) {
        course.setCategories(courseWithCategories.get().getCategories());
      }
      ...
      // Step 3: Fetch sections with lessons separately
      List<Section> sectionsWithLessons =
          courseRepository.findSectionsWithLessonsByCourseId(course.getId());
      course.setSections(sectionsWithLessons);
      ...
      baseResponseDto =
          mapToCourseDetailResponse(
              ...
              sampleVideoUrl,
              slug,
              enrollMentCount.intValue()); // Always false for
      // cache
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<CourseAdminResponseDto>>> findCoursesForAdmin(
    ...
    Page<Course> coursePage =
        courseRepository.findCoursesForAdmin(
            isApproved,
            processedCategoryIds,
            search,
            minPrice,
    ...
    List<CourseAdminResponseDto> courseResponses =
        coursesWithCategories.stream()
            .map(
                course ->
                    mapToCourseAdminResponse(
                        course, enrollmentCounts.getOrDefault(course.getId(), 0L)))
            .collect(Collectors.toList());

    // Create paginated response
    PaginatedResponse<CourseAdminResponseDto> paginatedResponse =
        PaginatedResponse.<CourseAdminResponseDto>builder()
            .content(courseResponses)
            .page(
                PaginatedResponse.PageInfo.builder()
  ...
  /**
   * Get instructor overview statistics including average rating and total courses count Both
   * queries now use consistent filtering for published, approved, and non-deleted courses
   *
   * @param instructorId the instructor ID
   * @return OverViewInstructorSummary containing average rating and total courses
   */
  private CourseDetailResponseDto.OverViewInstructorSummary getOverViewInstructorSummary(
      String instructorId) {
    ...
    try {
      Double averageRating =
          courseRepository.findAverageRatingByInstructorId(instructorId).orElse(0.0);
  ...
  private CourseDetailResponseDto mapToCourseDetailResponse(
      Course course,
    ...
    if (course.getInstructor() != null) {
      instructorSummary =
          CourseDetailResponseDto.InstructorSummary.builder()
      ...
      for (Lesson lesson : section.getLessons()) {
        if (lesson.getLessonType() != null && "QUIZ".equals(lesson.getLessonType().getName())) {
          ...
          if (lesson.getContent() == null) {
            ...
            Long questionCount = courseRepository.countQuizQuestionsBySectionId(section.getId());
            sectionQuizCount += questionCount != null ? questionCount.intValue() : 0;
          } else {
  ...
  private CoursePublicResponseDto mapToCoursePublicResponse(
      Course course, Long enrollCount, Boolean isEnrolled) {
    ...
    if (course.getCategories() != null && !course.getCategories().isEmpty()) {
      categorySummaries =
          course.getCategories().stream()
              .map(
                  cat ->
                      CoursePublicResponseDto.CategorySummary.builder()
                          .id(cat.getId())
                          .name(cat.getName())
                          .build())
              .collect(Collectors.toList());
    ...
    if (course.getInstructor() != null) {
      instructorSum =
          CoursePublicResponseDto.InstructorSummary.builder()
              .id(course.getInstructor().getId())
              .name(course.getInstructor().getName())
    ...
    Integer totalHours =
        totalDurationSeconds != null ? (int) Math.ceil(totalDurationSeconds / 3600.0) : 0;
  ...
  private CourseAdminResponseDto mapToCourseAdminResponse(Course course, Long enrollCount) {
    ...
    if (course.getCategories() != null && !course.getCategories().isEmpty()) {
      course
          ...
          .forEach(
              cat -> {
                CourseAdminResponseDto.CategoryInfo categoryInfo =
                    CourseAdminResponseDto.CategoryInfo.builder()
                        .id(cat.getId())
                        .name(cat.getName())
                        .build();
    ...
    if (course.getInstructor() != null) {
      instructorInfo =
          CourseAdminResponseDto.InstructorInfo.builder()
              .id(course.getInstructor().getId())
              .name(course.getInstructor().getName())
  ...
  public ResponseEntity<ApiResponse<CourseApprovalResponseDto>> approveCourse(String courseId) {
    ...
    // Create response DTO
    CourseApprovalResponseDto responseDto =
        CourseApprovalResponseDto.builder()
  ...
  @Override
  public ResponseEntity<ApiResponse<List<SectionWithLessonsDto>>> getCourseDetailsForAdmin(
      String courseId) {
    ...
    // Map to DTOs
    List<SectionWithLessonsDto> sectionDtos =
        sectionsWithLessons.stream()
    ...
    if ("QUIZ".equals(lessonType)) {
      ...
      if (!questions.isEmpty()) {
        List<QuizQuestionDto> questionDtos =
            questions.stream().map(this::mapToQuizQuestionDto).collect(Collectors.toList());
  ...
  private QuizQuestionDto mapToQuizQuestionDto(QuizQuestion question) {
    ...
    try {
      ...
      log.warn(
          "Failed to parse quiz question options for question {}: {}",
          question.getId(),
          e.getMessage());
    }
  ...
  public ResponseEntity<ApiResponse<CourseFilterMetadataResponseDto>> getCourseFilterMetadata() {
    try {
      ...
      if (priceRange != null) {
        minPrice =
            priceRange.getMinPrice() != null
                ? priceRange.getMinPrice()
                : new java.math.BigDecimal("0.0");
        maxPrice =
            priceRange.getMaxPrice() != null
      ...
      CourseFilterMetadataResponseDto metadata =
          CourseFilterMetadataResponseDto.builder().minPrice(minPrice).maxPrice(maxPrice).build();
