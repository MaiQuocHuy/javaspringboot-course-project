...
public class ExtractPaymentDetailFromSessionId {
    ...
    public PaymentDetailDto extractPaymentDetails(String sessionId) throws StripeException {
        try {
            ...
            PaymentDetailDto result = PaymentDetailDto.builder()
                    .sessionId(sessionId)
                    .userId(userId)
                    .courseId(courseId)
                    .paymentId(paymentId)
                    ...
                    .discountApplied(discountAmount != null && discountAmount.compareTo(BigDecimal.ZERO) > 0)
                    .paymentStatus(session.getPaymentStatus())
                    .sessionStatus(session.getStatus())
                    .customerEmail(session.getCustomerEmail())
                    .totalAmount(session.getAmountTotal() != null
            ...
            log.error("Failed to extract payment details from session ID {}: {}", sessionId, e.getMessage(), e);
            throw e;
        } catch (Exception e) {
            log.error("Unexpected error extracting payment details from session ID {}: {}", sessionId, e.getMessage(),
                    e);
    ...
    public Map<String, String> extractMetadata(String sessionId) throws StripeException {
        try {
            ...
            Map<String, String> metadata = session.getMetadata();
            log.info("Successfully extracted metadata for session: {} with {} entries",
                    sessionId, metadata != null ? metadata.size() : 0);
