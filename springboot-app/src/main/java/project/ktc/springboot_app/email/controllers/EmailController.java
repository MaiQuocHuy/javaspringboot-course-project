...
/**
 * REST Controller for Email Service Provides endpoints for sending emails with various
 * configurations
 */
@RestController
...
@Tag(
    name = "Email API",
    description = "Endpoints for sending emails, managing templates, and email operations")
@Validated
...
public class EmailController {
  ...
  @Operation(
      summary = "Send email",
      description =
          "Send email with custom configuration including attachments, templates, and multiple recipients")
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "401",
            description = "Unauthorized - Admin or Instructor role required"),
        @ApiResponse(responseCode = "500", description = "Email sending failed")
      })
  @PreAuthorize("hasRole('ADMIN') or hasRole('INSTRUCTOR')")
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EmailSendResult>>
      sendEmail(
          @Valid @RequestBody @Parameter(description = "Email request with all necessary details")
    ...
    try {
      ...
      if (result.isSuccess()) {
        ...
        return ApiResponseUtil.error(
            HttpStatus.INTERNAL_SERVER_ERROR, "Email sending failed: " + result.getErrorMessage());
      }

    } catch (Exception e) {
      log.error("Error sending email: {}", e.getMessage(), e);
      return ApiResponseUtil.error(
          HttpStatus.INTERNAL_SERVER_ERROR, "Email sending failed: " + e.getMessage());
    }
  }
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EmailSendResult>>
      sendSimpleEmail(
          ...
          @RequestParam
              ...
              String content,
          @RequestParam(defaultValue = "true") @Parameter(description = "Send email asynchronously")
              boolean async) {
    ...
    try {
      EmailSendResult result;
      if (async) {
        CompletableFuture<EmailSendResult> futureResult =
            emailService.sendSimpleEmailAsync(to, subject, content);
        result = futureResult.get(); // Wait for completion for API response
      } else {
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EmailSendResult>>
      sendTemplateEmail(
          @Valid @RequestBody @Parameter(description = "Template email request")
              TemplateEmailRequest templateRequest) {

    log.info(
        "Sending template email to: {}, template: {}",
  ...
  @Operation(
      ...
      description = "Send email with file attachments using multipart form data")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EmailSendResult>>
      sendEmailWithAttachment(
          ...
          @RequestParam
              ...
              @Parameter(description = "Email content")
              String content,
          @RequestParam(required = false) @Parameter(description = "Email attachments")
    ...
    try {
      ...
      EmailRequest.EmailRequestBuilder requestBuilder =
          EmailRequest.builder().to(List.of(to)).subject(subject).htmlBody(content).async(async);
  ...
  @Operation(
      ...
      description = "Check if email service is running and configured properly")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<Map<String, Object>>>
      healthCheck() {
