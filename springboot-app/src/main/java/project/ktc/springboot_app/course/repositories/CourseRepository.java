...
public interface CourseRepository
  ...
  @Query("SELECT c FROM Course c " + "LEFT JOIN FETCH c.categories cat " + "WHERE c.id = :courseId")
  ...
  @Query(
      "SELECT COUNT(qq) FROM QuizQuestion qq "
          + "JOIN qq.lesson l "
          + "JOIN l.section s "
          + "WHERE s.course.id = :courseId")
  ...
  @Query(
      "SELECT c.id, COUNT(e) FROM Course c "
  ...
  /**
   * Get average rating for instructor's published and approved courses only This ensures
   * consistency with course count query by only including courses that are currently available to
   * students
   */
  @Query(
  ...
  /**
   * Get total published and approved course count by instructor Only counts courses that are
   * currently published, approved and not deleted
   */
  @Query(
  ...
  /**
   * Check if a slug exists (case-insensitive) Used to prevent duplicate slugs with normalization
   */
  @Query(
  ...
  @Query(
      "SELECT COALESCE(SUM(l.content.duration), 0) FROM Lesson l "
          + "JOIN l.section s "
          + "WHERE s.course.id = :courseId AND l.content IS NOT NULL")
  Long getTotalDurationByCourseId(@Param("courseId") String courseId);

  @Query(
      "SELECT MIN(c.price) AS minPrice, MAX(c.price) AS maxPrice FROM Course c "
