...
public interface DiscountUsageRepository extends JpaRepository<DiscountUsage, String> {
  ...
  List<DiscountUsage> findByUsedAtBetween(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /** Calculate total discount amount by discount ID */
  @Query(
      "SELECT COALESCE(SUM(du.discountAmount), 0) FROM DiscountUsage du WHERE du.discount.id = :discountId")
  Double getTotalDiscountAmountByDiscountId(@Param("discountId") String discountId);

  /** Calculate total discount amount by user ID */
  @Query(
      "SELECT COALESCE(SUM(du.discountAmount), 0) FROM DiscountUsage du WHERE du.user.id = :userId")
  ...
  @Query("SELECT du FROM DiscountUsage du WHERE du.user.id = :userId " + "ORDER BY du.usedAt DESC")
