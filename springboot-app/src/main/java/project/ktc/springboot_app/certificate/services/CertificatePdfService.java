...
/**
 * Service for generating PDF certificates using html-to-image API Converts HTML
 * templates to
 * high-quality PDF certificates
 *
 * <p>
 * Features: - Loads HTML template from resources - Replaces placeholders with
 * dynamic data -
...
public class CertificatePdfService {
	...
	public byte[] generateCertificatePdf(CertificateDataDto certificateData) throws IOException {
		log.info(
				"Generating PDF certificate using html-to-image API for user: {} and course: {}",
	...
	/**
	 * Alternative method - delegates to main API method Maintained for backward
	 * compatibility
	 *
	 * @param certificateData
	 *            Certificate information
	 ...
	 *             if PDF generation fails
	 */
	public byte[] generateCertificatePdfDirect(CertificateDataDto certificateData)
			throws IOException {
	...
	private byte[] callHtmlToPdfApi(String htmlContent) throws IOException {
		try {
			log.info(
					"Calling html-to-image API at: {} with HTML content length: {} characters",
					pdfApiUrl,
					htmlContent.length());
			...
			ResponseEntity<byte[]> response = restTemplate.exchange(pdfApiUrl, HttpMethod.POST, requestEntity,
					byte[].class);
	...
	private Map<String, Object> createApiRequestBody(String htmlContent) {
		...
		log.debug(
				"Created API request body with HTML length: {} characters and options: {}",
				htmlContent.length(),
				options);
		return requestBody;
	}
	 ...
	 *
	 * @param certificateData
	 *            Certificate data to populate template
	...
	private String loadAndProcessHtmlTemplate(CertificateDataDto certificateData) throws IOException {
		...
		try {
			...
			try (InputStream inputStream = resource.getInputStream()) {
				...
				String courseLevel = certificateData.getCourseLevel() != null ? certificateData.getCourseLevel()
						: "General";
