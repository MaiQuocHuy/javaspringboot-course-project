...
public interface PermissionService {
  ...
  /**
   * Get all permissions with resource and action details (for admin purposes) This includes both
  ...
  /**
   * Get all permissions assigned to a specific role (for admin purposes) This includes both active
   * and inactive permissions assigned to the role
   *
  ...
  /**
   * Update permissions for a specific role (for admin purposes) This allows enabling/disabling
   * permissions at the role_permission level
   *
   * @param roleId the role ID
   * @param request the permission update request
   * @return list of updated permissions
   */
  List<RolePermission> updatePermissionsForRole(
      String roleId, project.ktc.springboot_app.permission.dto.PermissionUpdateRequest request);

  /**
   * Get hierarchical resource tree with permission assignments for a specific role This builds a
   * tree structure showing which resources the role has permissions for (both directly assigned and
  ...
  /**
   * Get all available permissions with constraint information for a specific role This shows all
   * permissions in the system and indicates which ones are constrained for the given role
   *
   * @param roleId the role ID to check constraints against
   * @return list of available permissions with constraint flags
   */
  List<project.ktc.springboot_app.permission.dto.AvailablePermissionDto> getAllAvailablePermissions(
      String roleId);

  /**
   * Get all permissions for a specific role grouped by resource with assignability information This
   * is used for admin interface to manage permissions for a role
   *
   * @param roleId the role ID
  ...
  /**
   * Get all permissions as DTOs (for admin interface)
   *
   * @return list of all permissions
   */
  List<project.ktc.springboot_app.permission.dto.PermissionDto> getAllPermissions();
}
