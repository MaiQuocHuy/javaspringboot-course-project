...
public interface ResourceRepository extends JpaRepository<Resource, String> {
     ...
     */
    @Query("SELECT r FROM Resource r WHERE r.parentResource IS NULL AND r.isActive = true ORDER BY r.name")
    List<Resource> findAllRootResources();

    /**
     * Find all child resources of a parent resource
     */
    @Query("SELECT r FROM Resource r WHERE r.parentResource = :parent AND r.isActive = true ORDER BY r.name")
     ...
     */
    @Query("SELECT r FROM Resource r WHERE r.parentResource = :parentResource OR r = :parentResource AND r.isActive = true ORDER BY r.name")
    List<Resource> findResourceHierarchy(@Param("parentResource") Resource parentResource);

    /**
     * Check if a resource has any children
     */
    @Query("SELECT COUNT(r) > 0 FROM Resource r WHERE r.parentResource = :parent AND r.isActive = true")
     ...
     */
    @Query("SELECT r FROM Resource r LEFT JOIN FETCH r.parentResource WHERE r.isActive = true ORDER BY r.name")
