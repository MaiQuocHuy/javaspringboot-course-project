...
public class StudentQuizServiceImp implements StudentQuizService {
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<QuizScoreResponseDto>>> getQuizScores(
    ...
    Page<QuizResult> quizResultsPage =
        quizResultRepository.findQuizScoresByUserId(currentUser.getId(), pageable);
  ...
  public ResponseEntity<ApiResponse<StudentQuizStatsDto>> getQuizStats() {
    try {
      ...
      BigDecimal roundedAverage =
          BigDecimal.valueOf(averageScoreValue).setScale(2, RoundingMode.HALF_UP);

      StudentQuizStatsDto stats =
          StudentQuizStatsDto.builder()
