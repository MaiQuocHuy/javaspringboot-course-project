...
public class InstructorDiscountUsageServiceImp implements InstructorDiscountUsageService {
  ...
  @Override
  public ResponseEntity<ApiResponse<PaginatedResponse<InstructorDiscountUsageResponseDto>>>
      getDiscountUsages(Pageable pageable) {

    String instructorId = SecurityUtil.getCurrentUserId();
    log.info(
        "Getting discount usages for instructor: {}, page: {}, size: {}",
        instructorId,
    ...
    try {
      Page<DiscountUsage> discountUsagePage =
          instructorDiscountUsageRepository.findByCourseInstructorId(instructorId, pageable);

      Page<InstructorDiscountUsageResponseDto> responsePage =
          discountUsagePage.map(InstructorDiscountUsageResponseDto::fromEntity);

      PaginatedResponse<InstructorDiscountUsageResponseDto> paginatedResponse =
          PaginatedResponse.<InstructorDiscountUsageResponseDto>builder()
      ...
      log.info(
          "Successfully retrieved {} discount usages for instructor: {}",
          responsePage.getTotalElements(),
          instructorId);

      return ApiResponseUtil.success(
          paginatedResponse, "Instructor discount usages retrieved successfully");
  ...
  public ResponseEntity<
      ...
      getDiscountUsageById(String discountUsageId) {
    ...
    try {
      DiscountUsage discountUsage =
          instructorDiscountUsageRepository.findByIdAndCourseInstructorId(
              discountUsageId, instructorId);

      if (discountUsage == null) {
        log.warn("Discount usage {} not found for instructor: {}", discountUsageId, instructorId);
        return ApiResponseUtil.notFound(
            "Discount usage not found or you don't have permission to view it");
      }

      InstructorDiscountUsageResponseDto responseDto =
          InstructorDiscountUsageResponseDto.fromEntity(discountUsage);

      return ApiResponseUtil.success(responseDto, "Discount usage retrieved successfully");

    } catch (Exception e) {
      log.error(
          "Error retrieving discount usage {} for instructor: {}",
          discountUsageId,
          instructorId,
          e);
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<InstructorDiscountUsageResponseDto>>>
    ...
    String instructorId = SecurityUtil.getCurrentUserId();
    log.info(
        "Getting discount usages for instructor: {} with filters - search: {}, type: {}, fromDate: {}, toDate: {}, page: {}, size: {}",
        instructorId,
        search,
        type,
        fromDate,
    ...
    try {
      ...
      Page<DiscountUsage> discountUsagePage =
          instructorDiscountUsageRepository.findByCourseInstructorIdWithFilter(
              instructorId, search, type, fromLocalDate, toLocalDate, pageable);
      ...
      log.info(
          "Successfully retrieved {} filtered discount usages for instructor: {}",
          responsePage.getTotalElements(),
          instructorId);
