...
/** Service implementation for system log operations Following SOLID principles: SRP, DIP, OCP */
@Service
@Slf4j
...
public class LogServiceImp implements LogService {
  ...
  public ResponseEntity<ApiResponse<LogResponseDto>> createLog(CreateLogDto createLogDto) {
    try {
      log.info(
          "Creating system log entry for user: {}, action: {}, entity: {}",
      ...
      // Create response DTO
      LogResponseDto responseDto = LogResponseDto.builder().id(savedLog.getId()).build();
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to create log entry. Please try again later.");
    }
  }
