...
/**
 * Processor to handle payout email configuration parsing Ensures that comma-separated email strings
 * are properly parsed into lists
 */
@Component
@Slf4j
public class PayoutEmailConfigProcessor {
  ...
  @Value(
      "${app.payout.notification.admin.emails:admin@ktc-learning.com,maiquochuy16122003@gmail.com}")
  ...
  public void processEmailConfiguration() {
    ...
    try {
      ...
      if (emailList.size() > 1
          && payoutProperties.getNotification().getAdmin().getEmails().size() == 1) {
        log.info(
            "🔄 Updating admin emails list from {} to {}",
