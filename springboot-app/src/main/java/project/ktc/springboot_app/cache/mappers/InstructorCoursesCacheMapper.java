...
public class InstructorCoursesCacheMapper {
	...
	/**
	 * Converts Course entity to InstructorCourseBaseCacheDto for Redis storage with
	 * review status
	 * information
	 */
	...
	public static CourseDashboardResponseDto mergeCacheData(
		...
		if (baseInfo.getCategories() != null) {
			categoryInfos = baseInfo.getCategories().stream()
					.map(
							cat -> BaseCourseResponseDto.CategoryInfo.builder()
									.id(cat.getId())
									.name(cat.getName())
									.build())
					.collect(Collectors.toList());
	...
	/**
	 * Determines course status from cache data based on isPublished field This
	 * logic mirrors the
	 * determineStatus method in InstructorCourseServiceImp but includes null checks
	 * since cache DTOs
