...
public class UserServiceImp implements UserService {
  ...
  public ResponseEntity<ApiResponse<project.ktc.springboot_app.user.dto.AdminUserDetailResponseDto>>
      getAdminUserById(String id) {
    try {
      // Get the current authenticated user from security context
      Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
      ...
      project.ktc.springboot_app.user.dto.AdminUserDetailResponseDto adminUserDetailDto =
          project.ktc.springboot_app.user.dto.AdminUserDetailResponseDto.builder().build();
      ...
      List<Object[]> enrolledCoursesData =
          userRepository.findEnrolledCoursesWithPaymentByUserId(id);
      List<project.ktc.springboot_app.user.dto.AdminUserDetailResponseDto.EnrolledCourseDto>
          enrolledCourses =
              enrolledCoursesData.stream()
                  .map(
                      data ->
  ...
  public ResponseEntity<ApiResponse<UserResponseDto>> createUser(CreateUserDto createUserDto) {
    try {
      ...
      UserRoleEnum roleEnum =
          createUserDto.getRole() != null ? createUserDto.getRole() : UserRoleEnum.STUDENT;
  ...
  public ResponseEntity<ApiResponse<AdminUserPageResponseDto>> getUsersWithPagination(
    ...
    try {
      ...
      // Convert to DTOs
      List<UserResponseDto> userResponseDtos =
          usersPage.getContent().stream().map(UserResponseDto::new).collect(Collectors.toList());

      // Create page response
      AdminUserPageResponseDto pageResponse =
          AdminUserPageResponseDto.builder()
