 ...
 *
 * <p>Features: - Profile-based CacheService selection (dev vs prod) - Consistent ObjectMapper
 * configuration for cache serialization - Clean separation of concerns between local and hybrid
 * cache strategies - Reusable and maintainable code structure
 *
 * @author KTC Team
 */
@Slf4j
...
public class CacheServiceConfigUnified {
  ...
  /**
   * Development Profile - Local Redis Cache Service - Uses RedisCacheServiceImp for simple local
   * Redis operations - No fallback mechanisms needed for development
   */
  @Bean("cacheService")
  @Primary
  @Profile("dev")
  ...
  /**
   * Production Profile - Hybrid Cache Service - Uses HybridRedisCacheService with both Redis and
   * Upstash REST fallback - Provides high availability and fault tolerance in production
   */
  // @Bean("cacheService")
  // @Primary
  // @Profile("prod")
  ...
  /**
   * Optimized ObjectMapper for Cache Serialization - Consistent DateTime formatting matching
   * JacksonConfig - Proper timezone handling - Module registration for Java 8 time types
   */
