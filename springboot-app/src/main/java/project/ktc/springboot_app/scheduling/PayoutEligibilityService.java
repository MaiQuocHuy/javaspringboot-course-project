...
/**
 * Service for checking payout eligibility of payments
 * Implements the same business logic as the manual payout API
 */
@Service
...
public class PayoutEligibilityService {
    ...
    public List<Payment> findEligiblePayments() {
        ...
        List<Payment> completedPayments = adminPaymentRepository.findPaymentsByStatusWithRefunds(
            Payment.PaymentStatus.COMPLETED, 
            PageRequest.of(0, payoutProperties.getBatchSize() * 3) // Get more to filter properly
        ).getContent();
        
        // Pre-filter to exclude already paid out payments at query level
        List<Payment> unpaidPayments = completedPayments.stream()
            .filter(payment -> payment.getPaidOutAt() == null)
            .toList();
            
        log.debug("Found {} completed payments, {} not yet paid out", 
                completedPayments.size(), unpaidPayments.size());

        // Filter payments based on eligibility criteria using unpaid payments
        List<Payment> eligiblePayments = unpaidPayments.stream()
            .filter(this::isPaymentEligibleForPayout)
            .limit(payoutProperties.getBatchSize())
            .collect(Collectors.toList());

        log.info("Found {} eligible payments out of {} unpaid payments (from {} total completed)", 
                eligiblePayments.size(), unpaidPayments.size(), completedPayments.size());
    ...
    public boolean isPaymentEligibleForPayout(Payment payment) {
        try {
            ...
            if (payment.getPaidOutAt() != null) {
                log.debug("Payment {} not eligible: already paid out at {}", payment.getId(), payment.getPaidOutAt());
                return false;
            }
            ...
            if (timeSinceCompletion.toDays() < payoutProperties.getWaiting().getPeriod().getDays()) {
                long hoursRemaining = (payoutProperties.getWaiting().getPeriod().getDays() * 24) - timeSinceCompletion.toHours();
                log.debug("Payment {} not eligible: within waiting period. {} hours remaining", 
                         payment.getId(), hoursRemaining);
                return false;
            }
            ...
            try {
                if (payment.getRefunds() != null && !payment.getRefunds().isEmpty()) {
                    boolean hasPendingRefund = payment.getRefunds().stream()
                        .anyMatch(refund -> refund != null && refund.getStatus() == Refund.RefundStatus.PENDING);
                    boolean hasCompletedRefund = payment.getRefunds().stream()
                        .anyMatch(refund -> refund != null && refund.getStatus() == Refund.RefundStatus.COMPLETED);
                ...
                log.warn("Error checking refunds for payment {}: {} - assuming no blocking refunds", 
                        payment.getId(), e.getMessage());
                // Continue processing - if we can't check refunds, don't block the payout
            }

            // 5. Check if instructor earning already exists
            Optional<InstructorEarning> existingEarning = instructorEarningRepository.findByPaymentId(payment.getId());
            ...
            log.error("Error checking payout eligibility for payment {}: {}", payment.getId(), e.getMessage(), e);
            return false;
        }
    }
    ...
    public PayoutEligibilitySummary getEligibilitySummary() {
        ...
        try {
            ...
            List<Payment> completedPayments = adminPaymentRepository.findPaymentsByStatus(
                Payment.PaymentStatus.COMPLETED, pageable).getContent();

            long totalCompleted = completedPayments.size();
            long alreadyPaidOut = completedPayments.stream()
                .filter(p -> p.getPaidOutAt() != null)
                .count();
            
            long withinWaitingPeriod = completedPayments.stream()
                .filter(p -> p.getPaidOutAt() == null)
                .filter(this::isWithinWaitingPeriod)
                .count();
            
            long withRefunds = completedPayments.stream()
                .filter(p -> p.getPaidOutAt() == null)
                .filter(this::hasBlockingRefunds)
                .count();
            
            long eligible = completedPayments.stream()
                .filter(this::isPaymentEligibleForPayout)
                .count();
