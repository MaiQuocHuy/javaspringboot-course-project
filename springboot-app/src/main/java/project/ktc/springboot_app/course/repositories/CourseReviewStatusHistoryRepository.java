...
@Repository
public interface CourseReviewStatusHistoryRepository extends JpaRepository<CourseReviewStatusHistory, String> {

    /**
     * Find the latest course review status history record for a specific course
     * by ordering by createdAt DESC and limiting to 1 result.
     * 
     * @param courseId the course ID to search for
     * @return Optional containing the latest status history record, or empty if
     *         none found
     */
    @Query("SELECT h FROM CourseReviewStatusHistory h " +
            "JOIN h.courseReview cr " +
            "WHERE cr.course.id = :courseId " +
            "ORDER BY h.createdAt DESC")
    List<CourseReviewStatusHistory> findByCourseIdOrderByCreatedAtDesc(@Param("courseId") String courseId);
