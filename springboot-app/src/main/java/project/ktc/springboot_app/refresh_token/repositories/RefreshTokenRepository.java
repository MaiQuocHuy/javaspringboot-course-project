...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, String> {
  ...
  @Query(
      "SELECT rt FROM RefreshToken rt WHERE rt.user.id = :userId AND rt.isRevoked = false AND rt.expiresAt > :currentTime")
  List<RefreshToken> findActiveTokensByUserId(
      @Param("userId") String userId, @Param("currentTime") LocalDateTime currentTime);

  /** Count active tokens for a user */
  @Query(
      "SELECT COUNT(rt) FROM RefreshToken rt WHERE rt.user.id = :userId AND rt.isRevoked = false AND rt.expiresAt > :currentTime")
