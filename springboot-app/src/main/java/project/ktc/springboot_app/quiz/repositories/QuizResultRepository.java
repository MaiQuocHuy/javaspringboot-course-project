...
public interface QuizResultRepository extends JpaRepository<QuizResult, String> {
  ...
  Optional<QuizResult> findByUserIdAndLessonId(
      @Param("userId") String userId, @Param("lessonId") String lessonId);

  /** Check if user has submitted quiz for a specific lesson */
  @Query(
      "SELECT COUNT(qr) > 0 FROM QuizResult qr WHERE qr.user.id = :userId AND qr.lesson.id = :lessonId")
  ...
  /**
   * Find all quiz results for a specific user in courses they are enrolled in with lesson, section,
  ...
  /**
   * Check if user can review a specific quiz result For now, all completed quizzes can be reviewed
   */
  @Query(
      "SELECT CASE WHEN COUNT(qr) > 0 THEN true ELSE false END "
          + "FROM QuizResult qr "
          + "WHERE qr.id = :quizResultId AND qr.user.id = :userId")
  Boolean canUserReviewQuiz(
      @Param("quizResultId") String quizResultId, @Param("userId") String userId);

  /** Find a specific quiz result for a user with lesson, section, and course information */
  @Query(
      "SELECT qr FROM QuizResult qr "
          + "JOIN FETCH qr.lesson l "
          + "JOIN FETCH l.section s "
          ...
          + "AND EXISTS (SELECT 1 FROM Enrollment e WHERE e.user.id = :userId AND e.course.id = c.id)")
  Optional<QuizResult> findQuizResultByIdAndUserId(
      @Param("quizResultId") String quizResultId, @Param("userId") String userId);

  /**
   * Find all enrolled students with their submission status for a lesson Returns both students who
   * have submitted and those who haven't
   */
  @Query(
      "SELECT new project.ktc.springboot_app.lesson.dto.StudentSubmissionDto("
  ...
  @Query(
      "SELECT new project.ktc.springboot_app.lesson.dto.SubmissionSummaryDto("
  ...
  /**
   * Find submission details by submission ID, lesson ID, and instructor ownership Validates that
   * the instructor owns the course containing the lesson
   */
  @Query(
      "SELECT qr FROM QuizResult qr "
          + "JOIN FETCH qr.user u "
  ...
  java.util.List<QuizResult> findRecentSubmissionsByUserId(
