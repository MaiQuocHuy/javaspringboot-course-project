...
public class SpringbootAppApplication {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "SPRING_PROFILES_ACTIVE from env: " + System.getenv("SPRING_PROFILES_ACTIVE"));
    System.out.println(
        "SPRING_PROFILES_ACTIVE from property: " + System.getProperty("SPRING_PROFILES_ACTIVE"));
    System.out.println("Attempting to load .env." + profile + " file...");

    Dotenv tempDotenv = null;
    try {
      tempDotenv = Dotenv.configure().filename(".env." + profile).ignoreIfMissing().load();
      System.out.println("✅ Loaded .env." + profile + " file successfully");
    } catch (Exception e) {
      System.out.println(
          "❌ No .env."
              + profile
              + " file found, using system environment variables. Error: "
              + e.getMessage());
    }
    final Dotenv dotenv = tempDotenv;

    // Helper method to get environment variable with fallback
    java.util.function.Function<String, String> getEnv =
        (key) -> {
          if (dotenv != null) {
    ...
    java.util.function.BiFunction<String, String, String> getEnvWithDefault =
        (key, defaultValue) -> {
          if (dotenv != null) {
    ...
    setSystemPropertyIfNotNull(
        "STRIPE_WEBHOOK_SECRET", getEnvWithDefault.apply("STRIPE_WEBHOOK_SECRET", ""));
    ...
    setSystemPropertyIfNotNull(
        "REDIS_SSL_ENABLED", getEnvWithDefault.apply("REDIS_SSL_ENABLED", "true"));
    setSystemPropertyIfNotNull(
        "UPSTASH_REDIS_REST_URL", getEnvWithDefault.apply("UPSTASH_REDIS_REST_URL", "localhost"));
    setSystemPropertyIfNotNull(
        "UPSTASH_REDIS_REST_TOKEN",
        getEnvWithDefault.apply(
            "UPSTASH_REDIS_REST_TOKEN",
    ...
    System.out.println(
        "REDIS_HOST: " + (System.getProperty("REDIS_HOST") != null ? "✅ Set" : "❌ Not set"));
    System.out.println(
        "REDIS_PORT: " + (System.getProperty("REDIS_PORT") != null ? "✅ Set" : "❌ Not set"));
    System.out.println("==========================================");
