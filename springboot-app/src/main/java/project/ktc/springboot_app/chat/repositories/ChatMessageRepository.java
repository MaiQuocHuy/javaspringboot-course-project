...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, String> {
  ...
  @Query(
      "SELECT m FROM ChatMessage m JOIN FETCH m.messageType mt WHERE m.course.id = :courseId ORDER BY m.createdAt DESC")
  Page<ChatMessage> findByCourseId(@Param("courseId") String courseId, Pageable pageable);

  @Query(
      "SELECT m FROM ChatMessage m JOIN FETCH m.messageType mt WHERE m.course.id = :courseId AND mt.name = :type ORDER BY m.createdAt DESC")
  Page<ChatMessage> findByCourseIdAndType(
      @Param("courseId") String courseId, @Param("type") String type, Pageable pageable);
  ...
  List<ChatMessage> findByCourseIdOrderByCreatedAtAsc(
  ...
  @Query(
      "SELECT m FROM ChatMessage m "
          ...
          + "ORDER BY m.createdAt DESC")
  List<ChatMessage> findByCourseIdBeforeMessageId(
  ...
  @Query(
      "SELECT m FROM ChatMessage m "
          ...
          + "ORDER BY m.createdAt DESC")
  List<ChatMessage> findByCourseIdAfterMessageId(
      @Param("courseId") String courseId,
