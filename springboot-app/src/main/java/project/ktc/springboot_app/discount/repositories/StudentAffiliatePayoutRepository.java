...
/**
 * Repository for student affiliate payout operations Handles queries for affiliate payouts from
 * student perspective
 */
@Repository
public interface StudentAffiliatePayoutRepository extends JpaRepository<AffiliatePayout, String> {
  ...
  @Query(
      "SELECT COUNT(ap) FROM AffiliatePayout ap WHERE ap.referredByUser.id = :studentId AND ap.payoutStatus = :status")
  ...
  @Query(
      "SELECT COALESCE(SUM(ap.commissionAmount), 0) FROM AffiliatePayout ap WHERE ap.referredByUser.id = :studentId")
  ...
  @Query(
      "SELECT COALESCE(SUM(ap.commissionAmount), 0) FROM AffiliatePayout ap WHERE ap.referredByUser.id = :studentId AND ap.payoutStatus = :status")
