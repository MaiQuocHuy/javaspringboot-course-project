...
/**
 * Admin Role Controller for role management operations Provides API endpoints
 * for CRUD operations
 * on roles and role-permission assignments
 */
@RestController
...
public class AdminRoleController {
	...
	public ResponseEntity<ApiResponse<List<RoleResponseDto>>> getAllRolesList() {
		...
		List<RoleResponseDto> roleResponses = roles.stream()
				.map(role -> RoleResponseDto.builder().id(role.getId()).name(role.getRole()).build())
				.toList();

		return ResponseEntity.ok(
				ApiResponse.success(roleResponses, "Roles list retrieved successfully"));
	}

	/** Get role by ID with its permissions */
	@GetMapping("/{roleId}")
	// @PreAuthorize("hasPermission('Role', 'role:READ')")
	public ResponseEntity<ApiResponse<RoleWithPermissionsDto>> getRoleById(
	...
	public ResponseEntity<ApiResponse<RoleResponseDto>> createRole(
		...
		RoleResponseDto response = RoleResponseDto.builder().id(createdRole.getId()).name(createdRole.getRole())
				.build();
	...
	public ResponseEntity<ApiResponse<RoleResponseDto>> updateRole(
		...
		RoleResponseDto response = RoleResponseDto.builder().id(updatedRole.getId()).name(updatedRole.getRole())
				.build();
	...
	public ResponseEntity<ApiResponse<Void>> assignPermissions(
			@PathVariable String roleId, @Valid @RequestBody AssignPermissionsRequest request) {
	...
	public ResponseEntity<ApiResponse<Void>> removePermissions(
			@PathVariable String roleId, @Valid @RequestBody RemovePermissionsRequest request) {
	...
	public ResponseEntity<ApiResponse<List<PermissionDto>>> getAvailablePermissions() {
		...
		return ResponseEntity.ok(
				ApiResponse.success(permissions, "Available permissions retrieved successfully"));
	}

	/** Get permissions assigned to a specific role */
	@GetMapping("/{roleId}/permissions")
	// @PreAuthorize("hasPermission('Role', 'role:READ')")
	public ResponseEntity<ApiResponse<List<RolePermissionDetailDto>>> getRolePermissions(
			@PathVariable String roleId) {
