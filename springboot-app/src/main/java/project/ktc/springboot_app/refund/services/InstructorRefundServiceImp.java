...
public class InstructorRefundServiceImp implements InstructorRefundService {
  ...
  @Override
  public ResponseEntity<ApiResponse<PaginatedResponse<InstructorRefundResponseDto>>>
      getAllRefundsByInstructorId(Pageable pageable) {
    try {
      String userId = SecurityUtil.getCurrentUserId();
      log.info("Fetching paginated refunds for instructor: {} by user: {}", userId);
      Page<Refund> refunds =
          instructorRefundRepository.findAllRefundsByInstructorId(userId, pageable);
      List<InstructorRefundResponseDto> refundDtos =
          refunds.getContent().stream()
              .map(InstructorRefundResponseDto::fromEntity)
              .collect(Collectors.toList());
      PaginatedResponse<InstructorRefundResponseDto> paginatedResponse =
          PaginatedResponse.<InstructorRefundResponseDto>builder()
              .content(refundDtos)
              .page(
  ...
  @Override
  public ResponseEntity<ApiResponse<List<InstructorRefundResponseDto>>>
  ...
  @Override
  public ResponseEntity<ApiResponse<InstructorRefundDetailsResponseDto>>
      getRefundByIdAndInstructorIdWithDetails(String refundId) {
    try {
      String userId = SecurityUtil.getCurrentUserId();
      log.info(
          "Fetching refund details for refund ID: {} and instructor ID: {} by user: {}",
          refundId,
          userId);
      Optional<Refund> refundOpt =
          instructorRefundRepository.findRefundByIdAndInstructorIdWithDetails(refundId, userId);
        ...
        try {
          ...
          if (stripeData != null) {
            instructorStripeData =
                InstructorRefundDetailsResponseDto.StripePaymentData.builder()
                    .transactionId(stripeData.getTransactionId())
                    .receiptUrl(stripeData.getReceiptUrl())
                    .build();
          }
          refundDetail =
              InstructorRefundDetailsResponseDto.fromEntityWithStripeData(
                  refund, instructorStripeData);
        } catch (Exception e) {
          log.error(
              "Error fetching Stripe payment details for session ID: {}",
              refund.getPayment().getSessionId(),
              e);
  ...
  public ResponseEntity<ApiResponse<RefundStatusUpdateResponseDto>> updateRefundStatus(
    ...
    log.info(
        "Processing refund status update for refund ID: {} to status: {}",
        refundId,
        updateDto.getStatus());
    ...
    try {
      ...
      if (!Refund.RefundStatus.PENDING.equals(refund.getStatus())) {
        log.warn(
            "Refund {} is not in PENDING state. Current status: {}", refundId, refund.getStatus());
      ...
      Optional<InstructorEarning> earningOpt =
      ...
      if (earningOpt.isPresent()) {
        ...
        if (InstructorEarning.EarningStatus.PAID.equals(earning.getStatus())) {
          ...
          return ApiResponseUtil.badRequest(
              "Cannot process refund - payment has already been paid out to instructor");
        }
      }
      ...
      if (Refund.RefundStatus.COMPLETED.equals(newStatus)) {
        ...
        try {
          ...
          if (enrollmentOpt.isPresent()) {
            ...
            log.info(
                "Removed enrollment for user {} from course {} due to refund completion",
                userId,
                courseId);
          } else {
            log.warn("No enrollment found for user {} in course {} to remove", userId, courseId);
          }
        } catch (Exception e) {
        ...
        try {
          ...
          log.info(
              "Updated payment {} status to REFUNDED due to refund completion", payment.getId());
        } catch (Exception e) {
          log.error(
              "Error updating payment status to REFUNDED for payment {}: {}",
              payment.getId(),
              e.getMessage(),
              e);
      ...
      try {
        ...
        String refundUrl =
            "/dashboard/payments"; // As requested, set to null since URL is not available yet
        String status =
            newStatus.name().toLowerCase(); // Convert to lowercase for the notification message

        notificationHelper
            .createRefundNotification(studentId, savedRefund.getId(), courseName, refundUrl, status)
            .thenAccept(
                notification ->
                    log.info(
            ...
            .exceptionally(
                ex -> {
                  log.error(
                      "❌ Failed to create refund notification for student {} (refund: {}): {}",
                      studentId,
                      refundId,
                      ex.getMessage(),
                      ex);
        ...
        log.info(
            "💰 Refund {} status updated to {} for student {} - notification created",
            refundId,
            newStatus,
            studentId);
      ...
      // 8. Build response
      RefundStatusUpdateResponseDto response =
          RefundStatusUpdateResponseDto.builder()
      ...
      log.info(
          "Refund status updated successfully for refund ID: {} to status: {}",
          refundId,
          newStatus);
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to update refund status. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<InstructorRefundResponseDto>>>
    ...
    try {
      ...
      Page<Refund> refunds =
          instructorRefundRepository.findAllRefundsByInstructorIdWithFilter(
      ...
      log.info(
          "Retrieved {} filtered refunds for instructor {} (page {} of {})",
          refundDtos.size(),
          userId,
          refunds.getNumber(),
      ...
      return ApiResponseUtil.internalServerError(
          "Error retrieving filtered refunds for instructor");
    }
  }
