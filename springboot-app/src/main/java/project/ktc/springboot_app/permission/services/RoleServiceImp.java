...
public class RoleServiceImp implements RoleService {
  ...
  public UserRole createRole(CreateRoleRequest request) {
    ...
    try {
      ...
      UserRole newRole = UserRole.builder().role(roleType).build();
      ...
      log.error(
          "Invalid role name provided: {}. Valid roles are: STUDENT, INSTRUCTOR, ADMIN", roleName);
  ...
  public Page<RoleWithPermissionsDto> getAllRolesWithPermissions(int page, int size) {
    ...
    try {
      ...
      List<RoleWithPermissionsDto> roleWithPermissionsDtos =
  ...
  @Override
  public List<project.ktc.springboot_app.permission.dto.RolePermissionDetailDto> getRolePermissions(
      String roleId) {
    ...
    return rolePermissions.stream()
        .map(
            rp ->
                project.ktc.springboot_app.permission.dto.RolePermissionDetailDto.builder()
  ...
  private RoleWithPermissionsDto convertToRoleWithPermissionsDtoInternal(UserRole role) {
    ...
    try {
      ...
      List<RolePermission> rolePermissions =
          rolePermissionRepository.findActiveByRoleId(role.getId());
      log.debug("Found {} active permissions for role {}", rolePermissions.size(), role.getId());

      // Convert permissions to summary DTOs
      List<RoleWithPermissionsDto.PermissionSummaryDto> permissionSummaries =
          rolePermissions.stream()
              .map(
                  rp ->
                      RoleWithPermissionsDto.PermissionSummaryDto.builder()
