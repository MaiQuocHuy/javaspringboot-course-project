...
public class StripePaymentDetailsService {
    ...
    try {
      ...
      try {
        ...
        if (e.getMessage() != null && e.getMessage().contains("No such checkout.session")) {
          log.warn(
              "Stripe session no longer exists for ID: {}. This is normal for expired test sessions.",
              sessionId);
          return null;
        } else {
      ...
      StripePaymentData.StripePaymentDataBuilder builder =
          StripePaymentData.builder().transactionId(paymentIntent.getId());
      ...
      log.error(
          "Error fetching Stripe payment details for session {}: {}", sessionId, e.getMessage(), e);
      return null;
    } catch (Exception e) {
      log.error(
          "Unexpected error fetching Stripe payment details for session {}: {}",
          sessionId,
          e.getMessage(),
          e);
      return null;
  ...
  public boolean isStripePayment(String paymentMethod) {
    return paymentMethod != null
        && (paymentMethod.equalsIgnoreCase("stripe") || paymentMethod.equalsIgnoreCase("STRIPE"));
  }
}
