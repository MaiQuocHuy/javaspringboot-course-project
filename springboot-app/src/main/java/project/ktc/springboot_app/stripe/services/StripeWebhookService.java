...
public class StripeWebhookService {
  ...
  /** Handles successful checkout sessions This is the main event for course purchases */
  private void handleCheckoutSessionCompleted(Event event) {
    ...
    try {
      if (event.getDataObjectDeserializer().getObject().isPresent()) {
        ...
        if (eventObject instanceof Session) {
          ...
          log.warn(
              "Event data object is not a Session instance. Got: {}",
    ...
    log.info(
        "📧 Customer email: {}, 💰 Amount total: {} cents",
    ...
    try {
      ...
      String courseId =
          session.getMetadata() != null ? session.getMetadata().get("courseId") : null;
      String userId = session.getMetadata() != null ? session.getMetadata().get("userId") : null;
      String discountCode =
          session.getMetadata() != null ? session.getMetadata().get("discountCode") : null;
      String discountAmountStr =
          session.getMetadata() != null ? session.getMetadata().get("discountAmount") : null;

      log.info(
          "📊 Session metadata - CourseId: {}, UserId: {}, DiscountCode: {}",
          courseId,
          userId,
          discountCode);
      ...
      if (courseId == null || userId == null) {
        log.error(
            "❌ Missing required metadata in session. CourseId: {}, UserId: {}", courseId, userId);
        log.error(
      ...
      log.info(
          "💵 Converting Stripe amount: {} cents = ${}",
          session.getAmountTotal(),
          stripeAmountInDollars);

      var paymentOpt =
          paymentService.findPaymentBySessionIdAndVerifyAmount(
      ...
      log.info(
          "✅ Payment amount verified successfully. Database: ${}, Stripe: ${}",
          payment.getAmount(),
          stripeAmountInDollars);
      ...
      if (discountCode != null && !discountCode.trim().isEmpty() && discountAmountStr != null) {
        try {
          ...
          discountPriceService.recordDiscountUsage(
              discountCode, userId, courseId, discountAmount, payment.getId());
          log.info(
              "✅ Discount usage recorded for code: {} with amount: {}",
              discountCode,
              discountAmount);
          ...
          log.error(
              "❌ Failed to record discount usage for code: {} - {}",
              discountCode,
      ...
      long duration = endTime - startTime;
      log.error(
          "❌ Error processing checkout session completion after {}ms: {}",
          duration,
          e.getMessage(),
          e);
              ...
              .mapToInt(
                  lesson ->
                      lesson.getContent().getDuration() != null
                          ? lesson.getContent().getDuration() / 60
                          : 0)
              .sum();
  ...
  private void handleCheckoutSessionExpired(Event event) {
    ...
    try {
      ...
      if (session == null) {
        log.error(
            "❌ Could not retrieve session from checkout.session.expired event after all attempts");
        return;
      }
  ...
  /** Helper method to extract session ID from event data when deserialization fails */
    ...
    try {
      ...
      if (eventJson.has("data") && eventJson.getAsJsonObject("data").has("object")) {
        ...
        if (dataObject.has("id")) {
          ...
          log.warn(
              "No 'id' field found in event data object. Available keys: {}", dataObject.keySet());
        }
      } else {
  ...
  public Session getSessionById(String sessionId) {
    try {
      ...
      if (session != null) {
        ...
        log.debug(
            "📋 Session details - Status: {}, Payment Status: {}, Amount: {} cents",
            session.getStatus(),
  ...
  private void createAffiliatePayoutAsync(
      String discountCode, String userId, String courseId, BigDecimal finalPrice) {
    ...
    try {
      ...
      DiscountUsage relevantUsage =
          discountUsages.stream()
      ...
      if (relevantUsage.getDiscount().getType() == DiscountType.REFERRAL) {
        ...
        affiliatePayoutService
            .createPayoutAsync(relevantUsage, finalPrice, relevantUsage.getDiscount().getId())
            .thenAccept(
                payout -> {
                  log.info(
                      "✅ Affiliate payout created successfully: {} for amount: ${}",
                      payout.getId(),
