 ...
 *
 * <p>This configuration provides: - Redis connection factory with Lettuce client - Custom
 * RedisTemplate with JSON serialization - Cache manager with different TTL for different cache
 * names - Connection pooling configuration
 *
 * @author KTC Team
 */
@Slf4j
...
public class RedisConfig {
  ...
  /**
   * Local Redis Connection Factory for development profile - Uses local Redis server without SSL -
  ...
  public RedisConnectionFactory localRedisConnectionFactory() {
    log.info(
        "Configuring LOCAL Redis connection to {}:{} with database {} (Profile: dev)",
        redisHost,
        redisPort,
        database);
  ...
  public RedisConnectionFactory remoteRedisConnectionFactory() {
    ...
    clientBuilder.commandTimeout(Duration.ofSeconds(5)).shutdownTimeout(Duration.ofMillis(200));
    ...
    LettuceConnectionFactory factory =
        new LettuceConnectionFactory(redisConfig, clientBuilder.build());
    factory.setValidateConnection(true);
  ...
  /**
   * Remote Redis Connection Factory for production profile - Uses remote Redis server with SSL
   * support - Optimized for production with longer timeouts and SSL
   */
  // @Bean("redisConnectionFactory")
  // @Profile("prod")
  // public RedisConnectionFactory remoteRedisConnectionFactory() {
  ...
  /** Create ObjectMapper with consistent LocalDateTime configuration matching JacksonConfig */
