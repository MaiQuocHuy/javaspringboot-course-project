...
/**
 * Controller for managing and monitoring automatic payout scheduling
 * Admin-only endpoints for controlling the payout system
 */
@RestController
...
public class PayoutSchedulingController {
    ...
    private final project.ktc.springboot_app.earning.repositories.InstructorEarningRepository instructorEarningRepository;

    /**
     * Manually trigger automatic payout processing
     * Useful for testing or emergency processing
     */
    @PostMapping("/trigger")
    ...
    public ResponseEntity<ApiResponse<String>> triggerPayoutProcessing() {
        ...
        try {
            ...
            return ApiResponseUtil.success("Automatic payout processing completed successfully",
                    "Payout processing triggered manually");
        } catch (Exception e) {
            log.error("Error during manual payout processing trigger: {}", e.getMessage(), e);
            return ApiResponseUtil.internalServerError("Failed to trigger payout processing: " + e.getMessage());
        }
    }

    /**
     * Get current payout eligibility status
     * Shows how many payments are eligible, waiting, etc.
     */
    @GetMapping("/status")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<PayoutEligibilityService.PayoutEligibilitySummary>> getPayoutStatus() {
        ...
        try {
            ...
            return ApiResponseUtil.internalServerError("Failed to retrieve payout status: " + e.getMessage());
        }
    }

    /**
     * Get scheduling configuration and health status
     * Shows current configuration and system health
     */
    @GetMapping("/config")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getSchedulingConfig() {
        ...
        try {
            ...
            config.put("instructorEarningPercentage", payoutProperties.getInstructor().getEarning().getPercentage());
            ...
            return ApiResponseUtil.success(config, "Payout scheduling configuration retrieved successfully");
        } catch (Exception e) {
            log.error("Error retrieving payout configuration: {}", e.getMessage(), e);
            return ApiResponseUtil.internalServerError("Failed to retrieve payout configuration: " + e.getMessage());
        }
    }

    /**
     * Generate daily summary manually
     * Useful for getting current statistics
     */
    ...
    public ResponseEntity<ApiResponse<String>> generateDailySummary() {
        ...
        try {
            ...
            return ApiResponseUtil.success("Daily summary generated successfully",
            ...
            return ApiResponseUtil.internalServerError("Failed to generate daily summary: " + e.getMessage());
        }
    }

    /**
     * Health check endpoint for the scheduling system
     * Returns the health status of the automatic payout system
     */
    @GetMapping("/health")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getSystemHealth() {
        ...
        try {
            ...
            return ApiResponseUtil.internalServerError("Failed to check system health: " + e.getMessage());
        }
    }

    /**
     * Send test email notification to verify email configuration
     * Useful for testing email notification system
     */
    ...
    public ResponseEntity<ApiResponse<String>> sendTestEmail() {
        ...
        try {
            ...
            return ApiResponseUtil.success("Test email notification sent successfully",
    ...
    /**
     * Get email configuration status and details
     * Shows current email notification settings
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getEmailConfig() {
        ...
        try {
            ...
            emailConfig.put("debugInfo", Map.of(
                ...
                "individualEmails", adminEmails.stream().map(email -> 
                    Map.of("email", email, "length", email.length())
                ).toList()
            ));
            ...
            return ApiResponseUtil.internalServerError("Failed to retrieve email configuration: " + e.getMessage());
        }
    }

    /**
     * Force refresh email configuration
     * Useful for reloading email settings without restarting the application
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> refreshEmailConfig() {
        ...
        try {
            ...
            return ApiResponseUtil.internalServerError("Failed to refresh email configuration: " + e.getMessage());
        }
    }

    /**
     * Debug endpoint to check payment states and eligibility
     * Useful for troubleshooting duplicate processing issues
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> debugPayments() {
        ...
        try {
            ...
            PayoutEligibilityService.PayoutEligibilitySummary summary = payoutEligibilityService.getEligibilitySummary();
            ...
            List<Map<String, Object>> paymentDetails = eligiblePayments.stream()
                ...
                .map(payment -> {
                    ...
                    detail.put("courseId", payment.getCourse() != null ? payment.getCourse().getId() : null);
                    detail.put("instructorId", payment.getCourse() != null && payment.getCourse().getInstructor() != null 
                        ? payment.getCourse().getInstructor().getId() : null);
                    
                    // Check if instructor earning already exists
                    var existingEarning = instructorEarningRepository.findByPaymentId(payment.getId());
            ...
            return ApiResponseUtil.internalServerError("Failed to retrieve debug information: " + e.getMessage());
        }
    }
}