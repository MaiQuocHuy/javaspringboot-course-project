...
/**
 * Implementation of CertificateAsyncService for handling asynchronous certificate processing This
 * service is dedicated to background tasks like PDF generation, cloud upload, and email
 * notifications
 */
@Slf4j
@Service
...
public class CertificateAsyncServiceImp implements CertificateAsyncService {
  ...
  /**
   * Process certificate PDF generation, cloud upload, and email notification asynchronously This
   * method runs in a separate thread and has its own transaction context Uses pessimistic locking
  ...
  public void processCertificateAsync(String certificateId) {
    ...
    try {
      ...
      if (certificate.getUser() == null || certificate.getCourse() == null) {
        log.error(
            "Certificate {} has missing relationships during async processing", certificateId);
        return;
      }
      ...
      log.info(
          "Async processing completed successfully for certificate: {}",
          certificate.getCertificateCode());

    } catch (Exception e) {
      log.error(
          "Failed to process certificate asynchronously for ID {}: {}",
          certificateId,
          e.getMessage(),
          e);
    ...
    try {
      ...
      CertificateDataDto certificateData =
          CertificateDataDto.builder()
              ...
              .courseLevel(
                  certificate.getCourse().getLevel() != null
                      ? certificate.getCourse().getLevel().toString()
                      : "General")
              .build();
      ...
      // Generate filename
      String filename =
          certificateImageService.generateImageFilename(
              certificate.getUser().getId(),
              certificate.getCourse().getId(),
              certificate.getCertificateCode());

      // Upload to Cloudinary
      ImageUploadResponseDto uploadResponse =
          cloudinaryService.uploadCertificateImage(imageData, filename);
      log.info("Url Image: {}", uploadResponse);

      // Update certificate with file URL atomically to prevent conflicts
      int updatedRows =
          certificateRepository.updateFileUrl(certificate.getId(), uploadResponse.getUrl());
      if (updatedRows == 0) {
        log.warn(
            "Failed to update certificate file URL - certificate may have been modified concurrently: {}",
            certificate.getCertificateCode());
      } else {
      ...
      log.info(
          "PDF generated and uploaded successfully for certificate: {}. URL: {}",
          certificate.getCertificateCode(),
      ...
      log.error(
          "Failed to generate/upload PDF for certificate {}: {}",
          certificate.getCertificateCode(),
          e.getMessage(),
          e);
    }
  ...
  private void sendCertificateNotificationEmail(Certificate certificate, String certificateUrl) {
    try {
      ...
      emailService
          .sendCertificateNotificationEmailAsync(
          ...
          .thenAccept(
              result -> {
                if (result.isSuccess()) {
                  log.info(
                      "Certificate notification email sent successfully for: {}",
                      certificate.getCertificateCode());
                } else {
                  log.error(
                      "Failed to send certificate notification email for {}: {}",
                      certificate.getCertificateCode(),
          ...
          .exceptionally(
              throwable -> {
                log.error(
                    "Exception sending certificate notification email for {}: {}",
                    certificate.getCertificateCode(),
      ...
      log.error(
          "Error sending certificate notification email for {}: {}",
          certificate.getCertificateCode(),
          e.getMessage(),
          e);
    }
  ...
  /**
   * Fetch the latest certificate with retry mechanism and pessimistic locking This prevents stale
   * data conflicts during async processing
   */
  private Certificate fetchLatestCertificateWithRetry(String certificateId) {
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        log.debug(
            "Attempting to fetch certificate with lock, attempt {} of {}", attempt, MAX_RETRIES);

        Optional<Certificate> certificateOpt =
            certificateRepository.findByIdWithLock(certificateId);

        if (certificateOpt.isPresent()) {
          Certificate certificate = certificateOpt.get();
          log.info(
              "Certificate fetched successfully with lock: {}", certificate.getCertificateCode());
          return certificate;
        } else {
        ...
        log.warn(
            "Failed to fetch certificate on attempt {} of {}: {}",
            attempt,
            MAX_RETRIES,
            e.getMessage());
