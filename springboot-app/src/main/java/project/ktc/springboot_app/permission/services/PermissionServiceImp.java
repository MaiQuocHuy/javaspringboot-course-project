...
public class PermissionServiceImp implements PermissionService {
  ...
  private final project.ktc.springboot_app.permission.repositories.FilterTypeRepository
  ...
  public PermissionServiceImp(
      ...
      project.ktc.springboot_app.permission.repositories.FilterTypeRepository
          filterTypeRepository) {
  ...
  public Set<String> loadUserPermissions(User user) {
    ...
    Set<String> permissions = rolePermissionRepository.findPermissionKeysByRoleType(userRole);
  ...
  public boolean hasPermission(User user, String permissionKey) {
    ...
    logger.debug(
        "Permission check - User: {}, Permission: {}, Result: {}",
        user.getEmail(),
        permissionKey,
        hasPermission);
  ...
  public boolean hasRolePermission(String roleType, String permissionKey) {
    ...
    logger.debug(
        "Role permission check - Role: {}, Permission: {}, Result: {}",
        roleType,
        permissionKey,
        hasPermission);
  ...
  @Override
  @Transactional
  public List<RolePermission> updatePermissionsForRole(
      String roleId, PermissionUpdateRequest request) {
    logger.debug(
        "[updatePermissionsForRole] Incoming {} permission items for role {}",
    ...
    Map<String, RolePermission> currentMap =
    ...
    for (PermissionUpdateRequest.PermissionUpdateItemDto item : request.getPermissions()) {
      ...
      Permission permission =
          permissionRepository
              ...
              .orElseThrow(
                  () -> new RuntimeException("Permission not found with key: " + permissionKey));
      ...
      if (filterTypeId == null || filterTypeId.trim().isEmpty()) {
        filterType =
            filterTypeRepository
                .getAllFilterType()
                .orElseThrow(
                    () -> new RuntimeException("FilterType not found with ID: filter-type-001"));
      } else {
        filterType =
            filterTypeRepository
                .findById(filterTypeId)
                .orElseThrow(
                    () -> new RuntimeException("FilterType not found with ID: " + filterTypeId));
      }
      ...
      if (rolePermission != null) {
        ...
        logger.debug(
            "Updated existing role-permission '{}' (active=true, filterType={})",
            permissionKey,
            filterType.getName());
      } else {
        ...
        rolePermissionRepository.save(rolePermission);
        logger.debug(
            "Created new role-permission '{}' (filterType={})",
            permissionKey,
            filterType.getName());
      }
    }
    ...
    logger.debug(
        "Permission update completed for role {}. Activated/updated: {}, Deactivated: {}",
        roleId,
        requestedKeys.size(),
        deactivatedCount);
  ...
  public List<ResourceDto> getResourceTreeForRole(String roleId) {
    ...
    List<RolePermission> rolePermissions = rolePermissionRepository.findAllByRole(role);
    Set<String> assignedResourceNames =
        rolePermissions.stream()
            .filter(rp -> rp.isActive() && rp.getPermission().isActive())
            .map(rp -> rp.getPermission().getResource().getName())
            .collect(Collectors.toSet());
    logger.debug(
        "Found {} directly assigned resource permissions for role", assignedResourceNames.size());
    ...
    List<ResourceDto> resourceTree =
        rootResources.stream()
            .map(
                resource ->
                    buildResourceDto(
                        resource, parentChildMap, assignedResourceNames, new HashSet<>()))
            .collect(Collectors.toList());

    logger.debug(
        "Built resource tree with {} root resources for role ID: {}", resourceTree.size(), roleId);
    return resourceTree;
  }
  ...
  private ResourceDto buildResourceDto(
      Resource resource,
      Map<String, List<Resource>> parentChildMap,
      Set<String> assignedResourceNames,
      Set<String> inheritedResourceNames) {
    ...
    List<ResourceDto> children =
        parentChildMap.getOrDefault(resource.getId(), List.of()).stream()
            .map(
                child ->
                    buildResourceDto(
                        child, parentChildMap, assignedResourceNames, childInheritedNames))
            .collect(Collectors.toList());
    ...
    if (resource.getParentResource() != null) {
      parent =
          ResourceDto.ParentResourceDto.builder()
  ...
  public List<AvailablePermissionDto> getAllAvailablePermissions(String roleId) {
    ...
    List<AvailablePermissionDto> availablePermissions =
        allPermissions.stream()
            .map(
                permission -> {
                  // Check if this permission has any assignment rules (restricted)
                  boolean isRestricted =
                      permissionRoleAssignRuleRepository.hasAssignmentRules(permission.getId());

                  // Get allowed roles for this permission
                  List<String> allowedRoles =
                      permissionRoleAssignRuleRepository.findAllowedRoleNamesByPermissionId(
                          permission.getId());
                  ...
                  if (!isRestricted) {
                    ...
                    canAssignToRole =
                        allowedRoles.contains(userRoleRepository.findById(roleId).get().getRole());
                  }
  ...
  @Override
  public project.ktc.springboot_app.permission.dto.RolePermissionGroupedDto
      getPermissionsGroupedByResourceForRole(String roleId) {
    ...
    List<RolePermission> currentRolePermissions = rolePermissionRepository.findAllByRole(role);
    Set<String> assignedPermissionKeys =
        currentRolePermissions.stream()
    ...
    Map<String, String> permissionFilterTypeMap =
        currentRolePermissions.stream()
    ...
    Map<String, List<project.ktc.springboot_app.permission.dto.RolePermissionDetailDto>>
        resourceGroups =
            allPermissions.stream()
                .filter(
                    permission ->
                        !restrictedResources.contains(permission.getResourceName())) // Filter out
                // restricted
                // resources
                .map(
                    permission -> {
                      ...
                      boolean isAssigned =
                          assignedPermissionKeys.contains(permission.getPermissionKey());

                      // Get current filter type for this permission
                      String currentFilterType =
                          permissionFilterTypeMap.get(permission.getPermissionKey());
                ...
                .collect(
                    Collectors.groupingBy(
                        project.ktc.springboot_app.permission.dto.RolePermissionDetailDto
                            ::getResource));

    logger.debug(
        "Grouped permissions into {} resource categories (filtered out restricted resources)",
        resourceGroups.size());
  ...
  private project.ktc.springboot_app.permission.dto.PermissionDto convertToPermissionDto(
      Permission permission) {
