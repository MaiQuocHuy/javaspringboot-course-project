...
public class AdminCourseServiceImp implements AdminCourseService {
  ...
  private void invalidateSpecificCourseCache(String courseId, String courseSlug) {
    try {
      ...
      log.error(
          "❌ Failed to invalidate specific course cache for ID: {} and slug: {}: {}",
          courseId,
          courseSlug,
          e.getMessage(),
          e);
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<CourseReviewResponseDto>>> getReviewCourses(
    ...
    log.info(
        "Fetching courses for review with filters: status={}, createdBy={}, dateRange=[{} to {}]",
        filterDto.getStatus(),
    ...
    try {
      ...
      // Execute query
      Page<CourseReviewProjection> projectionPage =
          adminCourseRepository.findCoursesForReview(
              statusList,
              filterDto.getCreatedBy(),
              filterDto.getDateFrom(),
              filterDto.getDateTo(),
  ...
  private PaginatedResponse<CourseReviewResponseDto> toPaginated(
      Page<CourseReviewResponseDto> page) {
  ...
  private CourseReviewResponseDto mapProjectionToDto(CourseReviewProjection projection) {
    return CourseReviewResponseDto.builder()
        ...
        .createdBy(
            CourseReviewResponseDto.CreatedByDto.builder()
  ...
  public ResponseEntity<ApiResponse<CourseReviewDetailResponseDto>> getCourseReviewDetail(
    ...
    try {
      ...
      log.error(
          "Error fetching course review detail for course ID {}: {}", courseId, e.getMessage(), e);
      throw new RuntimeException("Failed to retrieve course review detail", e);
    }
  }

  private CourseReviewDetailResponseDto mapCourseToDetailDto(
      Course course, List<Section> sections) {
    ...
    if (course.getCategories() != null && !course.getCategories().isEmpty()) {
      categorySummaries =
          course.getCategories().stream()
              .map(
                  cat ->
                      CourseReviewDetailResponseDto.CategorySummary.builder()
                          .id(cat.getId())
                          .name(cat.getName())
                          .build())
    ...
    int totalLessons = sections.stream().mapToInt(section -> section.getLessons().size()).sum();
    ...
    // Map sections
    List<CourseReviewDetailResponseDto.SectionDetailDto> sectionDtos =
        sections.stream().map(this::mapSectionToDetailDto).collect(Collectors.toList());
    ...
    return CourseReviewDetailResponseDto.builder()
        ...
        .createdBy(
            CourseReviewDetailResponseDto.CreatedByDto.builder()
  ...
  private CourseReviewDetailResponseDto.SectionDetailDto mapSectionToDetailDto(Section section) {
    ...
    // Map lessons
    List<CourseReviewDetailResponseDto.LessonDetailDto> lessonDtos =
        lessons.stream().map(this::mapLessonToDetailDto).collect(Collectors.toList());
  ...
  private CourseReviewDetailResponseDto.LessonDetailDto mapLessonToDetailDto(Lesson lesson) {
    ...
    if (lesson.getContent() instanceof VideoContent) {
      VideoContent video = (VideoContent) lesson.getContent();
      videoDto =
          CourseReviewDetailResponseDto.VideoDetailDto.builder()
              .id(video.getId())
              .url(video.getUrl())
      ...
      List<CourseReviewDetailResponseDto.QuestionDetailDto> questionDtos =
          questions.stream()
              .map(
                  question -> {
      ...
      quizDto =
          CourseReviewDetailResponseDto.QuizDetailDto.builder().questions(questionDtos).build();
    }
  ...
  public ResponseEntity<ApiResponse<CourseReviewStatusUpdateResponseDto>> updateCourseReviewStatus(
    ...
    log.info(
        "Admin request to update course review status - courseId: {}, status: {}, reviewer: {}",
        courseId,
        updateDto.getStatus(),
        reviewerEmail);
    ...
    try {
      ...
      CourseReviewStatus.ReviewStatus newStatus =
          "APPROVED".equals(updateDto.getStatus())
      ...
      CourseReviewStatusHistory history =
      ...
      // Build response DTO
      CourseReviewStatusUpdateResponseDto.CourseReviewStatusUpdateResponseDtoBuilder
          responseBuilder =
              CourseReviewStatusUpdateResponseDto.builder()
                  .id(course.getId())
                  .title(course.getTitle())
                  .description(course.getDescription())
      ...
      if ("REJECTED".equals(updateDto.getStatus())) {
        ...
        notificationHelper.createInstructorCourseRejectedNotification(
            courseRejected.getInstructor().getId(),
            courseRejected.getId(),
            courseRejected.getTitle(),
            String.format("/instructor/courses/%s", courseRejected.getId()),
            updateDto.getReason());
        ...
        notificationHelper.createInstructorCourseApprovedNotification(
            courseApproved.getInstructor().getId(),
            courseApproved.getId(),
            courseApproved.getTitle(),
      ...
      log.info(
          "Successfully updated course review status - courseId: {}, previousStatus: {}, newStatus: {}",
          courseId,
          previousStatus,
          newStatus);
