...
public interface InstructorSectionRepository extends JpaRepository<Section, String> {
  ...
  @Query(
      "SELECT s FROM Section s " + "WHERE s.course.id = :courseId " + "ORDER BY s.orderIndex ASC")
  ...
  List<Section> findSectionsByCourseIdAndOrderGreaterThan(
      @Param("courseId") String courseId, @Param("orderIndex") Integer orderIndex);
