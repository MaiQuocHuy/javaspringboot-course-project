...
/**
 * Notification service for automated payout processing Sends email notifications about payout
...
@ConditionalOnProperty(
    value = "app.payout.notification.enabled",
    havingValue = "true",
    matchIfMissing = true)
public class PayoutNotificationService {
   ...
   */
  public void sendPayoutProcessingSummary(
    ...
    try {
      String subject =
          String.format(
              "Automatic Payout Processing Summary - %s",
  ...
  public void sendPayoutProcessingError(Exception error) {
    ...
    try {
      String subject =
          "⚠️ Automatic Payout Processing Error - "
              + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
  ...
  private void sendNotificationToAdmins(String subject, String content) {
    ...
    log.info(
        "📧 Attempting to send notification to {} admin emails: {}",
        adminEmails.size(),
        adminEmails);
    ...
    for (String adminEmail : adminEmails) {
      try {
        ...
        if (result.isSuccess()) {
          ...
          log.error(
              "❌ Failed to send notification to {}: {}", adminEmail, result.getErrorMessage());
        ...
        log.error(
            "❌ Exception sending payout notification to admin {}: {}",
            adminEmail,
            e.getMessage(),
            e);
    ...
    try {
      ...
      var result =
          emailService.sendTemplateEmail(
              adminEmail, subject, "general-template", createTemplateData(subject, content));
  ...
  private String buildPayoutSummaryContent(
      int successCount, int failureCount, BigDecimal totalAmount) {
    ...
    content
        .append("<p><strong>Processing Time:</strong> ")
        .append(nowVN.format(FORMATTER))
        .append("</p>");
    content
        .append("<p><strong>✅ Successful Payouts:</strong> ")
        .append(successCount)
        .append("</p>");
    content.append("<p><strong>❌ Failed Payouts:</strong> ").append(failureCount).append("</p>");
    content
        .append("<p><strong>💰 Total Amount Processed:</strong> $")
        .append(totalAmount)
        .append("</p>");
    content.append("</div>");
    ...
    if (successCount > 0) {
      content.append(
          "<p>✅ Instructor earnings have been created and are available for payout.</p>");
    }
    ...
    content.append("<hr>");
    content.append(
        "<p><em>This is an automated notification from the KTC Learning Platform payout system.</em></p>");
  ...
  private String buildErrorNotificationContent(Exception error) {
    ...
    content.append(
        "<div style='margin: 20px 0; padding: 10px; background-color: #ffebee; border-left: 4px solid #f44336;'>");
    content
        .append("<p><strong>Error Time:</strong> ")
        .append(nowVN.format(FORMATTER))
        .append("</p>");
    content
        .append("<p><strong>Error Type:</strong> ")
    ...
    content.append(
        "<p>The automatic payout processing encountered an error and requires immediate attention:</p>");
    content.append("<ul>");
    ...
    content.append("<hr>");
    content.append(
        "<p><em>This is an automated error notification from the KTC Learning Platform payout system.</em></p>");

    return content.toString();
  }

  /** Build content for daily summary */
  private String buildDailySummaryContent(
      PayoutEligibilityService.PayoutEligibilitySummary summary) {
    StringBuilder content = new StringBuilder();
    content.append("<h3>📊 Daily Payout Summary</h3>");
    content.append("<div style='margin: 20px 0;'>");
    content
        .append("<p><strong>Report Date:</strong> ")
    ...
    content.append("<tr>");
    content.append(
        "<td style='border: 1px solid #ddd; padding: 8px;'>Total Completed Payments</td>");
    content
        .append("<td style='border: 1px solid #ddd; padding: 8px; text-align: right;'>")
        .append(summary.getTotalCompletedPayments())
        .append("</td>");
    content.append("</tr>");
    ...
    content
        .append("<td style='border: 1px solid #ddd; padding: 8px; text-align: right;'>")
        .append(summary.getAlreadyPaidOut())
        .append("</td>");
    content.append("</tr>");
    ...
    content
        .append("<td style='border: 1px solid #ddd; padding: 8px; text-align: right;'>")
        .append(summary.getWithinWaitingPeriod())
        .append("</td>");
    content.append("</tr>");
    ...
    content
        .append("<td style='border: 1px solid #ddd; padding: 8px; text-align: right;'>")
        .append(summary.getWithBlockingRefunds())
        .append("</td>");
    content.append("</tr>");

    content.append("<tr style='background-color: #e8f5e8;'>");
    content.append(
        "<td style='border: 1px solid #ddd; padding: 8px;'><strong>Eligible for Payout</strong></td>");
    content
        .append("<td style='border: 1px solid #ddd; padding: 8px; text-align: right;'><strong>")
        .append(summary.getEligibleForPayout())
        .append("</strong></td>");
    content.append("</tr>");
    ...
    if (summary.getEligibleForPayout() > 0) {
      content.append(
          "<div style='margin: 20px 0; padding: 10px; background-color: #e8f5e8; border-left: 4px solid #4caf50;'>");
      content
          .append("<p>✅ <strong>")
          .append(summary.getEligibleForPayout())
          .append("</strong> payments are ready for automatic processing.</p>");
      content.append("</div>");
    }

    content.append("<hr>");
    content.append(
        "<p><em>This is an automated daily summary from the KTC Learning Platform payout system.</em></p>");

    return content.toString();
  }

  /**
   * Send test notification to verify email configuration This method can be called manually to test
   * email sending
   */
  ...
  private void debugEmailConfiguration() {
    try {
      ...
      log.info(
          "  - Admin emails count: {}",
          payoutProperties.getNotification().getAdmin().getEmails().size());
  ...
  private String buildTestNotificationContent() {
    ...
    content.append(
        "<div style='margin: 20px 0; padding: 10px; background-color: #e3f2fd; border-left: 4px solid #2196f3;'>");
    content
        .append("<p><strong>Test Time:</strong> ")
        .append(nowVN.format(FORMATTER))
        .append("</p>");
    ...
    content.append("<ul>");
    content
        .append("<li><strong>Notification Enabled:</strong> ")
        .append(payoutProperties.getNotification().isEnabled())
        .append("</li>");
    content
        .append("<li><strong>Admin Emails:</strong> ")
        .append(String.join(", ", payoutProperties.getNotification().getAdmin().getEmails()))
        .append("</li>");
    content
        .append("<li><strong>Scheduling Enabled:</strong> ")
        .append(payoutProperties.getScheduling().isEnabled())
        .append("</li>");
    content.append("</ul>");
    ...
    content.append(
        "<div style='margin: 20px 0; padding: 10px; background-color: #e8f5e8; border-left: 4px solid #4caf50;'>");
    content.append(
        "<p>✅ <strong>Success!</strong> If you receive this email, the notification system is working correctly.</p>");
    content.append("</div>");

    content.append("<hr>");
    content.append(
        "<p><em>This is a test notification from the KTC Learning Platform payout system.</em></p>");
