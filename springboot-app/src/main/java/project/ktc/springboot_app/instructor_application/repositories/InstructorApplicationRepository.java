...
public interface InstructorApplicationRepository
    extends JpaRepository<InstructorApplication, String> {

  /** Find instructor application by user ID */
  @Query(
      "SELECT ia FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.isDeleted = false ORDER BY ia.submittedAt DESC")
  ...
  @Query(
      "SELECT COUNT(ia) > 0 FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.isDeleted = false")
  boolean existsByUserId(@Param("userId") String userId);

  /** Check if user already has an application in pending or approved status */
  @Query(
      "SELECT ia.status FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.status IN ('PENDING', 'APPROVED') AND ia.isDeleted = false")
  List<InstructorApplication.ApplicationStatus> findActiveStatusesByUserId(
      @Param("userId") String userId);

  /** Check if user has rejected application within recent period */
  @Query(
      "SELECT ia FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.status = 'REJECTED' AND ia.isDeleted = false ORDER BY ia.reviewedAt DESC")
  Optional<InstructorApplication> findLatestRejectedByUserId(@Param("userId") String userId);

  /** Count user's rejected applications */
  @Query(
      "SELECT COUNT(ia) FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.status = 'REJECTED' AND ia.isDeleted = false")
  long countRejectedApplicationsByUserId(@Param("userId") String userId);

  /** Count user's pending applications */
  @Query(
      "SELECT COUNT(ia) FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.status = 'PENDING' AND ia.isDeleted = false")
  long countPendingApplicationsByUserId(@Param("userId") String userId);

  /** Find all instructor applications */
  @Query(
      "SELECT ia FROM InstructorApplication ia JOIN FETCH ia.user WHERE ia.isDeleted = false ORDER BY ia.submittedAt DESC")
  @NonNull
  ...
  @Query(
      "SELECT ia FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.isDeleted = false ORDER BY ia.submittedAt DESC LIMIT 1")
  Optional<InstructorApplication> findFirstByUserIdOrderBySubmittedAtDesc(
      @Param("userId") String userId);

  /** Find latest instructor application status by user ID */
  @Query(
      "SELECT ia.status FROM InstructorApplication ia WHERE ia.user.id = :userId AND ia.isDeleted = false ORDER BY ia.submittedAt DESC LIMIT 1")
