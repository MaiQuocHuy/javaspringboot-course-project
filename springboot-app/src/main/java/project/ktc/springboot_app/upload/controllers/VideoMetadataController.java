...
@Tag(
    ...
    description = "Endpoints for retrieving video metadata from Cloudinary")
...
public class VideoMetadataController {
  ...
  @Operation(
      ...
      description =
          "Retrieves comprehensive metadata for a video from Cloudinary including title, format, duration, thumbnail, dimensions, and file size")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<VideoMetadataResponseDto>>
      getVideoMetadata(
          @Parameter(
                  description = "Cloudinary video URL or public ID",
                  example =
                      "https://res.cloudinary.com/demo/video/upload/v1234567890/sample_video.mp4",
                  required = true)
              @RequestParam
              @NotBlank(message = "Video URL or public ID cannot be blank")
              String videoUrlOrPublicId) {
    ...
    try {
      ...
      return ApiResponseUtil.success(metadata, "Video metadata retrieved successfully");

    } catch (IllegalArgumentException e) {
      log.warn("Invalid video URL or public ID format: {}", videoUrlOrPublicId, e);
      return ApiResponseUtil.error(
          HttpStatus.BAD_REQUEST, "Invalid video URL or public ID format: " + e.getMessage());
      ...
      if (e.getMessage() != null && e.getMessage().toLowerCase().contains("not found")) {
        return ApiResponseUtil.error(
            HttpStatus.NOT_FOUND, "Video not found on Cloudinary: " + videoUrlOrPublicId);
      }

      return ApiResponseUtil.error(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to retrieve video metadata: " + e.getMessage());
  ...
  @Operation(
      summary = "Get video metadata by public ID",
      description = "Retrieves comprehensive metadata for a video using its Cloudinary public ID")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Video metadata retrieved successfully"),
        @ApiResponse(responseCode = "404", description = "Video not found on Cloudinary"),
  ...
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<VideoMetadataResponseDto>>
      getVideoMetadataByPublicId(
          @Parameter(
                  description = "Cloudinary video public ID",
    ...
    try {
      ...
      if (e.getMessage() != null && e.getMessage().toLowerCase().contains("not found")) {
        return ApiResponseUtil.error(
            HttpStatus.NOT_FOUND, "Video not found on Cloudinary with public ID: " + publicId);
      }
