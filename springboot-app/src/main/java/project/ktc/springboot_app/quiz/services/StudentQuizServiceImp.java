...
public class StudentQuizServiceImp implements StudentQuizService {
	...
	@Override
	public ResponseEntity<ApiResponse<PaginatedResponse<QuizScoreResponseDto>>> getQuizScores(
			Pageable pageable) {
		...
		Page<QuizResult> quizResultsPage = quizResultRepository.findQuizScoresByUserId(currentUser.getId(), pageable);
		...
		PaginatedResponse<QuizScoreResponseDto> paginatedResponse = PaginatedResponse.<QuizScoreResponseDto>builder()
	...
	@Override
	public ResponseEntity<ApiResponse<QuizScoreDetailResponseDto>> getQuizScoreDetail(
			String quizResultId) {
		...
		List<QuizQuestion> quizQuestions = quizQuestionRepository.findByLessonId(quizResult.getLesson().getId());

		// Parse student answers
		Map<String, String> studentAnswers = parseStudentAnswers(quizResult.getAnswers());

		// Build response
		QuizScoreDetailResponseDto responseDto = mapToQuizScoreDetailDto(quizResult, quizQuestions, studentAnswers);
	...
	private Map<String, String> parseStudentAnswers(String answersJson) {
		try {
			...
			Map<String, String> answers = objectMapper.readValue(answersJson, new TypeReference<Map<String, String>>() {
			});
	...
	public ResponseEntity<ApiResponse<StudentQuizStatsDto>> getQuizStats() {
		try {
			...
			BigDecimal roundedAverage = BigDecimal.valueOf(averageScoreValue).setScale(2, RoundingMode.HALF_UP);
			...
			log.info(
					"Quiz statistics retrieved successfully for user {}: Total={}, Passed={}, Failed={}, Average={}",
					currentUserId,
					totalQuizzesValue,
					passedQuizzesValue,
					failedQuizzesValue,
