...
public class ChatMessageServiceImp implements ChatMessageService {
	...
	public ResponseEntity<ApiResponse<ChatMessageResponse>> sendMessage(
		...
		try {
			...
			var type = chatMessageTypeRepository
			...
			switch (type.getName().toUpperCase()) {
				case "TEXT" -> {
					ChatMessageText text = ChatMessageText.builder().message(message).content(request.getContent())
							.build();
	...
	public ResponseEntity<ApiResponse<PaginatedResponse<ChatMessageResponse>>> listMessages(
		...
		try {
			...
			List<ChatMessageResponse> messageDtos = messagesPage.getContent().stream().map(this::toResponse)
		...
		try {
			...
			if (beforeMessageId != null || afterMessageId != null) {
				...
				if (beforeMessageId != null) {
					...
					messages = chatMessageRepository.findByCourseIdBeforeMessageId(
					...
					messages = chatMessageRepository.findByCourseIdAfterMessageId(
							courseId, afterMessageUuid, pageable);
				}

				// For infinite scroll, return simple message list
				List<SimpleChatMessageResponse> messageList = messages.stream().map(this::toSimpleResponse)
						.collect(Collectors.toList());
	...
	public ResponseEntity<ApiResponse<ChatMessageResponse>> updateMessage(
		...
		try {
			...
			if (message.getTextDetail() != null) {
				...
				ChatMessageText textDetail = ChatMessageText.builder().message(message).content(request.getContent())
						.build();
