...
public class AdminPaymentServiceImp implements AdminPaymentService {
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<AdminPaymentResponseDto>>> getAllPayments(
    ...
    try {
      log.info(
          "Admin retrieving all payments with pagination: page={}, size={}",
          pageable.getPageNumber(),
          pageable.getPageSize());

      Page<Payment> payments = adminPaymentRepository.findAllPayments(pageable);

      List<AdminPaymentResponseDto> paymentDtos =
          payments.getContent().stream()
              .map(AdminPaymentResponseDto::fromEntity)
              .collect(Collectors.toList());

      PaginatedResponse<AdminPaymentResponseDto> paginatedResponse =
          PaginatedResponse.<AdminPaymentResponseDto>builder()
              .content(paymentDtos)
              .page(
      ...
      log.info(
          "Retrieved {} payments for admin (page {} of {})",
          paymentDtos.size(),
  ...
  public ResponseEntity<ApiResponse<List<AdminPaymentResponseDto>>> getAllPayments() {
    try {
      ...
      List<AdminPaymentResponseDto> paymentDtos =
          payments.stream().map(AdminPaymentResponseDto::fromEntity).collect(Collectors.toList());
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<AdminPaymentResponseDto>>> getAllPayments(
    ...
    try {
      ...
      Page<Payment> payments =
          adminPaymentRepository.findAllPaymentsWithFilter(
      ...
      log.info(
          "Retrieved {} payments out of {} total with filters applied",
          paymentDtos.size(),
  ...
  public ResponseEntity<ApiResponse<AdminPaymentDetailResponseDto>> getPaymentDetail(
    ...
    try {
      ...
      if (stripePaymentDetailsService.isStripePayment(payment.getPaymentMethod())
        ...
        log.info(
            "Fetching Stripe payment details for payment: {} with session: {}",
            paymentId,
        ...
        try {
          ...
          if (stripeData != null) {
            adminStripeData =
                AdminPaymentDetailResponseDto.StripePaymentData.builder()
          ...
          paymentDetail =
              AdminPaymentDetailResponseDto.fromEntityWithStripeData(payment, adminStripeData);

          log.info("Successfully fetched Stripe payment details for payment: {}", paymentId);
        } catch (Exception stripeError) {
          log.warn(
              "Failed to fetch Stripe details for payment {}: {}. Using basic payment info.",
              paymentId,
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve payment detail. Please try again later.");
    }
  }

  @Override
  @Transactional
  public ResponseEntity<ApiResponse<AdminUpdatePaymentStatusResponseDto>> updatePaymentStatus(
    ...
    try {
      ...
      if (payment.getStatus() != PaymentStatus.PENDING) {
        log.warn(
            "Cannot update payment status. Current status is: {} for payment: {}",
            payment.getStatus(),
            paymentId);
        return ApiResponseUtil.badRequest(
            String.format(
                "Cannot update payment status. Payment is currently %s", payment.getStatus()));
      }
      ...
      if (paymentStatus == PaymentStatus.COMPLETED) {
        ...
        try {
          enrollmentService.createEnrollmentFromWebhook(
              payment.getUser().getId(), payment.getCourse().getId(), payment.getSessionId());
          log.info(
              "Enrollment created successfully for user {} in course {} from payment {}",
          ...
          stripeWebhookService.sendPaymentConfirmationEmail(
              session, payment.getCourse().getId(), payment.getUser().getId());
        } catch (Exception e) {
          log.error(
              "Failed to create enrollment for user {} in course {} from payment {}: {}",
              payment.getUser().getId(),
              payment.getCourse().getId(),
              paymentId,
              e.getMessage());
      ...
      try {
        ...
        var oldPaymentLog =
            PaymentLogMapper.toLogDto(payment); // Use original payment data as "old" values
        ...
        log.info(
            "Payment status update logged for payment {} from {} to {}",
            paymentId,
            oldStatus,
            newStatus);
      ...
      AdminUpdatePaymentStatusResponseDto responseDto =
          AdminUpdatePaymentStatusResponseDto.builder()
      ...
      if (paymentStatus == PaymentStatus.COMPLETED) {
        ...
        notificationHelper.createAdminStudentPaymentNotification(
            payment.getId(), userName, courseTitle, payment.getAmount());
        ...
        try {
          ...
          notificationHelper
              .createPaymentSuccessNotification(
                  userId, payment.getId(), courseTitle, courseUrl, courseId)
              .thenAccept(
                  notification ->
                      log.info(
      ...
      log.info(
          "Successfully updated payment status for payment: {} from {} to {}",
          paymentId,
          oldStatus,
          newStatus);
  ...
  @Override
  public ResponseEntity<ApiResponse<Page<AdminPaymentResponseDto>>> getPaymentsByUserId(
      String userId, Pageable pageable) {
    try {
      log.info(
          "Admin retrieving payments for user: {} with pagination: page={}, size={}",
          userId,
          pageable.getPageNumber(),
          pageable.getPageSize());

      Page<Payment> payments =
          adminPaymentRepository.findPaymentsByUserIdForAdmin(userId, pageable);

      Page<AdminPaymentResponseDto> paymentDtos = payments.map(AdminPaymentResponseDto::fromEntity);

      log.info(
          "Retrieved {} payments for user {} (page {} of {})",
      ...
      return ApiResponseUtil.success(
          paymentDtos, String.format("Payments for user %s retrieved successfully", userId));
  ...
  @Override
  public ResponseEntity<ApiResponse<Page<AdminPaymentResponseDto>>> getPaymentsByCourseId(
      String courseId, Pageable pageable) {
    try {
      log.info(
          "Admin retrieving payments for course: {} with pagination: page={}, size={}",
          courseId,
          pageable.getPageNumber(),
          pageable.getPageSize());

      Page<Payment> payments =
          adminPaymentRepository.findPaymentsByCourseIdForAdmin(courseId, pageable);

      Page<AdminPaymentResponseDto> paymentDtos = payments.map(AdminPaymentResponseDto::fromEntity);

      log.info(
          "Retrieved {} payments for course {} (page {} of {})",
          paymentDtos.getNumberOfElements(),
          courseId,
          paymentDtos.getNumber() + 1,
          paymentDtos.getTotalPages());
      ...
      return ApiResponseUtil.success(
          paymentDtos, String.format("Payments for course %s retrieved successfully", courseId));

    } catch (Exception e) {
      log.error(
          "Error retrieving payments for course {} by admin: {}", courseId, e.getMessage(), e);
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve payments. Please try again later.");
    }
  }
  ...
  @Override
  public ResponseEntity<ApiResponse<Page<AdminPaymentResponseDto>>> searchPayments(
      String searchTerm, Pageable pageable) {
    try {
      log.info(
          "Admin searching payments with term: '{}' with pagination: page={}, size={}",
          searchTerm,
          pageable.getPageNumber(),
          pageable.getPageSize());
      ...
      log.info(
          "Found {} payments matching search term '{}' (page {} of {})",
          paymentDtos.getNumberOfElements(),
          searchTerm,
          paymentDtos.getNumber() + 1,
          paymentDtos.getTotalPages());
      ...
      return ApiResponseUtil.success(
          paymentDtos, String.format("Search results for '%s' retrieved successfully", searchTerm));

    } catch (Exception e) {
      log.error(
          "Error searching payments with term '{}' by admin: {}", searchTerm, e.getMessage(), e);
      return ApiResponseUtil.internalServerError(
          "Failed to search payments. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<AdminPaidOutResponseDto>> paidOutPayment(String paymentId) {
    try {
      ...
      if (payment.getStatus() != PaymentStatus.COMPLETED) {
        log.warn(
            "Payment {} is not in COMPLETED status. Current status: {}",
            paymentId,
            payment.getStatus());
      ...
      if (timeSinceCompletion.toDays() < 3) {
        ...
        log.warn(
            "Payment {} is within 3-day waiting period. {} hours remaining",
            paymentId,
            hoursRemaining);
        return ApiResponseUtil.badRequest(
            String.format(
                "Payment must wait 3 days before paid out. %d hours remaining", hoursRemaining));
      }
      ...
      Optional<InstructorEarning> existingEarning =
          instructorEarningRepository.findByPaymentId(paymentId);
      if (existingEarning.isPresent()) {
        log.warn(
            "Instructor earning already exists for payment {}: {}",
            paymentId,
      ...
      if (payment.getSessionId() != null && !payment.getSessionId().trim().isEmpty()) {
        try {
          ...
          PaymentDetailDto paymentDetails =
              ExtractPaymentDetailFromSessionId.extractPaymentDetails(payment.getSessionId());
          ...
          if (paymentDetails.getOriginalPrice() != null) {
            ...
            log.warn(
                "Original price not found in Stripe session metadata, using payment amount: {}",
                payment.getAmount());
          }
        } catch (Exception e) {
          log.error(
        ...
        log.info(
            "No session ID found for payment {}, using payment amount for calculation", paymentId);
      }
      ...
      // 10. Build response
      AdminPaidOutResponseDto response =
          AdminPaidOutResponseDto.builder()
              ...
              .message(
                  String.format(
                      ...
                      baseAmountForCalculation.equals(payment.getAmount())
                          ? "discounted"
                          : "original",
                      baseAmountForCalculation))
              .build();
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to process paid out. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<AdminPaymentStatisticsResponseDto>> getPaymentStatistics() {
    try {
      ...
      Long completedPayments =
          adminPaymentRepository.countPaymentsByStatus(PaymentStatus.COMPLETED);
      ...
      AdminPaymentStatisticsResponseDto paymentStatistics =
          AdminPaymentStatisticsResponseDto.builder()
              .total(totalPayments)
      ...
      log.info(
          "Payment statistics: Total={}, Pending={}, Completed={}, Failed={}, Refunded={}",
          totalPayments,
          pendingPayments,
          completedPayments,
          failedPayments,
      ...
      return ApiResponseUtil.success(
          paymentStatistics, "Payment statistics retrieved successfully");
  ...
  /**
   * Creates affiliate payout for referral discounts when payment is paid out This ensures
   * commission is only paid when payment is actually processed
   */
  public void createAffiliatePayoutForReferralDiscount(Payment payment) {
    try {
      ...
      List<DiscountUsage> discountUsages =
          discountUsageRepository.findByUserIdAndCourseId(
      ...
      log.error(
          "❌ Error in affiliate payout creation for payment: {} - {}",
          payment.getId(),
          e.getMessage(),
          e);
