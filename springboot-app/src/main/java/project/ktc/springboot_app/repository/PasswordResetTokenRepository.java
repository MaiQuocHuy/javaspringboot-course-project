 ...
 *
 * <p>Provides data access methods for managing password reset tokens including OTP verification,
...
public interface PasswordResetTokenRepository extends JpaRepository<PasswordResetToken, String> {
  ...
  /**
   * Finds a valid password reset token by user ID and OTP code. A valid token is one that is not
  ...
  /**
   * Finds the most recent password reset token for a user. Useful for checking rate limiting and
  ...
  /**
   * Finds all unexpired and unused tokens for a specific user. Used for invalidating previous
  ...
  /**
   * Invalidates (marks as used) all active tokens for a user. Called when a new password reset
  ...
  /**
   * Cleans up expired tokens from the database. Should be called periodically (e.g., via scheduled
  ...
  /**
   * Cleans up old used tokens to prevent database bloat. Removes tokens that were used more than a
   * specified number of days ago.
   *
   * @param cutoffTime the cutoff timestamp (tokens older than this will be deleted)
   * @return number of tokens deleted
   */
  @Modifying
  @Query(
  ...
  /**
   * Counts the number of password reset requests made by a user within a time window. Used for
  ...
  /**
   * Finds a token by its OTP code regardless of user (for uniqueness checks). Used to ensure OTP
  ...
  /**
   * Finds all tokens that will expire within a specified time. Useful for sending reminder
