...
public class InstructorEarningServiceImp implements InstructorEarningService {
  ...
  public ResponseEntity<ApiResponse<EarningsWithSummaryDto>> getEarnings(
    ...
    try {
      ...
      Page<InstructorEarning> earningsPage =
          instructorEarningRepository.findEarningsWithFilters(
              currentUserId, courseId, dateFrom, dateTo, pageable);

      // Convert to DTOs
      List<EarningResponseDto> earningDtos =
          earningsPage.getContent().stream()
      ...
      PaginatedResponse<EarningResponseDto> paginatedEarnings =
          PaginatedResponse.<EarningResponseDto>builder()
              .content(earningDtos)
              .page(
      ...
      EarningsWithSummaryDto responseData =
          EarningsWithSummaryDto.builder().earnings(paginatedEarnings).summary(summary).build();

      log.info(
          "Successfully retrieved {} earnings for instructor {}",
          earningDtos.size(),
          currentUserId);
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve earnings. Please try again later.");
    }
  }
  ...
  private EarningSummaryDto calculateSummary(
      String instructorId, String courseId, LocalDateTime dateFrom, LocalDateTime dateTo) {
    ...
    if (hasFilters(courseId, dateFrom, dateTo)) {
      totalEarnings =
          instructorEarningRepository.getTotalEarningsWithFilters(
              instructorId, courseId, dateFrom, dateTo);
      totalTransactions =
          instructorEarningRepository.getTotalTransactionsWithFilters(
              instructorId, courseId, dateFrom, dateTo);
    } else {
      totalEarnings = instructorEarningRepository.getTotalEarningsByInstructor(instructorId);
      totalTransactions =
          instructorEarningRepository.getTotalTransactionsByInstructor(instructorId);
    }
    ...
    try {
      ...
      InstructorEarning earning =
          instructorEarningRepository
              .findByIdAndInstructorId(earningId, currentUserId)
              .orElse(null);

      if (earning == null) {
        log.warn(
            "Earning not found or access denied for earningId: {} and instructorId: {}",
            earningId,
            currentUserId);
      ...
      log.error(
          "Error retrieving earning details for earningId: {}: {}", earningId, e.getMessage(), e);
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve earning details. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<List<MonthlyEarningsDto>>> getRecentRevenues() {
    ...
    try {
      for (int i = 0; i < 3; i++) {
        ...
        BigDecimal revenue =
            instructorEarningRepository.getTotalEarningsByMonth(instructorId, year, month);
        MonthlyEarningsDto monthlyEarningsDto =
            MonthlyEarningsDto.builder()
                .year(year)
                .month(month)
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve recent revenues. Please try again later.");
    }
  }
}
