...
public class InstructorCourseServiceImp implements InstructorCourseService {
	...
	public ResponseEntity<ApiResponse<CourseResponseDto>> createCourse(
		...
		try {
			...
			List<CategoryInfo> categoryInfoList = categories.stream()
					.map(
							cat -> CourseResponseDto.CategoryInfo.builder()
									.id(cat.getId())
									.name(cat.getName())
									.build())
					.collect(Collectors.toList());
			...
			CourseResponseDto responseDto = CourseResponseDto.builder()
					...
					.categories(
							updatedCourse.getCategories().stream()
									.map(
											category -> CourseResponseDto.CategoryInfo.builder()
													.id(category.getId())
													.name(category.getName())
													.build())
									.collect(Collectors.toList()))
	...
	/**
	 * Check if course can be published Requirements: title, description, thumbnail,
	 * and at least one
	...
	/**
	 * Check if course can be unpublished Requirements: course must be approved and
	 * currently
	 * published
	 */
	...
	public ResponseEntity<ApiResponse<InstructorCourseDetailResponseDto>> getCourseDetails(
		...
		Course course = instructorCourseRepository
	...
	private InstructorCourseDetailResponseDto mapToInstructorCourseDetailResponse(
		...
		if (course.getCategories() != null && !course.getCategories().isEmpty()) {
			categoryInfos = course.getCategories().stream()
					.map(
							category -> BaseCourseResponseDto.CategoryInfo.builder()
									.id(category.getId())
									.name(category.getName())
									.build())
					.collect(Collectors.toList());
		...
		if (course.getSections() != null && !course.getSections().isEmpty()) {
			sectionInfos = course.getSections().stream().map(this::mapToSectionInfo).collect(Collectors.toList());
		}
	...
	/**
	 * Handle course review status when publishing a course. - First time publish:
	 * Create PENDING
	 * status - Subsequent publishes (when record exists, even with null status):
	 * Set to RESUBMITTED
	 */
	...
	/**
	 * Handle course review status when unpublishing a course. When unpublishing, we
	 * remove/set the
	 ...
	 * course_review_status_history Returns a
