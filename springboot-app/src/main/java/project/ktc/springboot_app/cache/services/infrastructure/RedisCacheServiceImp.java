...
/**
 * Redis implementation of the CacheService interface. Provides low-level Redis operations for the
  ...
  public <T> T get(String key, Class<T> clazz) {
    try {
      ...
      if (value != null && clazz.isInstance(value)) {
        ...
        log.warn(
            "Cache value type mismatch for key: {}. Expected: {}, Got: {}",
            key,
            clazz.getSimpleName(),
      ...
      log.error(
          "Error retrieving data from cache with key: {} and type: {}",
          key,
          clazz.getSimpleName(),
          e);
      return null;
  ...
  public <T> List<T> getList(String key, Class<T> elementClass) {
    try {
      ...
      if (value != null && value instanceof List) {
        log.debug(
            "Cache hit for list key: {} with element type: {}", key, elementClass.getSimpleName());
        return (List<T>) value;
      } else if (value != null) {
        log.warn(
            "Cache value type mismatch for key: {}. Expected: List<{}>, Got: {}",
            key,
      ...
      log.error(
          "Error retrieving list from cache with key: {} and element type: {}",
          key,
          elementClass.getSimpleName(),
          e);
      return null;
