...
public class InstructorApplicationServiceImp implements InstructorApplicationService {
    ...
    try {
      ...
      String applicationId =
          applicationRepository
              .findFirstByUserIdOrderBySubmittedAtDesc(currentUserId)
      ...
      notificationHelper.createAdminInstructorApplicationNotification(
          currentUserId, applicant.getName(), applicant.getEmail());

      // Create student notification about application submission
      notificationHelper
          .createInstructorApplicationSubmittedNotification(
              currentUserId, applicationId) // Use the actual application ID
          .thenAccept(
              notification ->
                  log.info(
          ...
          .exceptionally(
              ex -> {
                log.error(
                    "❌ Failed to create application submitted notification for user {}: {}",
                    currentUserId,
                    ex.getMessage(),
                    ex);
                return null;
      ...
      DocumentUploadResponseDto response =
          DocumentUploadResponseDto.builder().userId(currentUserId).documents(documentUrls).build();
  ...
  private void validateUserEligibility(String userId) {
    ...
    if (rejectionCount >= MAX_REJECTION_COUNT) {
      throw new IneligibleApplicationException(
          "You have reached the maximum number of rejections ("
              + MAX_REJECTION_COUNT
  ...
  private Map<String, String> uploadDocumentsToCloudinary(
    ...
    try {
      ...
      project.ktc.springboot_app.upload.dto.DocumentUploadResponseDto certificateResult =
          cloudinaryService.uploadDocument(certificate);
      ...
      project.ktc.springboot_app.upload.dto.DocumentUploadResponseDto cvResult =
          cloudinaryService.uploadDocument(cv);
      ...
      if (other != null && !other.isEmpty()) {
        ...
        project.ktc.springboot_app.upload.dto.DocumentUploadResponseDto otherResult =
            cloudinaryService.uploadDocument(other);
  ...
  public ResponseEntity<ApiResponse<InstructorApplicationDetailResponseDto>> getApplicationByUserId(
      String userId) {
    try {
      ...
      InstructorApplication application =
          applicationRepository
              ...
              .orElseThrow(
                  () -> new RuntimeException("Application not found for user id: " + userId));

      log.debug(
          "Found application for User ID: {}, Status: {}",
          application.getUser().getId(),
          application.getStatus());

      // Map to DTO
      InstructorApplicationDetailResponseDto responseDto =
          instructorApplicationMapper.toApplicationDetailResponseDto(application);

      long submitAttemptRemain =
          MAX_REJECTION_COUNT - applicationRepository.countRejectedApplicationsByUserId(userId);

      responseDto.setSubmitAttemptRemain(submitAttemptRemain);

      return ApiResponseUtil.success(
          responseDto, "Fetched application detail successfully for user id: " + userId);
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve application detail. Try again later.");
    }
  }

  @Override
  @Transactional
  public ResponseEntity<ApiResponse<List<AdminInstructorApplicationResponseDto>>>
      getAllApplicationAdmin() {
    List<InstructorApplication> applications = applicationRepository.findAllLatestPerUser();

    List<AdminInstructorApplicationResponseDto> responseList =
        instructorApplicationMapper.toAdminResponseDtoList(applications);
    ...
    return ApiResponseUtil.success(
        responseList, "Fetched all instructor applications successfully");
  }
  ...
  public ResponseEntity<ApiResponse<List<AdminInstructorApplicationResponseDto>>>
    ...
    List<AdminInstructorApplicationResponseDto> responseList =
        instructorApplicationMapper.toAdminDetailResponseDtoList(applications);
    ...
    return ApiResponseUtil.success(
        responseList, "Fetched applications successfully for user ID: " + userId);
  }

  @Override
  @Transactional
  public ResponseEntity<ApiResponse<Void>> reviewApplication(
      String applicationId, AdminReviewApplicationRequestDto request) {
    try {
      // Check authentication
      Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
      if (authentication == null || !authentication.isAuthenticated()) {
      ...
      InstructorApplication application =
          applicationRepository
              .findByIdAndNotDeleted(applicationId)
              .orElseThrow(() -> new RuntimeException("Application not found"));

      // Check if already reviewed
      if (!application.getStatus().equals(InstructorApplication.ApplicationStatus.PENDING)) {
        log.warn(
            "Application {} already reviewed with status: {}",
            applicationId,
            application.getStatus());
        return ApiResponseUtil.badRequest(
            "Application already reviewed with status: " + application.getStatus());
      }
      ...
      if (request.getAction() == ApplicationStatus.REJECTED) {
        ...
        notificationHelper.createStudentInstructorApplicationRejectedNotification(
            application.getUser().getId(), application.getId(), request.getRejectionReason());
      } else {
        ...
        notificationHelper.createStudentInstructorApplicationApprovedNotification(
            application.getUser().getId(), application.getId());
      }

      applicationRepository.save(application);

      String message =
          request.getAction() == ApplicationStatus.APPROVED
