...
public class CommentMapper {
  ...
  /**
   * Build comment tree structure from flat list of comments Groups replies under their parent
   * comments
   *
  ...
  public List<CommentResponse> buildCommentTree(List<Comment> comments) {
    ...
    List<Comment> rootComments =
        comments.stream().filter(Comment::isRootComment).collect(Collectors.toList());

    List<Comment> replies = comments.stream().filter(Comment::isReply).collect(Collectors.toList());

    // Convert root comments to DTOs
    List<CommentResponse> rootResponses =
        rootComments.stream().map(this::toCommentResponse).collect(Collectors.toList());
