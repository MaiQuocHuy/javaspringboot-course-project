...
public interface InstructorCourseRepository extends JpaRepository<Course, String> {
   ...
   * @param reviewStatus Filter by course review status (from CourseReviewStatusHistory's action
   *     field)
  ...
  @Query(
      "SELECT COUNT(c) FROM Course c WHERE c.instructor.id = :instructorId AND c.isDeleted = false AND c.isApproved = true")
  ...
  @Query(
      "SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.course.id = :courseId AND p.status = 'COMPLETED'")
  BigDecimal getTotalRevenueByCourseId(@Param("courseId") String courseId);

  /** Get last content update timestamp for a course */
  @Query(
      "SELECT MAX(GREATEST(COALESCE(s.updatedAt, s.createdAt), "
  ...
  Optional<Course> findByIdAndInstructorId(
      @Param("courseId") String courseId, @Param("instructorId") String instructorId);
  ...
  List<Course> findByInstructorIdAndIsPublished(
      @Param("instructorId") String instructorId, @Param("isPublished") Boolean isPublished);
}
