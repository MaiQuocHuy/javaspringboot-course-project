...
public interface AffiliatePayoutRepository extends JpaRepository<AffiliatePayout, String> {
  ...
  List<AffiliatePayout> findByReferredByUserIdAndPayoutStatus(
      String referredByUserId, PayoutStatus status);
  ...
  @Query(
      "SELECT COALESCE(SUM(ap.commissionAmount), 0) FROM AffiliatePayout ap "
  ...
  List<AffiliatePayout> findByCreatedAtBetween(
  ...
  List<AffiliatePayout> findByPaidAtBetween(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
  ...
  @Query(
      "SELECT ap FROM AffiliatePayout ap WHERE ap.payoutStatus = 'PENDING' "
