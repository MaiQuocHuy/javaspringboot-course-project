...
/**
 * Request DTO specifically for common discount creation Handles different validation rules for
 * GENERAL vs REFERRAL types
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
@Schema(
    description = "Request object for creating a common discount with flexible date requirements")
public class CommonDiscountRequest {
  ...
  @Digits(
      integer = 3,
      fraction = 2,
      message = "Discount percentage must have at most 3 integer digits and 2 decimal places")
  ...
  @Schema(
      ...
      example = "GENERAL",
      allowableValues = {"GENERAL", "REFERRAL"},
      required = true)
  private DiscountType type;

  /** Required for REFERRAL type discounts, must be null for GENERAL type */
  @Schema(
      description = "Owner user ID (required for REFERRAL type, must be null for GENERAL type)",
      example = "user-001")
  private String ownerUserId;

  /** For GENERAL type: required For REFERRAL type: optional (will use current time if null) */
  @Schema(
      description = "Discount start date (required for GENERAL, optional for REFERRAL)",
      example = "2024-01-01T00:00:00")
  private LocalDateTime startDate;

  /** For GENERAL type: required For REFERRAL type: optional (will use far future date if null) */
  @Schema(
      description = "Discount end date (required for GENERAL, optional for REFERRAL)",
  ...
  @Schema(
      description = "Maximum number of times a single user can use this discount",
      example = "1")
  ...
  @AssertTrue(
      message =
          "REFERRAL discounts require ownerUserId, GENERAL discounts must have ownerUserId as null")
