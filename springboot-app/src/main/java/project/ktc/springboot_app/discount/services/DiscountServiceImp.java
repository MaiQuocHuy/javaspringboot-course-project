...
/**
 * Implementation of DiscountService with comprehensive business logic Handles all discount-related
...
public class DiscountServiceImp implements DiscountService {
  ...
  public ResponseEntity<ApiResponse<DiscountResponseDto>> createDiscount(
    ...
    try {
      log.info(
          "Creating discount with code: {} and type: {}", request.getCode(), request.getType());
      ...
      if (request.getType() == DiscountType.REFERRAL) {
        ...
        ownerUser = userRepository.findById(request.getOwnerUserId()).orElse(null);
        ...
        Optional<Discount> existingReferralDiscount =
            discountRepository.findByOwnerUserIdAndType(
                request.getOwnerUserId(), DiscountType.REFERRAL);

        if (existingReferralDiscount.isPresent()) {
          log.warn(
              "User already has a REFERRAL discount. Owner user ID: {}", request.getOwnerUserId());
          return ApiResponseUtil.conflict(
              "User already has a referral discount. Cannot create another one.");
        }
      ...
      log.info(
          "Discount created successfully with ID: {} and code: {}",
          savedDiscount.getId(),
          savedDiscount.getCode());
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to create discount. Please try again later.");
    }
  }

  @Override
  public ResponseEntity<ApiResponse<PaginatedResponse<DiscountResponseDto>>> getAllDiscounts(
      Pageable pageable) {
    try {
      log.info(
          "Retrieving all discounts with pagination: page={}, size={}",
      ...
      PaginatedResponse<DiscountResponseDto> paginatedResponse =
          convertPageToPaginatedResponse(responsePage);
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve discounts. Please try again later.");
    }
  }

  @Override
  public ResponseEntity<ApiResponse<PaginatedResponse<DiscountResponseDto>>> getDiscountsByType(
  ...
  public ResponseEntity<ApiResponse<DiscountResponseDto>> getDiscountById(String id) {
    try {
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve discount. Please try again later.");
    }
  }
  ...
  @Override
  public ResponseEntity<ApiResponse<PaginatedResponse<DiscountResponseDto>>>
      getDiscountsByOwnerUserId(String ownerUserId, Pageable pageable) {
    try {
  ...
  @Override
  @Transactional
  public ResponseEntity<ApiResponse<DiscountResponseDto>> updateDiscountStatus(
  ...
  public ResponseEntity<ApiResponse<Void>> deleteDiscount(String id) {
    try {
      ...
      if (usageCount > 0) {
        log.warn(
            "Cannot delete discount with ID: {} because it has been used {} times", id, usageCount);
        return ApiResponseUtil.badRequest(
            "Cannot delete discount that has been used. Consider deactivating instead.");
      }
  ...
  public boolean isDiscountValidForUse(String discountCode, String userId, String courseId) {
    try {
      log.info(
          "Validating discount code: {} for user: {} and course: {}",
          discountCode,
          userId,
          courseId);
  ...
  @Override
  @Transactional
  public ResponseEntity<ApiResponse<DiscountResponseDto>> createUserInductionDiscount(
      String userId) {
    try {
      ...
      log.info(
          "User induction discount created successfully with ID: {} and code: {} for user: {}",
          savedDiscount.getId(),
          savedDiscount.getCode(),
          userId);
      ...
      log.error(
          "Error creating user induction discount for user {}: {}", userId, e.getMessage(), e);
      return ApiResponseUtil.internalServerError(
          "Failed to create induction discount. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<DiscountResponseDto>> getUserInductionDiscount(String userId) {
    try {
      ...
      Discount userReferralDiscount =
          discountRepository.findByOwnerUserIdAndType(userId, DiscountType.REFERRAL).orElse(null);
      ...
      log.info(
          "User induction discount found with ID: {} and code: {} for user: {}",
          userReferralDiscount.getId(),
          userReferralDiscount.getCode(),
          userId);
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve induction discount. Please try again later.");
    }
  }
  ...
  public ResponseEntity<ApiResponse<List<DiscountResponseDto>>> getAvailablePublicDiscounts() {
    try {
      ...
      List<Discount> availableDiscounts =
          discountRepository.findByTypeAndIsActiveAndCurrentlyValid(
              DiscountType.GENERAL, true, LocalDateTime.now());

      // Convert to DTOs with usage information
      List<DiscountResponseDto> responseDtos =
          availableDiscounts.stream().map(this::convertToDto).collect(Collectors.toList());

      // Log details for each discount
      responseDtos.forEach(
          dto -> {
            log.info(
                "Available discount: code={}, usageLimit={}, currentUsage={}, remaining={}",
                dto.getCode(),
                dto.getUsageLimit(),
                dto.getCurrentUsageCount(),
                dto.getRemainingUsageCount());
          });

      log.info("Found {} available public discount codes", responseDtos.size());
      return ApiResponseUtil.success(
          responseDtos, "Available public discounts retrieved successfully");

    } catch (Exception e) {
      log.error("Error getting available public discounts: {}", e.getMessage(), e);
      return ApiResponseUtil.internalServerError(
          "Failed to retrieve available discounts. Please try again later.");
    }
  }
  ...
  public DiscountResponseDto convertToDto(Discount discount) {
    ...
    if (discount.getUsageLimit() != null) {
      ...
      log.info(
          "Discount {}: usageLimit={}, currentUsage={}, remaining={}",
          discount.getCode(),
      ...
      log.info(
          "Discount {}: unlimited usage (usageLimit=null), currentUsage={}, remaining=-1 (unlimited)",
          discount.getCode(),
          currentUsageCount);
    }

    DiscountResponseDto.DiscountResponseDtoBuilder builder =
    ...
    if (discount.getType() == DiscountType.REFERRAL && discount.getOwnerUser() != null) {
      ...
      DiscountResponseDto.OwnerUserInfo ownerUserInfo =
          DiscountResponseDto.OwnerUserInfo.builder()
              .id(ownerUser.getId())
  ...
  private <T> PaginatedResponse<T> convertPageToPaginatedResponse(Page<T> page) {
    ...
    return PaginatedResponse.<T>builder().content(page.getContent()).page(pageInfo).build();
  }
}
