...
public class PaymentServiceImp implements PaymentService {
  ...
  @Override
  public void updatePaymentStatusFromWebhook(
      String paymentId, String status, String stripeSessionId) {
    log.info(
        "Updating payment {} status to {} from Stripe session {}",
        paymentId,
        status,
        stripeSessionId);
      ...
      if (paymentOpt.isPresent()) {
        ...
        try {
          ...
          systemLogHelper.logUpdate(
              currentUser, "PAYMENT", paymentId, oldPaymentLog, newPaymentLog);
          log.info(
              "Payment status update logged for payment {}: {} -> {}",
              paymentId,
              oldStatus,
              status);
        } catch (Exception logError) {
          log.warn(
              "Failed to log payment status update for payment {}: {}",
              paymentId,
              logError.getMessage());
        }
  ...
  public String createPayment(
      String userId, String courseId, Double amount, String stripeSessionId) {
    ...
    try {
      ...
      try {
        ...
        log.warn(
            "Failed to log payment creation for payment {}: {}",
            savedPayment.getId(),
            logError.getMessage());
      }
      ...
      log.error(
          "Error creating payment for user {} and course {}: {}",
          userId,
          courseId,
          e.getMessage(),
          e);
    ...
    try {
      ...
      if (paymentOpt.isPresent()) {
        ...
        try {
          ...
          log.warn(
              "Failed to log payment session ID update for payment {}: {}",
              paymentId,
              logError.getMessage());
        }
      ...
      log.error(
          "Error updating payment session ID for payment {}: {}", paymentId, e.getMessage(), e);
      throw new RuntimeException("Failed to update payment session ID", e);
    }
  }

  @Override
  public Optional<Payment> findPaymentBySessionIdAndVerifyAmount(
      String stripeSessionId, Double paidAmount) {
    log.info(
        "Finding payment by session ID {} and verifying amount {}", stripeSessionId, paidAmount);
    ...
    try {
      ...
      if (paymentOpt.isPresent()) {
        ...
        if (Math.abs(expectedAmount - paidAmount) <= tolerance) {
          log.info(
              "Payment amount verified successfully for session {}: expected={}, paid={}",
              stripeSessionId,
              expectedAmount,
              paidAmount);
          return paymentOpt;
          ...
          log.error(
              "Payment amount mismatch for session {}: expected={}, paid={}",
              stripeSessionId,
              expectedAmount,
              paidAmount);
  ...
  /**
   * Helper method to get current user for logging purposes Returns a system user if no
   * authenticated user is available (e.g., webhook operations)
   */
  ...
  public ResponseEntity<ApiResponse<List<PaymentResponseDto>>> getStudentPayments() {
    try {
      ...
      // Convert to DTOs
      List<PaymentResponseDto> paymentDtos =
          payments.stream().map(PaymentResponseDto::fromEntity).collect(Collectors.toList());
  ...
  @Override
  public ResponseEntity<ApiResponse<PaymentDetailResponseDto>> getStudentPaymentDetail(
      String paymentId) {
    try {
      // Get the current authenticated user from security context
      Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
      ...
      if (stripePaymentDetailsService.isStripePayment(payment.getPaymentMethod())
        ...
        PaymentDetailResponseDto.StripePaymentData stripeData =
            stripePaymentDetailsService.fetchPaymentDetails(payment.getSessionId());
  ...
  public ResponseEntity<ApiResponse<StudentPaymentStatsDto>> getStudentPaymentStats() {
    try {
      ...
      BigDecimal totalAmountSpent =
          paymentRepository.calculateTotalAmountSpentByUserId(currentUserId);
      ...
      BigDecimal totalAmountSpentValue =
          totalAmountSpent != null ? totalAmountSpent : BigDecimal.ZERO;
      ...
      StudentPaymentStatsDto stats =
          StudentPaymentStatsDto.builder()
