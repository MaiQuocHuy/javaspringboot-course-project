...
/**
 * Repository for student discount usage operations
 * Handles queries for discount usage from student perspective
 */
@Repository
public interface StudentDiscountUsageRepository extends JpaRepository<DiscountUsage, String> {

    /**
     * Find all discount usages where the current user is the referrer
    ...
    @Query("""
            ...
            """)
    Page<DiscountUsage> findByReferredByUserId(@Param("studentId") String studentId, Pageable pageable);
     ...
     */
    @Query("SELECT COALESCE(SUM(du.discountAmount), 0) FROM DiscountUsage du WHERE du.referredByUser.id = :studentId")
