...
/** Implementation of DiscountPriceService for handling discount validation and price calculation */
@Service
@Slf4j
...
public class DiscountPriceServiceImp implements DiscountPriceService {
  ...
  public PriceCalculationResponse calculatePrice(
      String courseId, String userId, String discountCode) {
    log.info(
        "Calculating price for course: {}, user: {}, discount: {}", courseId, userId, discountCode);
    ...
    if (discountCode != null && !discountCode.trim().isEmpty()) {
      try {
        ...
        log.info(
            "Discount applied successfully: {} -> Original: {}, Discount: {}, Final: {}",
            discountCode,
            originalPrice,
            discountAmount,
            finalPrice);
  ...
  @Override
  @Transactional
  public void recordDiscountUsage(
      String discountCode,
      String userId,
      String courseId,
      ...
      String paymentId) {
    log.info(
        "Recording discount usage: code={}, user={}, course={}, amount={}",
        discountCode,
        userId,
        courseId,
        discountAmount);
    ...
    try {
      ...
      Discount discount =
          discountRepository
              .findByCodeIgnoreCase(discountCode)
              .orElseThrow(
                  () -> new IllegalArgumentException("Discount not found: " + discountCode));
  ...
  private Discount validateAndGetDiscount(
      String discountCode, String userId, String courseId, BigDecimal originalPrice) {
    ...
    Discount discount =
        discountRepository
            .findByCodeIgnoreCase(discountCode)
            .orElseThrow(
                () -> new IllegalArgumentException("Invalid discount code: " + discountCode));
    ...
    if (discount.getType() == DiscountType.REFERRAL
        && discount.getOwnerUser() != null
        && discount.getOwnerUser().getId().equals(userId)) {
      throw new IllegalArgumentException(
          "You cannot use your own referral discount code: " + discountCode);
    }
