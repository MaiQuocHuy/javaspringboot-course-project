...
public interface PermissionRepository extends JpaRepository<Permission, String> {
  ...
  @Query(
      "SELECT p FROM Permission p "
          ...
          + "AND r.isActive = true "
          + "AND a.isActive = true")
  Optional<Permission> findByResourceNameAndActionName(
      @Param("resourceName") String resourceName, @Param("actionName") String actionName);
  ...
  /**
   * Find permission by permission key regardless of active status (used for reactivation scenarios)
   */
  ...
  List<Permission> findPermissionsByRoleType(@Param("roleType") String roleType);
  ...
  /**
   * Find all permissions with their resource and action details (for admin purposes) This includes
   * both active and inactive permissions
   */
  @Query(
      "SELECT p FROM Permission p "
          + "JOIN FETCH p.resource r "
  ...
  Optional<project.ktc.springboot_app.permission.entity.Action> findActionByName(
      @Param("name") String name);

  // Helper: find Resource by name (alternative to existing filtered query)
  @Query("SELECT r FROM Resource r WHERE r.name = :name")
  Optional<project.ktc.springboot_app.permission.entity.Resource> findResourceByName(
      @Param("name") String name);
}
