...
public class AsyncMessageRequestValidator
    ...
    public boolean isValid(AsyncSendMessageRequest request, ConstraintValidatorContext context) {
        ...
        try {
            ...
            context.buildConstraintViolationWithTemplate("Invalid message type: " + request.getType())
    ...
    private boolean validateTextMessage(AsyncSendMessageRequest request, ConstraintValidatorContext context) {
        if (request.getContent() == null || request.getContent().isBlank()) {
            context.buildConstraintViolationWithTemplate("Content is required for text messages")
    ...
    private boolean validateMediaMessage(AsyncSendMessageRequest request, ConstraintValidatorContext context,
        ...
        if (!hasFileUrl) {
            context.buildConstraintViolationWithTemplate(
                    "FileUrl is required for " + messageType.getValue()
        ...
        if (request.getFileName() == null || request.getFileName().isBlank()) {
            context.buildConstraintViolationWithTemplate("FileName is required for media messages")
