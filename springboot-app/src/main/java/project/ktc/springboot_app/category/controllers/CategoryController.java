...
class AdminCategoryController {
    ...
    @Operation(summary = "Get categories with pagination", description = "Retrieve categories with pagination and optional search functionality")
    @ApiResponses(value = {
    ...
    @GetMapping
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<Page<CategoryResponseDto>>> getAllCategories(
            @Parameter(description = "Search term for category name or description", example = "programming") @RequestParam(required = false) String search,
    ...
    @GetMapping("/{id}")
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<CategoryResponseDto>> getCategoryById(
    ...
    @PostMapping
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<CategoryResponseDto>> createCategory(
            @Parameter(description = "Category data", required = true) @Valid @RequestBody CategoryRequestDto requestDto) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<CategoryResponseDto>> updateCategory(
            @Parameter(description = "Category ID", required = true) @PathVariable String id,

            @Parameter(description = "Updated category data", required = true) @Valid @RequestBody CategoryRequestDto requestDto) {
    ...
    @Operation(summary = "Delete category", description = "Delete a category (only if it has no associated courses)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Category deleted successfully"),
            @ApiResponse(responseCode = "400", description = "Cannot delete category with associated courses"),
            @ApiResponse(responseCode = "404", description = "Category not found"),
            @ApiResponse(responseCode = "403", description = "Access denied - Admin role required")
    })
    @DeleteMapping("/{id}")
