...
/**
 * Cache service for instructor statistics operations Handles caching of instructor dashboard
 * statistics with appropriate TTL and provides clean separation between service DTOs and cache DTOs
 */
@Service
@Slf4j
...
public class InstructorStatisticsCacheService {
  ...
  public InsDashboardDto getInstructorStatistics(String instructorId) {
    try {
      ...
      InstructorStatisticsCacheDto cacheDto =
          (InstructorStatisticsCacheDto) redisTemplate.opsForValue().get(cacheKey);
  ...
  public void storeInstructorStatistics(String instructorId, InsDashboardDto instructorStatistics) {
    try {
      ...
      InstructorStatisticsCacheDto cacheDto =
          InstructorStatisticsCacheMapper.toCacheDto(instructorStatistics, instructorId);
