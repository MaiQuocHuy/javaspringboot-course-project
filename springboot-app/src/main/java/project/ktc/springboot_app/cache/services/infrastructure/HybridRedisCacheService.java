...
/**
 * Hybrid Redis cache service that tries direct Redis connection first, then
 * falls back to REST API
 * if direct connection fails.
 *
 * <p>
 * This provides resilience when TCP connections to Redis are blocked but HTTPS
 * REST API calls
 * work.
 *
 * @author KTC Team
	...
	public <T> List<T> getList(String key, Class<T> elementClass) {
		...
		try {
			...
			if (jsonValue != null) {
				...
				List<T> value = objectMapper.readValue(
						jsonValue,
