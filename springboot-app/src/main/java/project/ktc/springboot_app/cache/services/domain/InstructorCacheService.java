...
/**
 * Instructor-specific cache service that provides high-level caching operations for instructor
 * courses with two-tier caching strategy: - Base info: Less frequently changing data (5-10 min TTL)
...
public class InstructorCacheService {
  ...
  /**
   * Gets cached paginated instructor courses base info (less frequently changing data) Base info
   * includes: id, title, description, price, categories, thumbnailUrl, level, isApproved,
  ...
  public PaginatedResponse<InstructorCourseBaseCacheDto> getInstructorCoursesBaseInfo(
    ...
    try {
      String cacheKey =
          buildInstructorCoursesBaseCacheKey(
              instructorId,
              pageable,
              search,
              status,
      ...
      PaginatedResponse<InstructorCourseBaseCacheDto> cachedData =
          (PaginatedResponse<InstructorCourseBaseCacheDto>) cacheService.get(cacheKey);
  ...
  /**
   * Gets cached dynamic course info for multiple courses Dynamic info includes: enrollmentCount,
   * averageRating, revenue, sectionCount, lastContentUpdate, permissions (canEdit, canDelete, etc.)
   */
  ...
  private InstructorCourseDynamicCacheDto getSingleCourseDynamicInfo(String courseId) {
    try {
      ...
      InstructorCourseDynamicCacheDto cachedData =
          (InstructorCourseDynamicCacheDto) cacheService.get(cacheKey);
