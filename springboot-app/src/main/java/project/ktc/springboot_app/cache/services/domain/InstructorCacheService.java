 ...
 * for instructor courses with two-tier caching strategy:
 * - Base info: Less frequently changing data (5-10 min TTL)
...
public class InstructorCacheService {
     ...
     * data)
     * Base info includes: id, title, description, price, categories, thumbnailUrl,
     * level,
     * isApproved, isPublished, createdAt, updatedAt, status
     */
    ...
    public PaginatedResponse<InstructorCourseBaseCacheDto> getInstructorCoursesBaseInfo(
        ...
        try {
            String cacheKey = buildInstructorCoursesBaseCacheKey(instructorId, pageable, search, status,
                    categoryIds, minPrice, maxPrice, rating, level, isPublished);

            log.debug("Attempting to retrieve instructor courses base info from cache with key: {}", cacheKey);
            ...
            log.error("Failed to retrieve instructor courses base info from cache for instructor: {}",
                    instructorId, e);
            return null;
        }
    }
    ...
    public void storeInstructorCoursesBaseInfo(
        ...
        try {
            ...
            log.debug("Caching instructor courses base info with key: {} - {} courses",
                    cacheKey, courseData.getContent().size());

            cacheService.store(cacheKey, courseData, BASE_INFO_TTL);
            log.debug("Successfully cached instructor courses base info for instructor: {}", instructorId);
    ...
    /**
     * Gets cached dynamic course info for multiple courses
     * Dynamic info includes: enrollmentCount, averageRating, revenue, sectionCount,
    ...
    public void invalidateInstructorCoursesCache(String instructorId) {
        try {
            ...
            if (!keysToRemove.isEmpty()) {
                ...
                log.debug("Successfully invalidated {} instructor courses cache entries for instructor: {}",
                        removedCount, instructorId);
            } else {
                log.debug("No cache entries found to invalidate for instructor: {}", instructorId);
            }

        } catch (Exception e) {
            log.error("Failed to invalidate instructor courses cache for instructor: {}", instructorId, e);
        }
    }
