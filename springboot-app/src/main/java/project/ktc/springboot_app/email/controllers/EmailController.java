...
/**
 * REST Controller for Email Service Provides endpoints for sending emails with
 * various
 * configurations
 */
@RestController
...
public class EmailController {
	...
	public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<EmailSendResult>> sendEmailWithAttachment(
		...
		try {
			...
			EmailRequest.EmailRequestBuilder requestBuilder = EmailRequest.builder().to(List.of(to)).subject(subject)
					.htmlBody(content).async(async);

			// Add attachments if provided
			if (attachments != null && !attachments.isEmpty()) {
				List<EmailAttachment> emailAttachments = attachments.stream()
						.map(
								file -> EmailAttachment.builder()
