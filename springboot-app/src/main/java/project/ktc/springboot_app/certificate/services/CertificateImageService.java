...
/**
 * Service for generating image certificates using html-to-image API Converts
 * HTML templates to
 ...
 * dynamic data -
 * Uses external html-to-image API for professional image conversion - Supports
 * multiple image
 * formats (PNG, JPEG, WebP) - Comprehensive error handling and logging -
...
public class CertificateImageService {
	...
	private byte[] callHtmlToImageApi(String htmlContent, String format) throws IOException {
		try {
			...
			ResponseEntity<byte[]> response = restTemplate.exchange(imageApiUrl, HttpMethod.POST, requestEntity,
					byte[].class);
