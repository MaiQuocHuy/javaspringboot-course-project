...
public class AdminAffiliatePayoutServiceImp implements AdminAffiliatePayoutService {
	...
	public Page<AffiliatePayoutResponseDto> getAffiliatePayouts(
		...
		try {
			...
			List<AffiliatePayout> filteredPayouts = allPayouts.stream()
					...
					.filter(
							payout -> minAmount == null || payout.getCommissionAmount().compareTo(minAmount) >= 0)
					.filter(
							payout -> maxAmount == null || payout.getCommissionAmount().compareTo(maxAmount) <= 0)
					.collect(Collectors.toList());

			// Convert to DTOs
			List<AffiliatePayoutResponseDto> payoutDtos = filteredPayouts.stream().map(this::convertToResponseDto)
					.collect(Collectors.toList());
	...
	public AffiliatePayoutResponseDto getPayoutById(Long id) {
		AffiliatePayout payout = affiliatePayoutRepository
				.findById(id.toString())
				.orElseThrow(
						() -> new ResourceNotFoundException("Affiliate payout not found with id: " + id));
	...
	private AffiliatePayoutResponseDto convertToResponseDto(AffiliatePayout payout) {
		return AffiliatePayoutResponseDto.builder()
				...
				.referredByUser(
						payout.getReferredByUser() != null
								? AffiliatePayoutResponseDto.ReferredByUserDto.builder()
								...
								: null)
				.course(
						payout.getCourse() != null
								? AffiliatePayoutResponseDto.CourseDto.builder()
										.id(payout.getCourse().getId())
										.name(payout.getCourse().getTitle())
										.build()
								: null)
				.discountUsage(
						payout.getDiscountUsage() != null
								? AffiliatePayoutResponseDto.DiscountUsageDto.builder()
										.id(payout.getDiscountUsage().getId())
										.discount(
												AffiliatePayoutResponseDto.DiscountUsageDto.DiscountDto.builder()
														.code(
																payout.getDiscountUsage().getDiscount() != null
																		? payout.getDiscountUsage().getDiscount()
																				.getCode()
																		: null)
														.type(
																payout.getDiscountUsage().getDiscount() != null
	...
	private AffiliatePayoutDetailResponseDto convertToDetailResponseDto(AffiliatePayout payout) {
		return AffiliatePayoutDetailResponseDto.builder()
				...
				.cancelledAt(payout.getCancelledAt())
				.referrer(
						AffiliatePayoutDetailResponseDto.ReferrerInfo.builder()
								.id(payout.getReferredByUser().getId())
								.name(payout.getReferredByUser().getName())
								.email(payout.getReferredByUser().getEmail())
				...
				.course(
						AffiliatePayoutDetailResponseDto.CourseInfo.builder()
								...
								.build())
				.discount(
						payout.getDiscountUsage() != null && payout.getDiscountUsage().getDiscount() != null
								...
								: null)
				.purchaser(
						payout.getDiscountUsage() != null && payout.getDiscountUsage().getUser() != null
				...
				.usageInfo(
						payout.getDiscountUsage() != null
								? AffiliatePayoutDetailResponseDto.UsageInfo.builder()
										.id(payout.getDiscountUsage().getId())
										...
										.finalPrice(
												payout
														.getCourse()
														.getPrice()
														.subtract(payout.getDiscountUsage().getDiscountAmount()))
										.build()
								: null)
				.build();
	}
	...
	private byte[] generateCsvBytes(List<AffiliatePayout> payouts) throws IOException {
		...
		// Write CSV header
		writer.write(
				"ID,User ID,Course ID,Course Title,Commission %,Commission Amount,Status,Paid At,Created At\n");
