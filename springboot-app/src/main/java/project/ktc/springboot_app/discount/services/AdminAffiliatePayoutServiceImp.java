...
public class AdminAffiliatePayoutServiceImp implements AdminAffiliatePayoutService {
  ...
  public Page<AffiliatePayoutResponseDto> getAffiliatePayouts(
    ...
    try {
      ...
      // Apply filters
      List<AffiliatePayout> filteredPayouts =
          allPayouts.stream()
              .filter(
                  payout ->
                      userId == null
              ...
              .filter(
                  payout ->
                      minAmount == null || payout.getCommissionAmount().compareTo(minAmount) >= 0)
              .filter(
                  payout ->
                      maxAmount == null || payout.getCommissionAmount().compareTo(maxAmount) <= 0)
              .collect(Collectors.toList());

      // Convert to DTOs
      List<AffiliatePayoutResponseDto> payoutDtos =
          filteredPayouts.stream().map(this::convertToResponseDto).collect(Collectors.toList());
