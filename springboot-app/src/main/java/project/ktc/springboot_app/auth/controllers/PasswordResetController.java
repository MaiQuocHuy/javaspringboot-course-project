 ...
 *
 * <p>This controller provides endpoints for: - Initiating password reset with email/OTP -
 * Confirming password reset with OTP and new password
 *
 * <p>Features: - Rate limiting and abuse prevention - Secure OTP generation and validation -
 * Comprehensive error handling - Input validation and sanitization - Audit logging for security
 *
 * @author KTC Team
 * @version 1.0
 * @since 2025-01-26
...
public class PasswordResetController {
   ...
   *
   * <p>This endpoint: - Validates the user's email address - Checks rate limiting to prevent abuse
   * - Generates a secure OTP code - Sends email with password reset instructions - Returns masked
   * email and expiry information
   *
   * <p>Rate Limiting: - Maximum 3 requests per hour per email - Minimum 5 minutes between requests
   *
   * <p>Security Features: - Email address validation - Rate limiting per user - Secure OTP
   * generation - Token expiration (15 minutes default) - Previous token invalidation
   *
  ...
  @Operation(
      ...
      description =
          "Send OTP to user's email for password reset process. Subject to rate limiting.")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<
          project.ktc.springboot_app.common.dto.ApiResponse<ForgotPasswordResponseDto>>
      initiateForgotPassword(
          @Parameter(description = "Forgot password request with user email", required = true)
              @Valid
              @RequestBody
   ...
   *
   * <p>This endpoint: - Validates the OTP code and user email - Checks password strength
   * requirements - Verifies password confirmation match - Updates user's password with encryption -
   * Marks the OTP token as used - Logs the successful password change
   *
   * <p>Security Features: - OTP verification with attempt limiting - Password strength validation -
   * Password confirmation matching - Token invalidation after use - Secure password hashing -
   * Account activity logging
   *
   * <p>OTP Validation: - Maximum 3 verification attempts per token - Token expires after 15 minutes
   * (default) - One-time use only - User-specific validation
   *
   * @param request the reset password confirmation request with OTP and new password
  ...
  @Operation(
      ...
      description =
          "Verify OTP and set new password. OTP has limited attempts and expiration time.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Password reset successfully"),
        @ApiResponse(
            responseCode = "400",
            description = "Invalid request - validation errors or password mismatch"),
        @ApiResponse(responseCode = "401", description = "Invalid or expired OTP code"),
        @ApiResponse(responseCode = "404", description = "User not found with provided email"),
        @ApiResponse(
            responseCode = "410",
            description = "OTP expired or maximum attempts exceeded"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<Void>>
      confirmPasswordReset(
          @Parameter(
                  description = "Password reset confirmation with OTP and new password",
                  required = true)
              @Valid
              @RequestBody
              ResetPasswordConfirmDto request) {
  ...
  /**
   * Health check endpoint for password reset service. Used for monitoring and ensuring the service
   * is operational.
   *
  ...
  @Operation(
      summary = "Password reset service health check",
      description = "Check if password reset service is operational")
