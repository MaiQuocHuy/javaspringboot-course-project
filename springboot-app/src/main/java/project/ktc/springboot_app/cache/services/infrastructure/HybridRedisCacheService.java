...
/**
 * Hybrid Redis cache service that tries direct Redis connection first, then falls back to REST API
 * if direct connection fails.
 *
 * <p>This provides resilience when TCP connections to Redis are blocked but HTTPS REST API calls
 * work.
 *
 * @author KTC Team
 */
...
public class HybridRedisCacheService implements CacheService {
  ...
  public void store(String key, Object value) {
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for store, falling back to REST API: {}",
            e.getMessage());
  ...
  public void store(String key, Object value, Duration timeout) {
    if (tryDirectConnection()) {
      try {
        ...
        log.debug(
            "Stored data in cache via direct connection with key: {} and TTL: {} seconds",
            key,
            timeout.toSeconds());
        markDirectConnectionWorking();
        return;
        ...
        log.warn(
            "Direct Redis connection failed for store with TTL, falling back to REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
    ...
    try {
      ...
      log.debug(
          "Stored data in cache via REST API with key: {} and TTL: {} seconds",
          key,
          timeout.toSeconds());
    } catch (Exception e) {
      log.error(
          "Error storing data in cache with key: {} and TTL: {} via both direct and REST",
          key,
          timeout,
          e);
    }
  ...
  public Object get(String key) {
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for get, falling back to REST API: {}", e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  public <T> T get(String key, Class<T> clazz) {
    if (tryDirectConnection()) {
      try {
        ...
        if (value != null && clazz.isInstance(value)) {
          log.debug(
              "Cache hit via direct connection for key: {} with type: {}",
              key,
              clazz.getSimpleName());
          markDirectConnectionWorking();
          return (T) value;
          ...
          log.warn(
              "Cache value type mismatch via direct connection for key: {}. Expected: {}, Got: {}",
              key,
              clazz.getSimpleName(),
        ...
        log.warn(
            "Direct Redis connection failed for typed get, falling back to REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
    ...
    try {
      ...
      log.error(
          "Error retrieving data from cache with key: {} and type: {} via both direct and REST",
          key,
          clazz.getSimpleName(),
          e);
      return null;
  ...
  public <T> List<T> getList(String key, Class<T> elementClass) {
    if (tryDirectConnection()) {
      try {
        ...
        if (value != null && value instanceof List) {
          log.debug(
              "Cache hit via direct connection for list key: {} with element type: {}",
              key,
          ...
          log.warn(
              "Cache value type mismatch via direct connection for key: {}. Expected: List<{}>, Got: {}",
              key,
              elementClass.getSimpleName(),
              value.getClass().getSimpleName());
        } else {
        ...
        log.warn(
            "Direct Redis connection failed for list get, falling back to REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
    ...
    try {
      ...
      if (jsonValue != null) {
        ...
        log.debug(
            "Cache hit via REST API for list key: {} with element type: {}",
            key,
            elementClass.getSimpleName());
        return value;
      } else {
      ...
      log.error(
          "Error retrieving list from cache with key: {} and element type: {} via both direct and REST",
          key,
          elementClass.getSimpleName(),
          e);
      return null;
  ...
  public boolean exists(String key) {
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for exists check, falling back to REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  public boolean expire(String key, Duration timeout) {
    if (tryDirectConnection()) {
      try {
        ...
        log.debug(
            "Set expiration via direct connection for key: {} to {} seconds, result: {}",
            key,
            timeout.toSeconds(),
            success);
        markDirectConnectionWorking();
        ...
        log.warn(
            "Direct Redis connection failed for expire, falling back to REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  public Set<String> getKeys(String pattern) {
    if (tryDirectConnection()) {
      try {
        ...
        log.debug(
            "Retrieved {} keys via direct connection with pattern: {}",
  ...
  public long remove(Set<String> keys) {
    ...
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for batch removal, trying REST API: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  public long getTtl(String key) {
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for TTL check, REST API doesn't support TTL operations: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  public void clear() {
    if (tryDirectConnection()) {
      try {
        ...
        log.warn(
            "Direct Redis connection failed for clear operation, REST API doesn't support clear all: {}",
            e.getMessage());
        markDirectConnectionFailed();
      }
    }
  ...
  private void initializeConnection() {
    ...
    try {
      ...
      log.warn(
          "Direct Redis connection failed at startup, will use REST API fallback: {}",
          e.getMessage());
