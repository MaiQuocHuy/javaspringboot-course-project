...
public interface DiscountService {
	...
	/**
	 * Create a user-specific induction discount (REFERRAL type) Each user can only
	 * create one
	 * REFERRAL discount
	 *
	 * @param userId
	 *            The user ID to create induction discount for
	 * @return ResponseEntity with created discount information
	 */
	ResponseEntity<ApiResponse<DiscountResponseDto>> createUserInductionDiscount(String userId);

	/**
	 * Get user's induction discount (REFERRAL type) Returns the user's personal
	 * REFERRAL discount if
	 * it exists
	 *
	 * @param userId
	 *            The user ID to get induction discount for
	...
	/**
	 * Get available public discount codes (GENERAL type) Returns all active GENERAL
	 * discounts that
