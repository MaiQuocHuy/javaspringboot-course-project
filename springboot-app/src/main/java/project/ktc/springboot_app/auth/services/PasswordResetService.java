 ...
 *
 * <p>
 * This service manages the complete password reset flow including: - OTP
 * generation and email
 * sending - OTP verification and validation - Password update with security
 * measures - Rate
 * limiting and abuse prevention
 *
 * <p>
 * Security features: - Rate limiting to prevent spam - Secure OTP generation -
 * Token expiration
 * and attempt limits - Previous token invalidation - Input validation and
 * sanitization
 *
 * @author KTC Team
 * @version 1.0
...
public class PasswordResetService {
	 ...
	 *
	 * @param request
	 *            the reset password confirmation request
	 * @throws ResourceNotFoundException
	 *             if user or token not found
	 * @throws ValidationException
	 *             if OTP is invalid or passwords don't match
	 * @throws BusinessLogicException
	 *             if token is expired or max attempts reached
	 */
	@Transactional
	public void confirmPasswordReset(ResetPasswordConfirmDto request) {
		...
		// Find valid token
		LocalDateTime now = LocalDateTime.now();
		Optional<PasswordResetToken> tokenOpt = passwordResetTokenRepository.findValidTokenByUserIdAndOtpCode(
	...
	/**
	 * Cleanup method to remove expired tokens (for scheduled execution). Should be
	 * called
