...
public interface SystemLogRepository extends JpaRepository<SystemLog, Long> {
  ...
  @Query(
      "SELECT sl FROM SystemLog sl WHERE sl.createdAt BETWEEN :startDate AND :endDate ORDER BY sl.createdAt DESC")
  List<SystemLog> findByDateRange(
      @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

  /** Find logs by multiple criteria with pagination */
  @Query(
      "SELECT sl FROM SystemLog sl WHERE "
  ...
  /** Find logs by action, entity type and date range Used for scheduled audit reports */
  @Query(
      "SELECT sl FROM SystemLog sl WHERE sl.action = :action AND sl.entityType = :entityType AND sl.createdAt BETWEEN :startDate AND :endDate ORDER BY sl.createdAt DESC")
