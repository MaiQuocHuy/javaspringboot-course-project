...
/**
 * Service for generating PDF certificates using html-to-image API Converts HTML templates to
 * high-quality PDF certificates
 *
 * <p>Features: - Loads HTML template from resources - Replaces placeholders with dynamic data -
 * Uses external html-to-image API for professional PDF conversion - Comprehensive error handling
 * and logging
 */
@Service
@Slf4j
public class CertificatePdfService {
  ...
  /**
   * Alternative method - delegates to main API method Maintained for backward compatibility
   *
   * @param certificateData Certificate information
   * @return PDF as byte array
   * @throws IOException if PDF generation fails
  ...
  private byte[] callHtmlToPdfApi(String htmlContent) throws IOException {
    try {
      ...
      ResponseEntity<byte[]> response =
          restTemplate.exchange(pdfApiUrl, HttpMethod.POST, requestEntity, byte[].class);
