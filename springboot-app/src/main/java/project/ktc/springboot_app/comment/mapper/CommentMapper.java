...
public class CommentMapper {
	 ...
	 *            Whether to include nested replies
	...
	public CommentResponse toCommentResponseWithReplies(Comment comment, boolean includeReplies) {
		...
		if (includeReplies && comment.getReplies() != null && !comment.getReplies().isEmpty()) {
			List<CommentResponse> replyResponses = comment.getReplies().stream()
					.map(
							reply -> toCommentResponseWithReplies(reply, true)) // Recursively include replies
	...
	/**
	 * Build comment tree structure from flat list of comments Groups replies under
	 * their parent
	 * comments
	 *
	 * @param comments
	 *            Flat list of comments and replies
	...
	public List<CommentResponse> buildCommentTree(List<Comment> comments) {
		...
		List<Comment> rootComments = comments.stream().filter(Comment::isRootComment).collect(Collectors.toList());

		List<Comment> replies = comments.stream().filter(Comment::isReply).collect(Collectors.toList());

		// Convert root comments to DTOs
		List<CommentResponse> rootResponses = rootComments.stream().map(this::toCommentResponse)
				.collect(Collectors.toList());
