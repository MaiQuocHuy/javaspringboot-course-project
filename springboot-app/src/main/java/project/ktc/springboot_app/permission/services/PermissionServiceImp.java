...
public class PermissionServiceImp implements PermissionService {
	...
	public List<RolePermission> updatePermissionsForRole(
		...
		for (PermissionUpdateRequest.PermissionUpdateItemDto item : request.getPermissions()) {
			...
			if (filterTypeId == null || filterTypeId.trim().isEmpty()) {
				filterType = filterTypeRepository
						.getAllFilterType()
						.orElseThrow(
	...
	private ResourceDto buildResourceDto(
		...
		List<ResourceDto> children = parentChildMap.getOrDefault(resource.getId(), List.of()).stream()
				.map(
						child -> buildResourceDto(
		...
		List<AvailablePermissionDto> availablePermissions = allPermissions.stream()
				.map(
						permission -> {
							...
							boolean isRestricted = permissionRoleAssignRuleRepository
									.hasAssignmentRules(permission.getId());

							// Get allowed roles for this permission
							List<String> allowedRoles = permissionRoleAssignRuleRepository
									.findAllowedRoleNamesByPermissionId(
											permission.getId());
							...
							if (!isRestricted) {
								...
								canAssignToRole = allowedRoles
										.contains(userRoleRepository.findById(roleId).get().getRole());
							}
	...
	public project.ktc.springboot_app.permission.dto.RolePermissionGroupedDto getPermissionsGroupedByResourceForRole(
		...
		Map<String, List<project.ktc.springboot_app.permission.dto.RolePermissionDetailDto>> resourceGroups = allPermissions
				.stream()
				.filter(
						permission -> !restrictedResources.contains(permission.getResourceName())) // Filter out
				// restricted
				// resources
				.map(
						permission -> {
				...
				.collect(
						Collectors.groupingBy(
								project.ktc.springboot_app.permission.dto.RolePermissionDetailDto::getResource));
