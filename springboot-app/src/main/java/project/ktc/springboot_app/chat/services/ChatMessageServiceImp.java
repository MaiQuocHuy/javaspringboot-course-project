...
public class ChatMessageServiceImp implements ChatMessageService {
  ...
  public ResponseEntity<ApiResponse<ChatMessageResponse>> sendMessage(
      String courseId, String senderEmail, SendMessageRequest request) {
    try {
      var course =
          courseRepository
              .findById(courseId)
      ...
      var type =
          chatMessageTypeRepository
              .findByNameIgnoreCase(request.getType())
      ...
      switch (type.getName().toUpperCase()) {
        case "TEXT" -> {
          ChatMessageText text =
              ChatMessageText.builder().message(message).content(request.getContent()).build();
          message.setTextDetail(text);
        }
        case "FILE" -> {
          ChatMessageFile file =
              ChatMessageFile.builder()
                  .message(message)
        ...
        case "AUDIO" -> {
          ChatMessageAudio audio =
              ChatMessageAudio.builder()
                  .message(message)
                  .audioUrl(request.getContent())
                  .fileName(request.getFileName())
                  .fileSize(request.getFileSize())
                  .duration(
                      request.getDuration() != null ? request.getDuration().longValue() : null)
                  .mimeType(null)
                  .thumbnailUrl(null)
                  .build();
        ...
        case "VIDEO" -> {
          ChatMessageVideo video =
              ChatMessageVideo.builder()
                  .message(message)
  ...
  public ResponseEntity<ApiResponse<PaginatedResponse<ChatMessageResponse>>> listMessages(
      String courseId, String type, Pageable pageable) {
    try {
      ...
      if (type == null || type.isBlank()) {
        ...
        messagesPage =
            chatMessageRepository.findByCourseIdAndType(courseId, type.toUpperCase(), pageable);
      }

      List<ChatMessageResponse> messageDtos =
          messagesPage.getContent().stream().map(this::toResponse).collect(Collectors.toList());

      // Create paginated response
      PaginatedResponse<ChatMessageResponse> paginatedResponse =
          PaginatedResponse.<ChatMessageResponse>builder()
              .content(messageDtos)
              .page(
  ...
  private ChatMessageResponse toResponse(ChatMessage m) {
    return ChatMessageResponse.builder()
        ...
        .audioDuration(
            m.getAudioDetail() != null && m.getAudioDetail().getDuration() != null
        ...
        .videoDuration(
            m.getVideoDetail() != null && m.getVideoDetail().getDuration() != null
  ...
  @Override
  @Transactional(readOnly = true)
  public ResponseEntity<ApiResponse<ChatMessagesListResponse>> getMessages(
      String courseId,
      String userEmail,
      Integer page,
      Integer size,
    ...
    try {
      ...
      if (size < 1 || size > 100) {
        return ApiResponseUtil.badRequest(
            "Invalid query parameters: size must be between 1 and 100");
      }
      ...
      if (beforeMessageId != null || afterMessageId != null) {
        ...
        if (beforeMessageId != null) {
          ...
          if (beforeMessageUuid == null) {
            return ApiResponseUtil.badRequest(
                "Invalid query parameters: beforeMessageId not found");
          }
          messages =
              chatMessageRepository.findByCourseIdBeforeMessageId(
                  courseId, beforeMessageUuid, pageable);
        } else {
          ...
          messages =
              chatMessageRepository.findByCourseIdAfterMessageId(
                  courseId, afterMessageUuid, pageable);
        }

        // For infinite scroll, return simple message list
        List<SimpleChatMessageResponse> messageList =
            messages.stream().map(this::toSimpleResponse).collect(Collectors.toList());

        ChatMessagesListResponse response =
            ChatMessagesListResponse.builder()
  ...
  public ResponseEntity<ApiResponse<Void>> deleteMessage(
      String courseId, String messageId, String userEmail) {
    try {
      // Validate input parameters
  ...
  public ResponseEntity<ApiResponse<ChatMessageResponse>> updateMessage(
      String courseId, String messageId, String userEmail, UpdateMessageRequest request) {
    try {
      // Validate input parameters
      if (courseId == null || courseId.isBlank()) {
        return ApiResponseUtil.badRequest("Invalid courseId format");
  ...
  @Override
  public ResponseEntity<ApiResponse<AsyncMessageAcknowledgment>> sendMessageAsync(
      String courseId, String senderEmail, AsyncSendMessageRequest request) {
    try {
      // Validate input parameters
      if (courseId == null || courseId.isBlank()) {
        return ApiResponseUtil.badRequest("Invalid courseId format");
      ...
      AsyncMessageAcknowledgment acknowledgment =
          AsyncMessageAcknowledgment.builder()
      ...
      return ResponseEntity.status(org.springframework.http.HttpStatus.ACCEPTED)
          .body(
              ApiResponse.<AsyncMessageAcknowledgment>builder()
                  .statusCode(202)
      ...
      return ApiResponseUtil.internalServerError(
          "Failed to process message request. Please try again later.");
    }
  }
  ...
  public void processMessageAsync(
    ...
    try {
      ...
      broadcastStatusUpdate(
          courseId, null, null, null, null, request.getTempId(), "PENDING", null, null, null);
  ...
  private String createPendingMessage(
      String courseId,
      String senderEmail,
    ...
    var type =
        chatMessageTypeRepository
            .findByNameIgnoreCase(messageType.getValue())
            .orElseThrow(() -> new NoSuchElementException("Invalid message type"));
  ...
  private void processTextMessage(
      String courseId, String senderEmail, AsyncSendMessageRequest request, String messageId) {
    try {
      ...
      broadcastStatusUpdate(
          courseId,
          response.getContent(),
          response.getSenderRole(),
          response.getSenderName(),
          response.getSenderThumbnailUrl(),
          request.getTempId(),
          "SENT",
          messageId,
          null,
  ...
  private void processMediaMessage(
    ...
    try {
      ...
      updateMessageWithMediaDetails(
          messageId, messageType, fileUrl, thumbnailUrl, duration, mimeType, resolution, request);
  ...
  private void updateMessageWithMediaDetails(
      String messageId,
      MessageType messageType,
      String fileUrl,
      String thumbnailUrl,
  ...
  private void broadcastStatusUpdate(
      ...
      String fileUrl,
      String error) {
    AsyncMessageStatusEvent.AsyncMessageStatusEventBuilder eventBuilder =
        AsyncMessageStatusEvent.builder().tempId(tempId).status(status);
