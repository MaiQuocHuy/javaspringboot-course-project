...
public class FileValidationService {
	...
	private static final List<String> ALLOWED_IMAGE_MIME_TYPES = List.of("image/jpeg", "image/png", "image/gif",
			"image/bmp", "image/jpg", "image/webp");
	...
	public void validateImageFile(MultipartFile file) {
		...
		log.debug(
				"Image file validation passed for: {} ({})",
	 ...
	 * @param file
	 *            MultipartFile to validate
	 * @throws InvalidVideoFormatException
	 *             if validation fails
	 */
	public void validateVideoFile(MultipartFile file) {
		...
		log.debug(
				"Video file validation passed for: {} ({})",
				file.getOriginalFilename(),
				file.getContentType());
	}
	...
	private void validateImageFileSize(MultipartFile file) {
		...
		if (file.getSize() > MAX_IMAGE_SIZE) {
			throw new InvalidImageFormatException(
					String.format(
							"Image file size (%d MB) exceeds maximum allowed size of %d MB",
	...
	private void validateVideoFileSize(MultipartFile file) {
		...
		if (file.getSize() > MAX_VIDEO_SIZE) {
			throw new InvalidVideoFormatException(
					String.format(
							"Video file size (%d MB) exceeds maximum allowed size of %d MB",
	...
	private void validateImageMimeType(MultipartFile file) {
		...
		if (!ALLOWED_IMAGE_MIME_TYPES.contains(contentType.toLowerCase())) {
			throw new InvalidImageFormatException(
					String.format(
							"Invalid image format '%s'. Allowed formats: %s",
	...
	private void validateVideoMimeType(MultipartFile file) {
		...
		if (!ALLOWED_VIDEO_MIME_TYPES.contains(contentType.toLowerCase())) {
			throw new InvalidVideoFormatException(
					String.format(
							"Invalid video format '%s'. Allowed formats: %s",
	...
	public void validateDocumentFile(MultipartFile file) {
		...
		if (file.getSize() > MAX_DOCUMENT_SIZE) {
			throw new InvalidDocumentFormatException(
					String.format(
							"Document file size exceeds maximum allowed size of %d MB. Current size: %.2f MB",
							MAX_DOCUMENT_SIZE / (1024 * 1024), (double) file.getSize() / (1024 * 1024)));
		}
		...
		if (contentType == null || !ALLOWED_DOCUMENT_MIME_TYPES.contains(contentType.toLowerCase())) {
			throw new InvalidDocumentFormatException(
					String.format(
							"Unsupported document format: %s. Allowed formats: %s",
							contentType, String.join(", ", ALLOWED_DOCUMENT_MIME_TYPES)));
		}
