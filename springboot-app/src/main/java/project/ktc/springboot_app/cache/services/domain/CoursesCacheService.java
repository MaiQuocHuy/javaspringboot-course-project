...
/**
 * Course-specific cache service that provides high-level caching operations for courses. Handles
 * course listing, details, and invalidation using the underlying cache service.
 *
 * @author KTC Team
 */
@Service
...
public class CoursesCacheService {
  ...
  public void storeCoursesList(
    ...
    try {
      String cacheKey =
          cacheKeyBuilder.buildCoursesListKey(
  ...
  public void storeSharedCoursesData(
      Pageable pageable,
      String categoryId,
      String search,
      CourseLevel level,
    ...
    try {
      String cacheKey =
          cacheKeyBuilder.buildSharedCoursesKey(
              pageable.getPageNumber(),
              pageable.getPageSize(),
              search,
              categoryId,
  ...
  public SharedCourseCacheDto getSharedCoursesData(
      Pageable pageable,
      String categoryId,
      String search,
      CourseLevel level,
  ...
  public void storeSharedCourseData(
      int page,
      int size,
      String search,
      String categoryId,
  ...
  public void storeUserEnrollmentStatus(
      String userId, String courseIdsKey, java.util.Map<String, Boolean> enrollmentStatus) {
    try {
  ...
  @SuppressWarnings("unchecked")
  public java.util.Map<String, Boolean> getUserEnrollmentStatus(
      String userId, String courseIdsKey) {
    try {
      String cacheKey = cacheKeyBuilder.buildUserEnrollmentStatusKey(userId, courseIdsKey);
  ...
  public void invalidateCourseByIdAndSlug(String courseId, String slug) {
    try {
      ...
      log.error(
          "Failed to invalidate course cache for courseId: {} and slug: {}", courseId, slug, e);
    }
  }

  /** Invalidates cache when enrollment changes occur (always refresh for consistency) */
  public void checkAndInvalidateForEnrollmentChange(
      String courseId, String slug, long previousCount, long newCount) {
    try {
      // Always invalidate cache when enrollment count changes
      if (previousCount != newCount) {
        log.info(
            "Enrollment change detected for course {} (from {} to {}). Invalidating cache.",
            courseId,
            previousCount,
            newCount);
        ...
        log.info(
            "Successfully invalidated all course caches for enrollment change in course: {}",
            courseId);
      } else {
  ...
  /**
   * Invalidates user-specific enrollment status cache This ensures that enrollment status is
   * immediately updated after purchase
   */
  public void invalidateUserEnrollmentStatus(String userId) {
    try {
      ...
      if (!keysToDelete.isEmpty()) {
        ...
        log.debug(
            "Invalidated {} user enrollment status cache entries for user: {}",
            deletedCount,
            userId);
      } else {
        log.debug(
            "No user enrollment status cache entries found to invalidate for user: {}", userId);
      }
  ...
  /**
   * Invalidates all course-related cache after enrollment change This ensures consistent enrollment
   * counts across all course APIs
   */
  public void invalidateCacheForEnrollmentChange(String courseId, String slug) {
    try {
      ...
      log.info(
          "Successfully invalidated all course caches after enrollment change for course: {}",
          courseId);
