...
/**
 * Implementation of AffiliatePayoutService for handling referral commissions
 * Processes payouts asynchronously to avoid blocking payment processing
 */
@Service
@Slf4j
...
public class AffiliatePayoutServiceImp implements AffiliatePayoutService {
    ...
    @Async("taskExecutor")
    @Transactional
    public CompletableFuture<AffiliatePayout> createPayoutAsync(DiscountUsage discountUsage, BigDecimal finalPrice,
            String paymentId) {
        log.info("Starting async affiliate payout creation for discount usage: {}", discountUsage.getId());

        try {
            AffiliatePayout payout = createPayout(discountUsage, finalPrice, paymentId);
            log.info("✅ Async affiliate payout created successfully: {} for amount: {}",
                    payout.getId(), payout.getCommissionAmount());
            return CompletableFuture.completedFuture(payout);
        } catch (Exception e) {
            log.error("❌ Failed to create async affiliate payout for discount usage: {} - {}",
                    discountUsage.getId(), e.getMessage(), e);
    ...
    @Override
    @Transactional
    public AffiliatePayout createPayout(DiscountUsage discountUsage, BigDecimal finalPrice, String paymentId) {
        log.info("Creating affiliate payout for discount usage: {} with final price: {}",
                discountUsage.getId(), finalPrice);

        // Validate that this is a referral discount
        if (discountUsage.getDiscount().getType() != DiscountType.REFERRAL) {
            throw new IllegalArgumentException("Affiliate payout can only be created for REFERRAL type discounts");
        }
        ...
        AffiliatePayout affiliatePayout = AffiliatePayout.builder()
        ...
        log.info("🎉 Affiliate payout created successfully: {} for referrer: {} with commission: ${}",
                savedPayout.getId(),
    ...
    public BigDecimal calculateCommissionAmount(BigDecimal finalPrice) {
        ...
        log.debug("Commission calculation: finalPrice={}, percent={}%, amount={}",
