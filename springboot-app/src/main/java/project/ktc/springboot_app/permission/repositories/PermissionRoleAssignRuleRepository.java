...
/**
 * Repository interface for Permission Role Assign Rule entity
 * Handles database operations for permission-role assignment rules
 */
@Repository
public interface PermissionRoleAssignRuleRepository extends JpaRepository<PermissionRoleAssignRule, String> {

    /**
     * Find all assignable permission IDs for a specific role
     * Returns only active rules where permissions can be assigned to the role
     * 
     * @param roleId the role ID to check
     * @return Set of permission IDs that can be assigned to this role
     */
    @Query("SELECT DISTINCT p.permission.id FROM PermissionRoleAssignRule p " +
            "WHERE p.role.id = :roleId AND p.isActive = true")
    Set<String> findAssignablePermissionIdsByRoleId(@Param("roleId") String roleId);

    /**
     * Find all assignable permission keys for a specific role
     * Returns only active rules where permissions can be assigned to the role
     * 
     * @param roleId the role ID to check
     * @return Set of permission keys that can be assigned to this role
     */
    @Query("SELECT DISTINCT p.permission.permissionKey FROM PermissionRoleAssignRule p " +
            "WHERE p.role.id = :roleId AND p.isActive = true")
     ...
     */
    @Query("SELECT COUNT(p) > 0 FROM PermissionRoleAssignRule p " +
            "WHERE p.role.id = :roleId AND p.permission.id = :permissionId AND p.isActive = true")
    boolean canAssignPermissionToRole(@Param("roleId") String roleId, @Param("permissionId") String permissionId);
     ...
     */
    @Query("SELECT p FROM PermissionRoleAssignRule p " +
    ...
    List<PermissionRoleAssignRule> findRolesByPermissionId(@Param("permissionId") String permissionId);
     ...
     */
    @Query("SELECT COUNT(DISTINCT p.permission.id) FROM PermissionRoleAssignRule p " +
            "WHERE p.role.id = :roleId AND p.isActive = true")
    ...
    PermissionRoleAssignRule findByRoleIdAndPermissionId(@Param("roleId") String roleId,
     ...
     */
    @Query("SELECT p.role.role FROM PermissionRoleAssignRule p " +
            "WHERE p.permission.id = :permissionId AND p.isActive = true")
