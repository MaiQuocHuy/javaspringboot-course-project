...
public class QuizQuestionValidator implements ConstraintValidator<ValidQuizQuestion, Object> {
    ...
    public boolean isValid(Object question, ConstraintValidatorContext context) {
        ...
        try {
            ...
            context.buildConstraintViolationWithTemplate("Unable to validate quiz question structure")
        ...
        if (questionText == null || questionText.trim().isEmpty()) {
            context.buildConstraintViolationWithTemplate("Question text is required")
        ...
        if (options == null || options.size() != 4) {
            context.buildConstraintViolationWithTemplate("Must have exactly 4 options (A, B, C, D)")
            ...
            if (!options.keySet().equals(requiredKeys)) {
                context.buildConstraintViolationWithTemplate("Options must be exactly A, B, C, D")
                        .addPropertyNode("options")
                        .addConstraintViolation();
                isValid = false;
            }
            ...
            for (Map.Entry<String, String> entry : options.entrySet()) {
                if (entry.getValue() == null || entry.getValue().trim().isEmpty()) {
                    context.buildConstraintViolationWithTemplate("Option " + entry.getKey() + " cannot be empty")
                            .addPropertyNode("options")
                            .addConstraintViolation();
                    isValid = false;
                }
        ...
        if (correctAnswer == null || correctAnswer.trim().isEmpty()) {
            context.buildConstraintViolationWithTemplate("Correct answer is required")
            ...
            context.buildConstraintViolationWithTemplate("Correct answer must be A, B, C, or D")
                    .addPropertyNode("correctAnswer")
                    .addConstraintViolation();
            isValid = false;
        }
