...
@Tag(
    ...
    description = "Endpoints for instructor application management")
...
public class InstructorApplicationController {
  ...
  @Operation(
      summary = "Upload instructor application documents",
      description =
          """
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "400",
            description =
                "Validation failed - invalid files, ineligible user, or missing required fields",
  ...
  public ResponseEntity<
          project.ktc.springboot_app.common.dto.ApiResponse<DocumentUploadResponseDto>>
      uploadDocuments(
          @Parameter(
                  description = "Professional certification file (PDF, DOCX, PNG, JPG - max 15MB)",
                  required = true)
              @RequestParam("certificate")
              MultipartFile certificate,
          @Parameter(description = "Link to GitHub, LinkedIn, or demo portfolio", required = true)
              @RequestParam("portfolio")
              String portfolio,
          @Parameter(description = "Resume/CV file (PDF, DOCX, JPG - max 15MB)", required = true)
              @RequestParam("cv")
              MultipartFile cv,
          @Parameter(
                  description = "Additional supporting documents (e.g., awards, projects, etc.)",
                  required = false)
  ...
  @Operation(
      summary = "Upload instructor application documents",
      description = "Get the latest instructor application details by user ID.")
  public ResponseEntity<
          project.ktc.springboot_app.common.dto.ApiResponse<InstructorApplicationDetailResponseDto>>
      getApplicationByUserId(@PathVariable String userId) {
