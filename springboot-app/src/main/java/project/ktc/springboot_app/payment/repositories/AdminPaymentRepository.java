...
public interface AdminPaymentRepository extends JpaRepository<Payment, String> {
  ...
  /**
   * Get all payments with pagination support for admin Orders by creation date descending (most
   * recent first) Includes user and course information
   */
  @Query(
  ...
  /**
   * Get payment by ID with full details including user and course for admin Returns payment with
  ...
  /**
   * Get payment by ID with refunds for payout processing Eager loads refunds to avoid
  ...
  /**
   * Get payments filtered by status with pagination Useful for admin to filter payments by their
   * status
   */
  @Query(
      "SELECT p FROM Payment p "
          ...
          + "ORDER BY p.createdAt DESC")
  Page<Payment> findPaymentsByStatus(
      @Param("status") Payment.PaymentStatus status, Pageable pageable);

  /**
   * Get payments by status with refunds for payout eligibility checking Eager loads refunds to
  ...
  @Query(
      "SELECT DISTINCT p FROM Payment p "
          ...
          + "WHERE p.status = :status "
          + "ORDER BY p.createdAt DESC")
  Page<Payment> findPaymentsByStatusWithRefunds(
      @Param("status") Payment.PaymentStatus status, Pageable pageable);

  /**
   * Get payments for a specific user (admin view) Includes all payment details for administrative
   * purposes
   */
  @Query(
      "SELECT p FROM Payment p "
  ...
  /**
   * Get payments for a specific course (admin view) Useful for admin to see all payments for a
   * particular course
   */
  @Query(
      "SELECT p FROM Payment p "
          ...
          + "ORDER BY p.createdAt DESC")
  Page<Payment> findPaymentsByCourseIdForAdmin(
      @Param("courseId") String courseId, Pageable pageable);

  /**
   * Search payments by user email or course title (admin functionality) Allows admin to search for
   * payments using partial matches
   */
  @Query(
      "SELECT p FROM Payment p "
          + "LEFT JOIN FETCH p.user u "
  ...
  /**
   * Get all payments with advanced search and filtering for admin Supports search by payment ID,
   * user name, or course title Supports filtering by status and date range
   */
  @Query(
      "SELECT p FROM Payment p "
          + "LEFT JOIN FETCH p.user u "
  ...
  /**
   * Get all payments with advanced search and filtering for admin (no pagination) Supports search
   * by payment ID, user name, or course title Supports filtering by status and date range
   */
  @Query(
      "SELECT p FROM Payment p "
          + "LEFT JOIN FETCH p.user u "
