 ...
 * - Initiating password reset with email/OTP
...
public class PasswordResetController {
     ...
     * - Returns masked email and expiry information
     * 
     * Rate Limiting:
    ...
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<ForgotPasswordResponseDto>> initiateForgotPassword(
        ...
        log.info("Password reset initiation requested for email pattern: {}***",
        ...
        try {
            ...
            log.info("Password reset OTP sent successfully. Expiry: {}, Previous tokens invalidated: {}",
                    response.getExpiresAt(), response.getPreviousTokensInvalidated());

            return ApiResponseUtil.success(
                    response,
                    "Password reset OTP sent to your email address. Please check your inbox.");
     ...
     * 
     * This endpoint:
     * - Validates the OTP code and user email
     * - Checks password strength requirements
     * - Verifies password confirmation match
     * - Updates user's password with encryption
     * - Marks the OTP token as used
     * - Logs the successful password change
     * 
     * Security Features:
     ...
     * 
     * @param request the reset password confirmation request with OTP and new
     *                password
    ...
    public ResponseEntity<project.ktc.springboot_app.common.dto.ApiResponse<Void>> confirmPasswordReset(
        ...
        log.info("Password reset confirmation requested for email pattern: {}***",
                request.getEmail().substring(0, Math.min(3, request.getEmail().length())));

        try {
            passwordResetService.confirmPasswordReset(request);

            log.info("Password reset completed successfully for user with email pattern: {}***",
                    request.getEmail().substring(0, Math.min(3, request.getEmail().length())));
