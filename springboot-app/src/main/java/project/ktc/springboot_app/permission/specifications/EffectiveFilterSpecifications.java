 ...
 * Uses the new filter type system with FilterType entity
 */
@Slf4j
public class EffectiveFilterSpecifications {
    ...
    public static Specification<Course> applyCourseFilter() {
        return (Root<Course> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
            FilterType.EffectiveFilterType filter = CustomPermissionEvaluator.EffectiveFilterContext.getCurrentFilter();
    ...
    /**
     * Apply effective filter with additional business logic constraints
     * Combines effective filter with published/approved status
     * 
     ...
     *         constraints
     */
    public static Specification<Course> applyCourseFilterWithConstraints(boolean includePublishedOnly) {
        return (Root<Course> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
    ...
    /**
     * Generic method to apply effective filter to any entity with instructor
     * relationship
     * 
                ...
                case OWN -> {
                    ...
                    try {
                        ...
                        try {
                            ...
                            log.warn("Entity {} has neither instructor nor createdBy field for OWN filter",
    ...
    /**
     * Generic method to apply effective filter to any entity with createdBy
     * relationship
     * 
     * @param <T> the entity type
     * @return Specification that applies the current effective filter
