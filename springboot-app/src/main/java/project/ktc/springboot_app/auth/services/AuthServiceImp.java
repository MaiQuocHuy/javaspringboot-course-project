...
public class AuthServiceImp implements AuthService {
  ...
  public ResponseEntity<ApiResponse<Void>> registerUser(RegisterUserDto registerUserDto) {
    ...
    try {
      ...
      if (existingRoleOpt.isPresent()) {
        ...
        userRole = UserRole.builder().role(roleType).build();
  ...
  public ResponseEntity<ApiResponse<Void>> registerApplication(
    ...
    log.info(
        "Starting registration with application for role: {}", registerApplicationDto.getRole());

    // Validation checks
    if (registerApplicationDto.getEmail() == null
        || registerApplicationDto.getEmail().trim().isEmpty()) {
      return ApiResponseUtil.badRequest("Email cannot be null or empty");
    }
    if (registerApplicationDto.getPassword() == null
        || registerApplicationDto.getPassword().trim().isEmpty()) {
      return ApiResponseUtil.badRequest("Password cannot be null or empty");
    }
    if (registerApplicationDto.getName() == null
        || registerApplicationDto.getName().trim().isEmpty()) {
      return ApiResponseUtil.badRequest("Name cannot be null or empty");
    }
    ...
    if (registerApplicationDto.getRole() == UserRoleEnum.INSTRUCTOR) {
      ...
      if (certificate == null || certificate.isEmpty()) {
        return ApiResponseUtil.badRequest(
            "Certificate file is required for instructor application");
      }
    ...
    try {
      ...
      if (registerApplicationDto.getRole() == UserRoleEnum.INSTRUCTOR) {
        try {
          createInstructorApplication(
              savedUser, certificate, cv, other, registerApplicationDto.getPortfolio());
          ...
          notificationHelper
              .createInstructorApplicationSubmittedNotification(
                  savedUser.getId(), applicationId) // Use the actual application ID
              .thenAccept(
                  notification ->
                      log.info(
          ...
          log.error(
              "Failed to create instructor application for user: {}, error: {}",
              savedUser.getId(),
              e.getMessage());
      ...
      log.info(
          ...
          registerApplicationDto.getRole().name(),
          savedUser.getEmail());
  ...
  @Override
  @Transactional
  public ResponseEntity<ApiResponse<Map<String, Object>>> loginAdmin(
      LoginAdminDto dto, HttpServletResponse response) {
    // Validation checks
    if (dto == null) {
      return ApiResponseUtil.badRequest("Login data cannot be null");
    }
    ...
    try {
      ...
      if (foundUser.getRole() == null
          || foundUser
              .getRole()
              .getRole()
              .equals(UserRoleEnum.INSTRUCTOR.name().toUpperCase().trim())
          || foundUser
              .getRole()
              .getRole()
              .equals(UserRoleEnum.STUDENT.name().toUpperCase().trim())) {
      ...
      String cookieValue =
          String.format(
              ...
              refreshTokenStr, 30 * 24 * 60 * 60 // 30 days
              );
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> googleLogin(GoogleLoginDto dto) {
    ...
    try {
      ...
      log.error(
          "Error during Google login for email: {}, error: {}", dto.getEmail(), e.getMessage(), e);
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> refreshAccessToken(String refreshToken) {
    ...
    try {
      ...
      Map<String, Object> refreshResponse =
          Map.of("accessToken", newAccessToken, "refreshToken", existingRefreshToken.getToken());
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> resetPassword(
    ...
    try {
      ...
      return ApiResponseUtil.success(
          Map.of("message", "Password reset successfully"), "Password reset successful");
  ...
  public ResponseEntity<ApiResponse<Void>> logout(String refreshToken) {
    ...
    try {
      ...
      log.info(
          "Successfully logged out user with token: {}",
  ...
  private void createInstructorApplication(
      User user,
      MultipartFile certificate,
      MultipartFile cv,
      MultipartFile other,
    ...
    try {
      ...
      Optional<InstructorApplication> existingApplication =
          instructorApplicationRepository.findByUserId(user.getId());
