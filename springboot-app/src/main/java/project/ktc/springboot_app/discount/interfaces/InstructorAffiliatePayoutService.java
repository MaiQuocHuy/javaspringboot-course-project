...
public interface InstructorAffiliatePayoutService {
  ...
  /**
   * Get all affiliate payouts for courses owned by the instructor (Shows who gets commission on
   * instructor's courses)
   *
   * @param pageable Pagination parameters
   * @return ResponseEntity containing paginated list of affiliate payouts
   */
  ResponseEntity<ApiResponse<PaginatedResponse<InstructorAffiliatePayoutResponseDto>>>
      getAffiliatePayouts(Pageable pageable);

  /**
   * Get all affiliate payouts for courses owned by the instructor with search and filter options
   * (Shows who gets commission on instructor's courses)
   *
   * @param search Search term for affiliate payout ID, referrer name, course title, or discount
   *     code
