 ...
 * 
 * Provides data access methods for managing password reset tokens including
 * OTP verification, token cleanup, and user-specific token management.
 * 
 * @author KTC Team
 * @version 1.0
 * @since 2025-01-26
...
public interface PasswordResetTokenRepository extends JpaRepository<PasswordResetToken, String> {
    ...
    /**
     * Finds a valid password reset token by user ID and OTP code.
     * A valid token is one that is not used, not expired, and hasn't reached max
     * attempts.
     * 
    ...
    /**
     * Finds the most recent password reset token for a user.
     * Useful for checking rate limiting and preventing spam requests.
     * 
     * @param userId the user ID
    ...
    /**
     * Finds all unexpired and unused tokens for a specific user.
     * Used for invalidating previous tokens when a new one is generated.
     * 
     * @param userId      the user ID
     * @param currentTime the current timestamp for expiration check
    ...
    /**
     * Invalidates (marks as used) all active tokens for a user.
     * Called when a new password reset token is generated to ensure only one valid
     * token exists.
     * 
     * @param userId      the user ID
     * @param currentTime the current timestamp for expiration check
     ...
     * Should be called periodically (e.g., via scheduled task) to maintain database
     * performance.
     * 
    ...
    @Query("DELETE FROM PasswordResetToken prt " +
            "WHERE prt.expiresAt < :currentTime")
    int deleteExpiredTokens(@Param("currentTime") LocalDateTime currentTime);

    /**
     * Cleans up old used tokens to prevent database bloat.
     * Removes tokens that were used more than a specified number of days ago.
     * 
     * @param cutoffTime the cutoff timestamp (tokens older than this will be
     *                   deleted)
     * @return number of tokens deleted
     */
    @Modifying
     ...
     * window.
     * Used for implementing rate limiting to prevent abuse.
     * 
     * @param userId   the user ID
     * @param fromTime the start of the time window
     * @return count of requests in the time window
     */
    @Query("SELECT COUNT(prt) FROM PasswordResetToken prt " +
            "WHERE prt.user.id = :userId " +
            "AND prt.createdAt >= :fromTime")
    long countTokensCreatedByUserSince(@Param("userId") String userId, @Param("fromTime") LocalDateTime fromTime);

    /**
     * Finds a token by its OTP code regardless of user (for uniqueness checks).
     * Used to ensure OTP codes are unique across the system.
     * 
     * @param otpCode     the OTP code
     * @param currentTime the current timestamp for expiration check
    ...
    /**
     * Finds all tokens that will expire within a specified time.
     * Useful for sending reminder notifications or cleanup warnings.
     * 
     * @param expiryTime the expiry threshold
