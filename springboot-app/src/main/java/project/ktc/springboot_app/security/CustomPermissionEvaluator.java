...
/**
 * Custom Permission Evaluator for Spring Security @PreAuthorize Uses the new database schema with
 * filter_types table
 */
@Component
@Primary
...
public class CustomPermissionEvaluator implements PermissionEvaluator {
  ...
  public boolean hasPermission(
    ...
    try {
      AuthorizationService.AuthorizationResult result =
          authorizationService.evaluatePermission(user, permissionKey);
  ...
  /**
   * Evaluate permission for target by ID This method handles instance-level permission checks with
   * ResourceOwnershipService integration.
   *
   * @param authentication the authentication object
   * @param targetId the target resource ID (UUID string)
   * @param targetType the target resource type (e.g., "Course", "Review", "Enrollment", "User")
   * @param permission the permission string (e.g., "course:READ", "course:WRITE")
   * @return true if access is granted
   */
  @Override
  public boolean hasPermission(
    ...
    try {
      ...
      boolean hasInstanceAccess =
          checkInstanceLevelAccess(user, resourceId, targetType, result.getEffectiveFilter());
  ...
  public static class EffectiveFilterContext {
    private static final ThreadLocal<FilterType.EffectiveFilterType> currentFilter =
        new ThreadLocal<>();
