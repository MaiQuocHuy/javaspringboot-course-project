...
/**
 * Permission Role Assign Rule Entity Defines which permissions can be assigned to which roles Only
 * permissions defined in this table can be assigned to the corresponding role
 */
@Entity
@Table(
    name = "permission_role_assign_rules",
    uniqueConstraints = {
      @UniqueConstraint(
...
public class PermissionRoleAssignRule extends BaseEntity {
  ...
  @JoinColumn(
      name = "role_id",
      nullable = false,
      foreignKey = @ForeignKey(name = "fk_permission_role_assign_rule_role"))
  ...
  @JoinColumn(
      name = "permission_id",
      nullable = false,
      foreignKey = @ForeignKey(name = "fk_permission_role_assign_rule_permission"))
