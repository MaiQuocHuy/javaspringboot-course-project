...
public class StudentLessonServiceImp implements StudentService {
   ...
   *
   * <p>This method handles lesson completion with the following logic: - If lesson was previously
   * completed, it updates the completion timestamp - If first completion, it creates a new lesson
   * completion record - Checks and updates course completion status if all lessons are completed -
  ...
  public ResponseEntity<ApiResponse<String>> completeLesson(String sectionId, String lessonId) {
    ...
    try {
      ...
      Optional<LessonCompletion> existingCompletionOpt =
          lessonCompletionRepository.findByUserIdAndLessonId(currentUserId, lessonId);
      ...
      if (existingCompletionOpt.isPresent()) {
        ...
        log.info(
            "Updating existing lesson completion time for student {} on lesson {}",
            currentUserId,
            lessonId);
      } else {
        ...
        completion.setCompletedAt(LocalDateTime.now());
        log.info(
            "Creating new lesson completion for student {} on lesson {}", currentUserId, lessonId);
      }

      lessonCompletionRepository.save(completion);
      log.info(
          "Successfully recorded lesson completion for student {} on lesson {}",
          currentUserId,
          lessonId);
      ...
      try {
        ...
        log.warn(
            "Failed to invalidate course structure cache for course: {}, error: {}",
            courseId,
            e.getMessage());
      ...
      return ApiResponseUtil.success(
          "Lesson completion recorded successfully", "Lesson marked as complete");
  ...
  private void checkAndUpdateCourseCompletion(String userId, String courseId) {
    try {
      ...
      if (totalLessons > 0 && completedLessons.equals(totalLessons)) {
        ...
        if (enrollmentOpt.isPresent()) {
          ...
          if (enrollment.getCompletionStatus() != Enrollment.CompletionStatus.COMPLETED) {
            ...
            log.info(
                "Updated enrollment status to COMPLETED for user {} in course {}",
                userId,
                courseId);
            ...
            ResponseEntity<ApiResponse<CertificateResponseDto>> createdCertificate =
            ...
            try {
              ...
              ApiResponse<CertificateResponseDto> responseBody =
                  createdCertificate != null ? createdCertificate.getBody() : null;
              CertificateResponseDto certificateData =
              ...
              if (certificateId != null) {
                notificationHelper
                    .createCertificateNotification(
                        userId, certificateId, courseName, certificateUrl)
                    .thenAccept(
                        notification ->
                            log.info(
                ...
                log.error(
                    "❌ Certificate creation returned null/invalid response for user {} course {}",
                    userId,
                    courseId);
              }

              log.info(
                  "🎓 Student {} completed course {} - certificate created and notification sent",
                  userId,
                  courseName);
      ...
      log.error(
          "Error checking course completion for user {} in course {}: {}",
          userId,
          courseId,
          e.getMessage(),
          e);
   ...
   *
   * <p>This method handles quiz submission with the following logic: - If quiz was previously
   * submitted, it updates the existing result and completion time - If first submission, it creates
   * new quiz result and lesson completion records - Automatically marks lesson as completed after
   * successful quiz submission - Checks and updates course completion status if all lessons are
   * completed
   *
   * @param sectionId The ID of the section containing the lesson
  ...
  public ResponseEntity<ApiResponse<QuizSubmissionResponseDto>> submitQuiz(
    ...
    try {
      ...
      Optional<QuizResult> existingResultOpt =
          quizResultRepository.findByUserIdAndLessonId(currentUserId, lessonId);
      ...
      if (existingResultOpt.isPresent()) {
        ...
        log.info(
            "Overwriting existing quiz result for student {} on lesson {}",
            currentUserId,
            lessonId);
      } else {
        // Create new result
      ...
      // 12. Create response
      QuizSubmissionResponseDto responseDto =
          QuizSubmissionResponseDto.builder()
              .score(score)
      ...
      log.info(
          "Successfully processed quiz submission for student {} on lesson {}. Score: {}/{}",
          currentUserId,
          lessonId,
          correctAnswers,
          totalQuestions);
  ...
  private String generateFeedback(int correctAnswers, int totalQuestions, BigDecimal score) {
    if (score.compareTo(BigDecimal.valueOf(90)) >= 0) {
      return String.format(
          "Excellent work! You answered %d out of %d questions correctly.",
          correctAnswers, totalQuestions);
    } else if (score.compareTo(BigDecimal.valueOf(70)) >= 0) {
      return String.format(
          "Great job! You answered %d out of %d questions correctly.",
          correctAnswers, totalQuestions);
